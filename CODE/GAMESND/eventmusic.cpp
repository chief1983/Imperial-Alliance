 ...
 *
 * $Log: eventmusic.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:40  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:56:55  Darkhill
 * no message
 *
...
int hostile_ships_present()
{
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {
		...
		if ( (shipp->flags & (SF_DYING|SF_DEPARTING|SF_ENGINES_DISABLED)) || (shipp->flags2 & SF2_DISABLED) || (ship_subsys_disrupted(shipp, SUBSYSTEM_ENGINE)) ) 
			continue;
