...
BEGIN_MESSAGE_MAP(CFREDApp, CWinApp)
	...
	ON_COMMAND(ID_TOOLS_FILE_NEW, CWinApp::OnFileNew)
	ON_COMMAND(ID_FILE_NEW, CWinApp::OnFileNew)
...
bool cmdline_load = false;
CString cmdline_path;
...
{	
	...
	// Goober5000
	Format_fs2_open = 1;//GetProfileInt("Preferences", "FS2 open format", Format_fs2_open);
	...
	*str_end = '\0';
	...
	if (cmdInfo.m_strFileName[0] != 0) 
		FREDDoc_ptr->OnOpenDocument(cmdInfo.m_strFileName);
...
void CFREDApp::write_ini_file(int degree)
{
	...
	WriteProfileString("Folders", "Last Used", current_dir);
...
void CFREDApp::read_window(char *name, window_data *wndd)
{
	...
	// read in last used folder
	current_dir = GetProfileString("Folders", "Last Used");
...
{
	char *path = "http://mb.emperorshammer.net/viewforum.php?f=38";
