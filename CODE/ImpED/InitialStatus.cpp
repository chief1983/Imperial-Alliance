...
void initial_status::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_SFOILS,	m_sfoils_open);
...
BEGIN_MESSAGE_MAP(initial_status, CDialog)
	...
	ON_BN_CLICKED(IDC_SFOILS, OnSfoils)
...
BOOL initial_status::OnInitDialog() 
{
	...
	if (Ship_info[Ships[Objects[cur_object_index].instance].ship_info_index].sfoils == 1) {
		if (Ships[Objects[cur_object_index].instance].sfoil_status & SFOIL_OPEN)
			m_sfoils_open = 1;
		else
			m_sfoils_open = 0;
	} else
		GetDlgItem(IDC_SFOILS)->EnableWindow(FALSE);
	...
	if (m_multi_edit) {
		...
		while (objp != END_OF_LIST(&obj_used_list)) {
			if (((objp->type == OBJ_SHIP) || (objp->type == OBJ_START)) && (objp->flags & OF_MARKED)) {
				...
				if (m_sfoils_open == 1) {
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_CLOSED;
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_OPENING;
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_CLOSING;
					Ships[get_ship_from_obj(objp)].sfoil_status |= SFOIL_OPEN;
				} else if (!m_sfoils_open) {
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_OPEN;
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_OPENING;
					Ships[get_ship_from_obj(objp)].sfoil_status &= ~SFOIL_CLOSING;
					Ships[get_ship_from_obj(objp)].sfoil_status |= SFOIL_CLOSED;
		...
		if (m_sfoils_open) {
			Ships[m_ship].sfoil_status &= ~SFOIL_CLOSED;
			Ships[m_ship].sfoil_status &= ~SFOIL_OPENING;
			Ships[m_ship].sfoil_status &= ~SFOIL_CLOSING;
			Ships[m_ship].sfoil_status |= SFOIL_OPEN;
		} else if (!m_sfoils_open) {
			Ships[m_ship].sfoil_status &= ~SFOIL_OPEN;
			Ships[m_ship].sfoil_status &= ~SFOIL_OPENING;
			Ships[m_ship].sfoil_status &= ~SFOIL_CLOSING;
		...
		if (obj >= 0) {
			...
			// their parent does
			if ( (s2type & SIF_CARGO) || (s1type & (SIF_BIG_SHIP | SIF_MEDIUM_SHIP) ) ) {
				reset_arrival_to_false( m_docked_with );
			} else if ( (s1type & SIF_CARGO) || (s2type & (SIF_BIG_SHIP | SIF_MEDIUM_SHIP) ) ) {
...
void initial_status::OnSfoils()
{
	...
	((CButton *) GetDlgItem(IDC_SFOILS))->SetCheck(m_sfoils_open);
}
