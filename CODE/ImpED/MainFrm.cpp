...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_TOOLS_FILE_MISSIONNOTES, OnFileMissionnotes)
	ON_WM_LBUTTONUP()
	ON_WM_DESTROY()
...
#define FRED_HELP_URL "\\data\\impeddocs\\index.html"
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	Assert(z != -1);
	m_wndToolBar.SetButtonInfo(z, ID_NEW_SHIP_TYPE, TBBS_SEPARATOR, 185);
	...
	//  be dockable
	m_wndToolBar2.EnableDocking(CBRS_ALIGN_TOP | CBRS_ALIGN_BOTTOM);
...
void CMainFrame::OnClose()
{
	...
	if (FREDDoc_ptr->IsModified()) {
		...
		rval = MessageBox("You have not saved your work.\n(Which isn't surprising...)\nSave it now?",
			"ImpED",
...
//		m_new_ship_type_combo_box.AddString("Player Start");		
		m_new_ship_type_combo_box.AddString("Hyper buoy");
		...
		if (!fDisabled) {
			if (z >= Num_ship_types)
				...
				switch (Ship_info[z].species)	{
					...
					case SPECIES_ANCIENTS:	newTextColor = RGB(192, 0, 192);	break;
					case SPECIES_USER1:		newTextColor = RGB(0, 128, 128);	break;
					case SPECIES_USER2:		newTextColor = RGB(255, 255, 0);	break;
					case SPECIES_USER3:		newTextColor = RGB(255, 128, 0);	break;
					case SPECIES_USER4:		newTextColor = RGB(128, 128, 128);	break;
...
void url_launch(char *url)
{
	...
	if (r < 32) {
		...
		switch (r) {
			...
			case ERROR_PATH_NOT_FOUND: txt =	XSTR("\r\nUnable to locate ImpED Help file: \\data\\impeddocs\\index.html\r\n", 1479); break;
...
{
	m_wndToolBar2.ShowWindow((m_wndToolBar2.GetStyle() & WS_VISIBLE) == 0);
	RecalcLayout();
...
BOOL CMainFrame::OnSaveDocument(LPCTSTR pathname)
{
