 ...
 *
 * $Log: FREDView.cpp,v $
 * Revision 1.12  2004/06/15 20:50:28  wmcoolmon
 * hud-set-color, current-speed, and FRED fixes (Wouldn't compile).
 *
 * Revision 1.11  2004/04/03 18:06:35  Kazan
 * Fixes:
 *
 * Revision 1.10  2004/02/13 04:25:37  randomtiger
 ...
 *
 * Revision 1.9  2004/02/05 23:23:19  randomtiger
 * Merging OGL Fred into normal Fred, no conflicts, should be perfect
 *
 * Revision 1.8.2.5  2004/01/24 13:00:43  randomtiger
 ...
 *
 * Revision 1.8.2.4  2004/01/20 19:11:29  randomtiger
 * Cut unneeded code, text now works in graphics window.
 *
 * Revision 1.8.2.3  2004/01/19 00:53:37  randomtiger
 * More Fred OGL changes.
 *
 * Revision 1.8.2.2  2004/01/18 14:53:17  randomtiger
 * Textures work now, initialisation runs from cobe.lib not internally.
 *
 * Revision 1.8.2.1  2004/01/17 22:11:25  randomtiger
 ...
 *
 * Revision 1.8  2003/10/15 22:10:34  Kazan
 * Da Species Update :D
 *
 * Revision 1.7  2003/09/05 05:32:36  Goober5000
 * moved custom buttons from shield sys dialog and added dialog to set
 * ship flags globally
 * --Goober5000
 *
 * Revision 1.6  2003/08/26 20:32:41  Goober5000
...
extern vector Viewer_pos, Last_eye_pos, rel_movement_vec;
...
void CFREDView::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CFREDView, CView)
	...
	ON_COMMAND(ID_TOOLS_EDITORS_SHIPS, OnEditorsShips)
	ON_WM_KEYDOWN()
	ON_WM_KEYUP()
	ON_WM_SETFOCUS()
	ON_WM_KILLFOCUS()
	...
	ON_COMMAND(ID_TOOLS_EDITORS_GOALS, OnEditorsGoals)
	...
	ON_UPDATE_COMMAND_UI(ID_SELECT_LIST, OnUpdateSelectListDown)
	...
	ON_COMMAND(ID_TOOLS_EDITORS_WING, OnEditorsWing)
	ON_COMMAND(ID_EDITORS_PLAYER, OnEditorsPlayer)
	ON_COMMAND(ID_TOOLS_EDITORS_PLAYER, OnEditorsPlayer)
	ON_COMMAND(ID_EDITORS_ORIENT, OnEditorsOrient)
	ON_COMMAND(ID_TOOLS_EDITORS_ORIENT, OnEditorsOrient)
	ON_COMMAND(ID_EDITORS_EVENTS, OnEditorsEvents)
	ON_UPDATE_COMMAND_UI(ID_EDITORS_ORIENT, OnUpdateEditorsOrient)
	ON_COMMAND(ID_TOOLS_EDITORS_EVENTS, OnEditorsEvents)
	...
	ON_COMMAND(ID_TOOLS_EDITORS_BG_BITMAPS, OnEditorsBgBitmaps)
	...
	ON_COMMAND(ID_TOOLS_EDITORS_WAYPOINT, OnEditorsWaypoint)
	ON_COMMAND(ID_START_GROUPS, OnEditFlightGroups)
	...
	ON_COMMAND(ID_TOOLS_EDIT_UNDO, OnEditUndo)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_EDIT_UNDO, OnUpdateToolsEditUndo)
	ON_COMMAND(ID_EDITORS_BRIEFING, OnEditorsBriefing)
	ON_COMMAND(ID_TOOLS_EDITORS_BRIEFING, OnEditorsBriefing)
	ON_COMMAND(ID_EDITORS_DEBRIEFING, OnEditorsDebriefing)
	ON_COMMAND(ID_TOOLS_EDITORS_DEBRIEFING, OnEditorsDebriefing)
	...
	ON_COMMAND(ID_TOOLS_EDIT_DELETE_WING, OnEditDeleteWing)
	ON_COMMAND(ID_MARK_WING, OnMarkWing)
	ON_UPDATE_COMMAND_UI(ID_CONTROL_OBJ, OnUpdateControlObj)
	ON_COMMAND(ID_EDIT_DELETE, OnEditDelete)
	ON_COMMAND(ID_TOOLS_EDIT_DELETE, OnEditDelete)
	...
	ON_COMMAND(ID_TOOLS_CMD_BRIEF, OnCmdBrief)
...
void CALLBACK expire_game_proc( HWND wnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	...
	if ( expire_game == EXPIRE_BAD_CHECKSUM )
		MessageBox (wnd, "ImpED can no longer run due to internal overlay error", NULL, MB_OK | MB_ICONERROR |MB_TASKMODAL|MB_SETFOREGROUND);
	else
...
void CFREDView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	m_ship_list.AddString("test");

	CWnd* pBoxOne;
	CWnd *pWndPrev;
	pBoxOne = (CWnd*) GetDlgItem(IDC_SHIP_LIST);
	//GotoDlgCtrl(pBoxOne);

	if (pWndPrev = CWnd::FindWindow(NULL,"Ship Selection"))
	{
		//pWndPrev->m_ship_list.AddString("test");
		
		if (pWndPrev->GetDlgItem(IDC_SHIP_LIST))
		{
			//pWndPrev->ShowWindow(SW_HIDE);
//MessageBox("a");
		pWndPrev->SetDlgItemText(IDC_SHIP_LIST, "test");
		}
	}
	//pWndPrev->m_ship_list.AddItem("test", RGB(255,0,0));
	//pBoxOne->GetWindowText(c);
	//MessageBox(c);
...
void CFREDView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (button_down) {
		...
		if (Dup_drag == DUP_DRAG_OF_WING) {
			...
			sprintf(msg, "Add cloned ships to flight group %s?", Wings[Duped_wing].name);
...
//	GetParentFrame()->ActivateFrame();
	...
	if (objnum != -1) {
		...
		if (menu.LoadMenu(IDR_MENU_EDIT_POPUP)) {
			...
			shipPopup.AppendMenu(MF_STRING | MF_ENABLED, SHIP_TYPES + Id_select_type_jump_node, "Hyper buoy");
...
void CFREDView::OnMiscStatistics() 
{
	...
	sprintf(buf,
		"Number of Objects: %d\n"
		"Number of Ships:   %d\n"
		"Number of Flight Groups:   %d\n",
		num_objects, ship_get_num_ships(), num_wings);

	MessageBox(buf, "ImpED Statistics");
}
...
void CFREDView::OnSelectList() 
{
	...
	ship_select_down = !ship_select_down;
	
	if (ship_select_down == 1)
	{
	pDialog = new ship_select();
	BOOL ret = pDialog->Create(IDD_SHIP_SELECT,this);
      if(!ret)   //Create failed.
         AfxMessageBox("Error creating Dialog");
	 	  pDialog->ShowWindow(SW_SHOW);
	 }

	if (ship_select_down == 0)
		pDialog->EndDialog(0);
	
	//dlg.DoModal();
}

void CFREDView::OnUpdateSelectListDown(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(ship_select_down);
}

extern matrix Last_control_orient;
extern vector Last_control_pos;
...
void zoom(float factor)
{
	...
	//center.xyz.x = view_x;//rel_movement_vec.xyz.x;
	//center.xyz.y = view_y;//rel_movement_vec.xyz.y;
	//center.xyz.z = view_z;//rel_movement_vec.xyz.z;
	//ptr = &Objects[cur_object_index];

	//center = ptr->pos; //view_pos    view_orient.vec.fvec Last_control_orient.vec.fvec
	//view_pos.xyz.x = view_x;
	//view_pos.xyz.y = view_y;
	//view_pos.xyz.z = view_z;
	//dist = 1000.1f;
	vm_vec_scale_add(&view_pos, &center, &eye_orient.vec.fvec, -factor);
...
void reset_view()
{
	...
	vm_vec_make(&view_pos, 0.0f, 150.0f, -200.f);
...
void CFREDView::OnZoomExtents() 
{
	...
	//{
	test = new zoom_map();
	BOOL ret = test->Create(IDD_ZOOM_MAP,this);
      if(!ret)   //Create failed.
         AfxMessageBox("Error creating Dialog");
...
int query_single_wing_marked()
{
	...
	while (i--) {
		...
		if ((Objects[obj].type != OBJ_SHIP) && (Objects[obj].type != OBJ_START))
			Error(LOCATION, "Invalid objects detected in flight group \"%s\"", Wings[cur_wing].name);
...
void CFREDView::OnDisbandWing() 
{
	if (query_single_wing_marked()) {
		...
		FREDDoc_ptr->autosave("flight group disband");

	} else
		MessageBox("One and only one flight group must be selected for this operation");
}
...
//			return 1;
	...
	if (!stricmp(flight_group1, "None"))
		if (error("You need to have at least one player editable flight group."))
			return 1;
		...
		if ((ptr->type == OBJ_SHIP) || (ptr->type == OBJ_START)) {
			...
			if (wing >= 0) {  // ship is part of a wing, so check this
				if (wing < 0 || wing >= MAX_WINGS){  // completely out of range?
					return internal_error("A ship has an illegal flight group index");
				}

				j = Wings[wing].wave_count;
				if (!j){
					return internal_error("A ship is in a non-existent flight group");
				}

				if (j < 0 || j > MAX_SHIPS_PER_WING){
					return internal_error("Invalid number of ships in flight group \"%s\"", Wings[z].name);
				}
				...
				if (j < 0){
					return internal_error("Ship/flight group references are corrupt");
				}
			...
			if (i < 0 || i >= Num_jump_nodes){
				return internal_error("Object has illegal hyper buoy index");
			}
		} else {
	...
	for (i=0; i<MAX_WINGS; i++) {
		...
		if (j) {  // is wing being used?
			count++;
			if (j < 0 || j > MAX_SHIPS_PER_WING){
				return internal_error("Invalid number of ships in flight group \"%s\"", Wings[i].name);
			}

			while (j--) {
				obj = wing_objects[i][j];
				if (obj < 0 || obj >= MAX_OBJECTS){
					return internal_error("Flightgroup_objects has an illegal object index");
				}

				if (!query_valid_object(obj)){
					return internal_error("Flightgroup_objects references an unused object");
				}
...
// so this object has valid information, such as the instance.
				...
				if ((Objects[obj].type == OBJ_SHIP) || (Objects[obj].type == OBJ_START)) {
					...
					switch (ship_query_general_type(ship))
					{
						...
						default:
							if (error("Ship \"%s\" is an illegal type to be in a flight group", Ships[ship].ship_name)){
								return 1;
							}
					}
				} else {
					return internal_error("Flightgroup_objects of \"%s\" references an illegal object type", Wings[i].name);
				}

				if (Ships[ship].wingnum != i){
					return internal_error("Flight group/ship references are corrupt");
				}
				...
				if (team < 0){
					...
					if (error("ship teams mixed within same flight group (\"%s\")", Wings[i].name)){
						return 1;
					}
				}
			}
			...
			if (Wings[i].threshold + Wings[i].wave_count > MAX_SHIPS_PER_WING) {
				...
				if(error("Threshold for flight group \"%s\" is higher than allowed.  Reset to %d", Wings[i].name, Wings[i].threshold)){
					return 1;
				}
			}
			...
			for (j=0; j<obj_count; j++){
				if (names[j]){
					if (!stricmp(names[j], Wings[i].name)){
						return internal_error("Flight group name is also used by an object (%s)", names[j]);
					}
				}
			}

			if(fred_check_sexp(Wings[i].arrival_cue, OPR_BOOL, "arrival cue of flight group \"%s\"", Wings[i].name)){
				return -1;
			}

			if(fred_check_sexp(Wings[i].departure_cue, OPR_BOOL, "departure cue of flight group \"%s\"", Wings[i].name)){
				return -1;
			}

			if (Wings[i].arrival_location != ARRIVE_AT_LOCATION) {
				if (Wings[i].arrival_anchor < 0)
					if (error("Flight group \"%s\" requires a valid arrival target", Wings[i].name))
						return 1;
			}

			if (Wings[i].departure_location != DEPART_AT_LOCATION) {
				if (Wings[i].departure_anchor < 0)
					if (error("Flight group \"%s\" requires a valid departure target", Wings[i].name))
						return 1;
			}

			if ((str = error_check_initial_orders(Wings[i].ai_goals, -1, i))>0) {
				if (*str == '*')
					return internal_error("Initial orders error for flight group \"%s\"\n\n%s", Wings[i].name, str + 1);
				else if (*str == '!')
					return 1;
				else if (error("Initial orders error for flight group \"%s\"\n\n%s", Wings[i].name, str))
					return 1;
			}
	...
	if (count != num_wings){
		return internal_error("num_fgs is incorrect");
	}
...
/*	for (i=0; i<num_messages; i++) {
	...
	for (i=0; i<MAX_WINGS; i++) {
		...
		if ( starting_wing && (Wings[i].flags & WF_REINFORCEMENT) ) {
			if ( error("Starting flight group %s marked as reinforcement.  This flight group\nshould either be renamed, or unmarked as reinforcement.", Wings[i].name) ){
				return 1;
			}
		}
		...
		for ( j = 0; j < Wings[i].wave_count; j++ ) {
			...
			if ( j == 0 ) {
				...
				if (error("Flight group %s has ships with different player orders which\nare ignored.  They must all be the same", Wings[i].name ) ){
					return 1;
				}
			}
		}
		...
		if ( starting_wing ) {
			if ( starting_orders == -1 ) {
				...
				if ( starting_orders != default_orders ) {
					if ( error("Player starting flight group %s has orders which don't match other starting wings\n", Wings[i].name) ){
						return 1;
					}
				}
			}
	...
	if (Num_jump_nodes < 0 || Num_jump_nodes > MAX_JUMP_NODES){
		return internal_error("Hyper buoy count is illegal");
	}
...
int CFREDView::global_error_check_mixed_player_wing(int w)
{
	...
	if (mixed)
		if (error("%s flight group must all be of the same species", Wings[w].name))
			return 1;
...
int CFREDView::global_error_check_player_wings(int multi)
{
	...
	// wing name checking.
	// removed to enable variable wing names
...
char *error_check_initial_orders(ai_goal *goals, int ship, int wing)
{
	...
	for (i=0; i<MAX_AI_GOALS; i++) {
		switch (goals[i].ai_mode) {
			...
			case AI_GOAL_DOCK:
				if (ship < 0)
					return "Flight groups can't dock";
				// fall through..
		...
		if (flag > 0) {
			...
			if (!stricmp(goals[i].ship_name, source))
				if (ship >= 0)
					...
					return "Target of flight group's goal is itself";
		}
		...
		if (flag == 1) {  // target waypoint required
			...
			if (wing >= 0) {  // check if target ship is in wing
				if (Ships[inst].wingnum == wing && Objects[Ships[inst].objnum].type != OBJ_START)
					return "Target ship of flight group's goal is within said flight group";
			}
			...
			if (j >= MAX_WINGS)
				return "*Invalid target flight group name";

			if (ship >= 0) {  // check if ship is in target wing
				if (Ships[ship].wingnum == j)
					return "Target flight group of ship's goal is same flight group said ship is part of";
			}
			...
			case AI_GOAL_DISABLE_ENGINES:
				if (team == team2) {
					if (ship >= 0)
						...
						return "Flight group assigned to attack same team";
				}
...
void CFREDView::OnRunFreespace() 
{
	...
	r = CreateProcess(".\\ImperialAlliance.exe", NULL, NULL, NULL, FALSE, 0, NULL, "./", &si, &pi);
...
void CFREDView::OnUpdateToolsEditUndo(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(Undo_available);
}
...
void CFREDView::OnInitialUpdate() 
{
	...
	if ( memcmp(ptr, DEFAULT_CHECKSUM_STRING, strlen(DEFAULT_CHECKSUM_STRING)) ) {
		...
		ptr = &stamp[0];
		ptr += 8;
		sprintf(text, "This version of ImpED has been compiled for %s", ptr);
...
void CFREDView::OnEditFlightGroups()
{
	CDialog *fg_edit;
	
	fg_edit = new fg_editor();
	BOOL ret = fg_edit->Create(IDD_STARTING_GROUPS,this);
     if(!ret)   //Create failed.
         AfxMessageBox("Error creating Dialog");
	 
	fg_edit->ShowWindow(SW_SHOW);
