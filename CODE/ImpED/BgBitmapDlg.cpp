 ...
 * $Revision: 1.4 $
 * $Date: 2003/10/23 23:52:26 $
 * $Author: phreak $
 *
 * Background space images manager dialog
 *
 * $Log: BgBitmapDlg.cpp,v $
...
//	DDX_Text(pDX, IDC_PITCH, m_pitch);
	...
	DDX_Check(pDX, IDC_NO_NEB_TRAILS, m_no_neb_trails);
	//}}AFX_DATA_MAP
}
...
void bg_bitmap_dlg::OnClose() 
{
	...
	if(m_fullneb){		
		...
		if( m_no_neb_trails ) {
			The_mission.flags |= MISSION_FLAG_NO_NEB_TRAILS;
		} else {
			The_mission.flags &= ~MISSION_FLAG_NO_NEB_TRAILS;
		}
...
{		
	...
	if(m_fullneb){
		...
		GetDlgItem(IDC_NO_NEB_TRAILS)->EnableWindow(TRUE);
...
//		GetDlgItem(IDC_HEADING)->EnableWindow(TRUE);		
		...
		GetDlgItem(IDC_NO_NEB_TRAILS)->EnableWindow(FALSE);
	}		
}
