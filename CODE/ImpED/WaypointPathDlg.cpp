...
void waypoint_path_dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBIndex(pDX, IDC_JUMP_IFF, m_iff);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(waypoint_path_dlg, CDialog)
	...
	//ON_WM_CLOSE()
	ON_WM_INITMENU()
	ON_CBN_SELCHANGE(IDC_JUMP_IFF, OnSelChangeIff)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void waypoint_path_dlg::initialize_data(int full_update)
{
	...
	box = (CComboBox *) GetDlgItem(IDC_JUMP_IFF);
	box->ResetContent();
	for (int i=0; i<Num_iff; i++){
		box->AddString(Iff_names[i]);
	}

	if (iff_enable) 
		box->SetCurSel(Jump_nodes[Objects[cur_object_index].instance].iff);

	GetDlgItem(IDC_NAME)->EnableWindow(enable);
	GetDlgItem(IDC_JUMP_IFF)->EnableWindow(iff_enable);
}
...
int waypoint_path_dlg::update_data(int redraw)
{
	...
	int i, z, inst;
	object *ptr;
	CComboBox *box = (CComboBox *) GetDlgItem(IDC_JUMP_IFF);
	...
	if (cur_waypoint_list >= 0) {
		...
		for (i=0; i<MAX_WINGS; i++)
			if (!stricmp(Wings[i].name, m_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This waypoint path name is already being used by a flight group\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		for (i=0; i<Num_jump_nodes; i++)
			if (!stricmp(Jump_nodes[i].name, m_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This waypoint path name is already being used by a hyper buoy\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		if (!strnicmp(m_name, "player ", 7)) {
			...
			bypass_errors = 1;
			z = MessageBox("Hyper buoy names can't start with the word 'player'\n"
				"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		for (i=0; i<MAX_WINGS; i++)
			if (!stricmp(Wings[i].name, m_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This hyper buoy name is already being used by a wing\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		while (ptr != END_OF_LIST(&obj_used_list)) {
			if (ptr->type == OBJ_SHIP) {
				if (!stricmp(m_name, Ships[ptr->instance].ship_name)) {
					...
					bypass_errors = 1;
					z = MessageBox("This hyper buoy name is already being used by a ship\n"
						"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		for (i=0; i<MAX_WAYPOINT_LISTS; i++)
			if (Waypoint_lists[i].count && !stricmp(Waypoint_lists[i].name, m_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This hyper buoy name is already being used by a waypoint path\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		for (i=0; i<Num_jump_nodes; i++)
			if ((i != inst) && !stricmp(Jump_nodes[i].name, m_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This hyper buoy name is already being used by another hyper buoy\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
		...
		string_copy(Jumppoint_lists[inst].name, m_name, NAME_LENGTH, 1);
			...
			while (ptr != END_OF_LIST(&obj_used_list)) {
				if (ptr->type == OBJ_JUMP_NODE)
					if (ptr->instance == point) {
						mark_object(OBJ_INDEX(ptr));
						((CComboBox *)GetDlgItem(IDC_JUMP_IFF))->SetCurSel(Jump_nodes[ptr->instance].iff);
					}
				ptr = GET_NEXT(ptr);
			}
...
void waypoint_path_dlg::OnSelChangeIff()
{
	if (Objects[cur_object_index].type == OBJ_JUMP_NODE) {
		m_iff = ((CComboBox *)GetDlgItem(IDC_JUMP_IFF))->GetCurSel();
		Jump_nodes[Objects[cur_object_index].instance].iff = m_iff;
	}
