			...
			if (Asteroid_field.debris_genre == DG_ASTEROID) {
				...
				for (i=0; i<True_NumSpecies; i++) {
					if (Asteroid_field.field_debris_type[i] >= 0) {
						temp += Species_names[Asteroid_field.field_debris_type[i] & DEBRIS_NAME_MASK];
					}
				}
#else
				for (i=0; i<3; i++) {
...
void DumpStats::get_object_stats(CString &buffer)
{
	...
	CString temp;
	int num_small_ships, num_medium_ships, num_big_ships, num_huge_ships;

	memset(obj_type_count,0, sizeof(obj_type_count));
	num_small_ships = num_medium_ships = num_big_ships = num_huge_ships = 0;
	...
	for ( objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp) ) {
		...
		if ( (objp->type == OBJ_SHIP) || (objp->type == OBJ_START) ) {
			if ( Ship_info[Ships[objp->instance].ship_info_index].flags & SIF_SMALL_SHIP ) {
				num_small_ships++;
			} else if ( Ship_info[Ships[objp->instance].ship_info_index].flags & SIF_MEDIUM_SHIP ) {
				num_medium_ships++;
	...
	for (int i=0; i<MAX_OBJECT_TYPES; i++) {
		if (obj_type_count[i] > 0) {
			...
			case OBJ_WING:
				temp.Format("Flight Group Count: %d\r\n", obj_type_count[i]);
				buffer += temp;
				break;

			case OBJ_JUMP_NODE:
				temp.Format("Hyper buoy Count: %d\r\n", obj_type_count[i]);
				buffer += temp;
				break;
	...
	temp.Format("\tNum medium ships: %d\r\n", num_medium_ships);
	buffer += temp;
	...
	if (total_jumpnodes > 0) {
		temp.Format("\ttotal_hyper_buoy: %d\r\n", total_jumpnodes);
		buffer += temp;
	}
	...
	for (i=0; i<MAX_WINGS; i++) {
		if (Wings[i].wave_count > 0) {
			temp.Format("\tFlight Group Name: %s,  num_ships: %d,  num_waves: %d\r\n", Wings[i].name, Wings[i].wave_count, Wings[i].num_waves);
			buffer += temp;
	...
	for (i=0; i<MAX_WINGS; i++) {
		if (Wings[i].wave_count > 0) {
			if (Wings[i].hotkey != -1) {
				temp.Format("\tFlight Group name: %s, hotkey: F%d\r\n", Wings[i].name, (Wings[i].hotkey + 5));
				buffer += temp;
			}
		}
	}
...
void DumpStats::get_species_ship_breakdown(CString &buffer)
{
...
#if defined(MORE_SPECIES)
	for (species=0; species < True_NumSpecies; species++) {
		buffer += Species_names[species];
		buffer += "\r\n";
#else
	for (species=0; species<3; species++) {
		...
		buffer += "\tHyperbuoy, Escape pod, Mines, Space objects:\r\n";
		for ( objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp) ) {
			if (objp->type == OBJ_START || objp->type == OBJ_SHIP) {
				shipp = &Ships[objp->instance];
				...
				if (Ship_info[shipp->ship_info_index].species == species) {
					//if (shipp->wingnum == -1)
					//if (shipp->cargo1 > 0)
					if (Ship_info[shipp->ship_info_index].flags & (SIF_NAVBUOY | SIF_ESCAPEPOD | SIF_SENTRYGUN | SIF_SPACE_OBJECT)) {
						temp.Format("\t\tName: %s, Type: %s Cargo: %s\r\n", shipp->ship_name, Ship_info[shipp->ship_info_index].name, Cargo_names[shipp->cargo1]);
						buffer += temp;
					}
				}
		...
		buffer += "\tPlatform, Capital, Supercap:\r\n";
		for ( objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp) ) {
			if (objp->type == OBJ_START || objp->type == OBJ_SHIP) {
				shipp = &Ships[objp->instance];
