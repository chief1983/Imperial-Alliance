...
void ship_flags_dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_WEAPON_JAM, m_weapon_jam);
	//}}AFX_DATA_MAP
...
BOOL ship_flags_dlg::OnInitDialog() 
{
	...
	int scannable = 0, kamikaze = 0, no_dynamic = 0, red_alert_carry = 0, special_warp = 0, weapon_jammer = 0;
	object *objp;
		...
		if ((objp->type == OBJ_START) || (objp->type == OBJ_SHIP)) {
			if (objp->flags & OF_MARKED) {
				...
				if (first) {
					...
					weapon_jammer = (Ships[i].flags2 & SF2_JAMMER_ON) ? 1 : 0;
	...
	m_weapon_jam.SetCheck(weapon_jammer);
