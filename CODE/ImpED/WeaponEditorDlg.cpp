...
void WeaponEditorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBIndex(pDX, IDC_GUN4, m_gun4);
...
BOOL WeaponEditorDlg::OnInitDialog() 
{
	...
	if (m_multi_edit) {
		...
		while (ptr != END_OF_LIST(&obj_used_list)) {
			if (((ptr->type == OBJ_SHIP) || (ptr->type == OBJ_START)) && (ptr->flags & OF_MARKED)) {
				inst = ptr->instance;
				if (!(ship_get_SIF(inst) & (SIF_MEDIUM_SHIP | SIF_BIG_SHIP | SIF_HUGE_SHIP)))
					big = 0;
	...
	box = (CComboBox *) GetDlgItem(IDC_GUN4);
	box->AddString("None");
	for (i=0; i<end1; i++){
		box->AddString(Weapon_info[i].name);
	}
	...
	if (big)
		GetDlgItem(IDC_POD)->EnableWindow(FALSE);
	
	box = (CComboBox *) GetDlgItem(IDC_POD);
	box->AddString("None");
	box->AddString("Cloak");
	box->AddString("Solid Booster");
	box->AddString("Decoy");
	box->AddString("Jammer");
	box->AddString("S.L.A.M.");
	box->AddString("Turbocharger");
	box->AddString("Tractor");

	m_pod = Ships[m_ship].pod_weapon;
...
void WeaponEditorDlg::change_selection()
{
	...
	if (m_last_item >= 0) {
		...
		cur_weapon->primary_bank_weapons[3] = m_gun4 - 1;
		if (m_missile1 > 0)
	...
	m_gun1 = m_gun2 = m_gun3 = m_gun4 = m_missile1 = m_missile2 = m_missile3 = m_missile4 = -1;
	...
	if (cur_weapon->num_primary_banks > 3) {
		m_gun4 = cur_weapon->primary_bank_weapons[3] + 1;
...
void WeaponEditorDlg::update_pilot()
{
	...
	int pod_num = ((CComboBox*)GetDlgItem(IDC_POD))->GetCurSel();
	...
	Ships[m_ship].pod_weapon = pod_num;
}
