...
void render_one_model(object *objp)
{
	...
	if ((Show_ship_models || Show_outlines) && ((objp->type == OBJ_SHIP) || (objp->type == OBJ_START))){
		...
		if (!Cmdline_nohtl) {
		  	gr_set_proj_matrix( (4.0f/9.0f) * 3.14159f * View_zoom,  gr_screen.aspect*(float)gr_screen.clip_width/(float)gr_screen.clip_height, 1.0f,30000);
			gr_set_view_matrix(&Eye_position, &Eye_matrix);
		}
		else
		{
			j |= MR_FORCE_LOWER_DETAIL;
		}
...
skip:
	...
	// Another screw up by the FSO team.  The code above messes up the
	// waypoint displays and the z-axis lines of craft.  Put back in
	// the original code for displaying.  Maybe go back sometime and 
	// fix the above code.
...
//		if (!viewpoint || OBJ_INDEX(objp) != cur_object_index)
		{
			if (Fred_outline)	{
				...
				//if (temp_tex == 0)
					model_render(Ships[z].modelnum, &objp->orient, &objp->pos, j);//, -1, -1, Ships[0].replacement_textures);
				//else
				//	model_render(Ships[z].modelnum, &objp->orient, &objp->pos, j, -1, -1, &Fred_texture_replacements[0].new_texture_id);
...
//(int model_num, matrix *orient, vector * pos, uint flags = MR_NORMAL, int objnum = -1, int lighting_skip = -1, int *replacement_textures = NULL);
			model_subsystem	*psub;
			ship_subsys	*pss;
			
			for ( pss = GET_FIRST(&Ships[z].subsys_list); pss !=END_OF_LIST(&Ships[z].subsys_list); pss = GET_NEXT(pss) )
			{
				psub = pss->system_info;
			
				// do solar/radar/gas/activator rotation here
				ship_do_submodel_rotation(&Ships[z], psub, pss, objp);
			}
		}
				...
				*buf = 0;
				if (Show_ship_info)
				{
					if ((objp->type == OBJ_SHIP) || (objp->type == OBJ_START)) {
						...
						sprintf(buf, "%s\nHyper buoy", Jump_nodes[objp->instance].name);
					} else
						Assert(0);
				}
...
// RT FONT	gr_set_font(Fred_font);
	...
	if (Briefing_dialog) {
		//int b=0;
		for (int b=0; b<ptr->num_tags; b++) {
			gr_set_color_fast(&colour_white);
			gr_string_win(fl2i(ptr->tag[b].pos.xyz.x), fl2i(ptr->tag[b].pos.xyz.z), ptr->tag[b].text);
		}
	}
	...
	if (query_valid_object(Cursor_over)) {  // display a tool-tip like infobox
		...
		} else if (Objects[Cursor_over].type == OBJ_JUMP_NODE) {
			sprintf(buf, "%s\nHyper buoy\n( %.1f , %.1f , %.1f ) ", Jump_nodes[inst].name,
