...
ai_goal_list Ai_goal_list[] = {
	...
	"Disable ship",				AI_GOAL_DISABLE,
	"Disable engines",			AI_GOAL_DISABLE_ENGINES,
...
bool fred_init()
{
	...
	if (!vm_init(64*1024*1024)) {
		MessageBox( NULL, "Not enough memory to run ImpEd.\r\nTry closing down some other applications.\r\n", "Not Enough Memory", MB_OK );
		return false;
	}
...
//	os_set_window((uint) hwndApp);
	...
	*/
	...
	// needs to be set to zero when models have been uv mapped
	Cmdline_nohtl = 1;
	Cmdline_FRED2_htl = 0;  
...
//	Fred_font = gr_init_font("font01.vf");
	...
	Init_Species_Definitions();		// Get the species stuff loaded! - Kazan
...
int create_ship(matrix *orient, vector *pos, int ship_type)
{
	...
	Objects[obj].phys_info.speed = 100.0f;
	...
	case SPECIES_TERRAN:
		...
		Ships[ship].iff = IFF_COLOR_IMPERIAL;
		break;
	case SPECIES_VASUDAN:
		Ships[ship].team = TEAM_HOSTILE;
		Ships[ship].flags &= ~SF_CARGO_REVEALED;
		Ships[ship].iff = IFF_COLOR_REBEL;
		break;
	case SPECIES_SHIVAN:
		Ships[ship].team = TEAM_UNKNOWN;
		Ships[ship].flags &= ~SF_CARGO_REVEALED;
		Ships[ship].iff = IFF_COLOR_BLUE;
	}
	/*if (z1 == SPECIES_SHIVAN || z1 == SPECIES_VASUDAN) {
		Ships[ship].team = TEAM_HOSTILE;
		Ships[ship].flags &= ~SF_CARGO_REVEALED;
	...
	Ships[ship].sfoil_status |= SFOIL_CLOSED;
	

	// calc kamikaze stuff
	if (Ships[ship].special_hitpoint_index != -1)
	{
		temp_initial_hull_strength = (float) atoi(Sexp_variables[Ships[ship].special_hitpoint_index+HULL_STRENGTH].text);
	}
	else
	{
	...
	Ai_info[Ships[ship].ai_index].kamikaze_damage = min(1000.0f, 200.0f + (temp_initial_hull_strength / 4.0f));
...
int create_object(vector *pos, int list)
{
	...
	if (cur_model_index == Id_select_type_waypoint)
		...
		if (Num_jump_nodes >= MAX_JUMP_NODES) {
			Fred_main_wnd->MessageBox("Unable to create more hyper buoy.  You have reached the limit.", NULL, MB_OK | MB_ICONEXCLAMATION);
			obj = -2;
...
void clear_mission()
{
	...
	strcpy(The_mission.notes, "This is a ImpED created mission.\n");
	...
	*Mission_text = *Mission_text_raw = EOF_CHAR;
	...
	The_mission.red_alert = 0;
	The_mission.scramble = 0;
...
int common_object_delete(int obj)
{
	...
	if (type == OBJ_START) {
		...
		while (i < Num_jump_nodes - 1) {
			...
			Jumppoint_lists[i] = Jumppoint_lists[i+ 1];
			i++;
		}
	...
	if (r) {
		...
		switch (r & SRC_MASK) {
			...
			case SRC_WING_ARRIVAL:
				sprintf(text, "the arrival cue of flight group \"%s\"", Wings[n].name);
				break;

			case SRC_WING_DEPARTURE:
				sprintf(text, "the departure cue of flight group \"%s\"", Wings[n].name);
				break;
	...
	switch (code & SRC_MASK) {
		case SRC_SHIP_ORDER:
			...
			if (!query_initial_orders_empty(Ai_info[Ships[n].ai_index].goals))
				if ((Ships[n].wingnum >= 0) && (query_initial_orders_conflict(Ships[n].wingnum)))
					Fred_main_wnd->MessageBox("This ship's flight group also has initial orders", "Possible conflict");
		...
		case SRC_WING_ORDER:
			...
			if (query_initial_orders_conflict(n))
				Fred_main_wnd->MessageBox("One or more ships of this flight group also has initial orders", "Possible conflict");
...
{
	return (wing == wing_name_lookup("alpha", 1)) || (wing == wing_name_lookup("beta", 1)) || (wing == wing_name_lookup("gamma", 1));
}
