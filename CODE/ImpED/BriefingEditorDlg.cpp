 ...
 * $Revision: 1.3 $
 * $Date: 2002/08/15 04:35:44 $
 * $Author: penguin $
 *
 * Briefing editor dialog box class.
 *
 * $Log: BriefingEditorDlg.cpp,v $
...
void briefing_editor_dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBIndex(pDX, IDC_IFF, m_icon_iff);
...
BEGIN_MESSAGE_MAP(briefing_editor_dlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_IFF, OnSelchangeIff)
	...
	ON_BN_CLICKED(IDC_TAG, OnBriefTag)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void briefing_editor_dlg::create()
{
	...
	box = (CComboBox *) GetDlgItem(IDC_IFF);
	for (i=0; i<Num_iff; i++)
...
void briefing_editor_dlg::update_data(int update)
{
	...
	if (m_last_stage >= 0) {
		...
		if (m_last_icon >= 0) {
			...
			ptr->icons[m_last_icon].iff = m_icon_iff;
			if ((ptr->icons[m_last_icon].iff != m_icon_iff) && (!m_change_local)) {
				set_modified();
				reset_icon_loop(m_last_stage);
				while (get_next_icon(m_id))
					iconp->iff = m_icon_iff;
			}
...
*/
	...
	GetDlgItem(IDC_TAG) -> EnableWindow(enable);
	...
	if ((m_cur_stage >= 0) && (m_cur_icon >= 0) && (m_cur_icon < ptr->num_icons)) {
		...
		m_icon_iff= ptr->icons[m_cur_icon].iff;
...
//		m_icon_text = convert_multiline_string(ptr->icons[m_cur_icon].text);
	...
	GetDlgItem(IDC_IFF) -> EnableWindow(enable);
...
void briefing_editor_dlg::OnAddStage() 
{
	...
	Briefing->stages[m_cur_stage].tag = (brief_tag *)malloc(sizeof(brief_tag)*MAX_TAG_LINES);
	Briefing->stages[m_cur_stage].num_tags = 0;
...
{
	char *name;
	int z, len, team, iff, ship, waypoint, jump_node, count = -1;
...
void briefing_editor_dlg::OnSelchangeIff()
{
	update_data(1);
}
...
void briefing_editor_dlg::OnBriefTag()
{
	/*brief_stage *ptr = NULL;
	int num_tags;
	char c[10];
	...
	dlg = new brief_map_tag();
	BOOL ret = dlg->Create(IDD_BRIEF_TAG, this);

	if (!ret)
		AfxMessageBox("Error creating briefing tag dialog");
	...
	strcpy(ptr->tag[ptr->num_tags-1].text, "Fuck");
	ptr->tag[ptr->num_tags-1].pos.xyz.x = ptr->num_tags * 10;
	ptr->tag[ptr->num_tags-1].pos.xyz.z = ptr->num_tags * 10;*/
