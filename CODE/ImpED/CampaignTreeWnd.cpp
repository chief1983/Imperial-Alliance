...
void campaign_tree_wnd::OnCpgnFileOpen() 
{
	...
	CFileDialog dlg(TRUE, "iac", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Imperial Alliance Campaign files (*.iac)|*.iac||", this);
	if (dlg.DoModal() == IDOK)
	{
...
void campaign_tree_wnd::OnCpgnFileSaveAs() 
{
	...
	CFileDialog dlg(FALSE, "iac", old_name, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Imperial Alliance Campaign files (*.iac)|*.iac||", this);
	if (dlg.DoModal() == IDOK)
	{
		name = dlg.GetFileName();
		if (strlen(name) > MAX_FILENAME_LEN - 1) {
