...
//	m_debris_species = a_field[cur_field].debris_species;
	...
	int num_field_debris_info = 4 + ((True_NumSpecies*3)); // none, asteroid small|medium|big, and [species] debris small|medium|big
#else
	int num_field_debris_info = MAX_DEBRIS_TYPES + 1;
#endif
	// since most SW ships are build from the same material don't need
	// to have debris for each species.  so its been trimed down to one
	// type.
	for (idx=0; idx<(4 + True_NumSpecies)/*num_field_debris_info*/; idx++) {
