 ...
 * $Revision: 1.26 $
 * $Date: 2004/05/11 02:17:13 $
 * $Author: Goober5000 $
 *
 * Mission saving in Fred.
 *
 * $Log: MissionSave.cpp,v $
...
int CFred_mission_save::save_mission_info()
{
	...
	if (Format_fs2_open)
	{
		...
		if (strlen(The_mission.skybox_model) > 0)
		{
			...
			//kill of any extension, we will add one here
...
int CFred_mission_save::save_briefing()
{
	...
	for ( nb = 0; nb < Num_teams; nb++ ) {
		...
		for (i=0; i<Briefings[nb].num_stages; i++) {
			...
			if (bs->num_tags > 0) {
				sprintf(out, "$start_tag");
				fout("\n%s", out);
				//required_string_fred("$start_tag");
				sprintf(out, "+num_tags: %d", bs->num_tags);
				fout("\n%s", out);
			}
...
int CFred_mission_save::save_objects()
{
	...
	for (i=z=0; i<MAX_SHIPS; i++) {
		...
		required_string_either_fred("$Name:", "#Flight Groups");
		required_string_fred("$Name:");
		...
		if ( Ships[i].departure_location == DEPART_AT_LOCATION ) {
			required_string_fred("$Departure Jump Index:");
			parse_comments();

			if (Ships[i].jump_anchor >= 0) {
				// subtract 1 to take into account the "None" string added to the top of the list of
				// jump points
				fout(" %d", Ships[i].jump_anchor-1);
			} else {
				fout(" <error>");
			}
		}
		...
		if (Format_fs2_open)
		{
			...
			if (Ships[i].flags2 & SF2_JAMMER_ON)
				fout(" \"jammer\"");
			if ( Ships[i].sfoil_status & SFOIL_OPEN )
				fout(" \"sfoils-open\"");
			fout(" )");
		}
		// -----------------------------------------------------------
		...
		// i'm such a horrible person for doing this.  should really go back and fix all the pod
		// weapons to be not hard coded like this.  maybe someday, and someday monkeys might fly
		// out of my ass
		if (Ships[i].pod_weapon > 0) {
			fout("\n+POD: ");

			switch (Ships[i].pod_weapon) {
				case 1:
					fout("Cloak");
					break;
				case 2:
...
int CFred_mission_save::save_common_object_data(object *objp, ship *shipp)
{
	...
	{
		temp_initial_hull_strength = (float) atoi(Sexp_variables[shipp->special_hitpoint_index+HULL_STRENGTH].text);
	}
	else
	{
		temp_initial_hull_strength = sip->initial_hull_strength;
	}

	if ((int) objp->hull_strength != temp_initial_hull_strength) {
...
int CFred_mission_save::save_wings()
{
	...
	fred_parse_flag = 0;
	required_string_fred("#Flight Groups");
	parse_comments(2);
	fout("\t\t;! %d total", num_wings);

	// not the way its supposed to be i think.  but this will save the
	// player editable group names
	fout("\n\n$Starting FGs:");
	fout("\n+FG 1: %s", flight_group1);
	fout("\n+FG 2: %s", flight_group2);
	fout("\n+Fg 3: %s", flight_group3);
	...
	for (i=0; i<MAX_WINGS; i++) {
		...
		if ( Wings[i].departure_location == DEPART_AT_LOCATION ) {
			...
			if (Wings[i].jump_anchor >= 0) {
				// subtract 1 to take into account the "None" string added to the top of the list of
				// jump points
				fout(" %d", Wings[i].jump_anchor-1);
			} else {
				fout(" <error>");
			}
		}
...
int CFred_mission_save::save_waypoints()
{
	...
	for (i=0; i<Num_jump_nodes; i++) {
		...
		required_string_fred("$Hyper Buoy:", "$Hyper Buoy Name:");
		parse_comments(2);
		save_vector(ptr->pos);

		required_string_fred("$Hyper Buoy Name:", "$Hyper Buoy:");
		parse_comments();
		fout(" %s", Jump_nodes[i].name);

		required_string_fred("$IFF:");
		parse_comments();
		fout( " %s", Iff_names[Jump_nodes[i].iff]);
	}
...
void CFred_mission_save::save_ai_goals(ai_goal *goalp, int ship)
{
	...
	for (i=0; i<MAX_AI_GOALS; i++) {
		...
		if (goalp[i].ai_mode == AI_GOAL_CHASE_ANY) {
			...
			fout("( ai-hyper-out %d ) ", goalp[i].priority);
			...
			if (!goalp[i].ship_name) {
				...
				switch (goalp[i].ai_mode) {
					...
					case AI_GOAL_DOCK:
						if (ship < 0) {
							valid = 0;
							Warning(LOCATION, "Flight group aren't allowed to have a docking goal\n");
