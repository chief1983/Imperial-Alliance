...
void ShipGoalsDlg::initialize(ai_goal *goals, int ship)
{
	...
	for (item=0; item<ED_MAX_GOALS; item++) {
		...
		if (flag & 0x8) {
			...
			//	if (!stricmp(goalp[item].ship_name, Jumppoint_lists[i].name)) {
					//MessageBox(goalp[item].ship_name);
					m_data[item] = 24576;
			//char e[10];
			//sprintf(e, "init %d", m_data[item]);
			//MessageBox(e);
			//		break;
			//	}
		}
		...
		if (flag & 0x4) {  // data is a waypoint path name
			for (i=0; i<Num_waypoint_lists; i++)
				if (!stricmp(goalp[item].ship_name, Waypoint_lists[i].name)) {
					char a[100];
					sprintf(a, "%d %s", i, goalp[item].ship_name);
					MessageBox(a);
...
void ShipGoalsDlg::set_item(int item, int init)
{
	...
	switch (mode) {
		case AI_GOAL_CHASE | AI_GOAL_CHASE_WING:
		case AI_GOAL_GUARD | AI_GOAL_GUARD_WING:
		case AI_GOAL_DISABLE | AI_GOAL_DISABLE_ENGINES:
			for (i=0; i<MAX_WINGS; i++)
	...
	if (mode == AI_GOAL_DESTROY_SUBSYSTEM) {
		...
		//m_object_box[item]->AddString("None");
		for (i=0; i<Num_jump_nodes; i++)
			z = m_object_box[item]->AddString(Jumppoint_lists[i].name);

		m_object_box[item]->SetItemData(z, i | TYPE_JUMPPOINT);
			if (init && (m_data[item] == (i | TYPE_JUMPPOINT)))
				m_object[item] = z;
...
void ShipGoalsDlg::update_item(int item, int multi)
{
	...
	switch (mode) {
		...
		case AI_GOAL_WARP:
			//MODIFY(goalp[item].ai_mode, mode);
			//jump_point = m_object_box[item]->GetItemData(m_object[item]);
			//goalp[item].jp_index = jump_point;
			//break;
			return;
	...
	*save = 0;
	...
	switch (m_data[item] & TYPE_MASK) {
		...
		case TYPE_JUMPPOINT:
			goalp[item].ship_name = ai_get_goal_ship_name(Jumppoint_lists[m_data[item] & DATA_MASK].name, &not_used);
			break;
