 ...
 * $Revision: 1.7 $
 * $Date: 2004/06/15 20:50:29 $
 * $Author: wmcoolmon $
 *
 * Single ship editing dialog
 *
 * $Log: ShipEditorDlg.cpp,v $
...
void CShipEditorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBIndex(pDX, IDC_SHIP_IFF, m_iff);
...
BEGIN_MESSAGE_MAP(CShipEditorDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_SHIP_TEAM, OnSelchangeTeam)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CShipEditorDlg::Create()
{
	...
	ptr = (CComboBox *) GetDlgItem(IDC_SHIP_IFF);
	ptr->ResetContent();
	for (i=0; i<Num_iff; i++){
		ptr->AddString(Iff_names[i]);
	}
...
void CShipEditorDlg::initialize_data(int full_update)
{
	...
	if (ship_count) {
						...
						if (Ships[i].wingnum < 0) {
							if (!cue_init) {
								...
								if (m_departure_location == DEPART_AT_DOCK_BAY)
									...
									m_departure_target = Ships[i].jump_anchor;
						} else {
								cue_init++;
								...
								if (m_departure_location == DEPART_AT_DOCK_BAY) {
									...
									if ( Ships[i].jump_anchor != m_departure_target ){
		...
		if (cue_init) {
			// hard coded to be true all the time.  if disbanding a flight group this gets turned false
			m_arrival_tree.load_tree(1);
	...
	if ( m_departure_location == DEPART_AT_DOCK_BAY ){
		...
		if (m_departure_target >= 0) {
			m_departure_target = box->FindStringExact(-1, list[m_departure_target]);
		} else
			m_departure_target = 0;
	...
	GetDlgItem(IDC_SHIP_IFF)->EnableWindow(enable);
	...
	// enable the "set player" button only if single player, single edit, and Alpha, Beta, or Gamma wings
	if (mission_type && total_count && !multi_edit && wing_is_player_wing(cur_wing))
	...
	if (multi_edit) {  // editing multiple ships (ships and/or players)
		...
		for (i=0; i<Num_jump_nodes; i++)
			if (!stricmp(Jump_nodes[i].name, m_ship_name)) {
				...
				bypass_errors = 1;
				z = MessageBox("This ship name is already being used by a hyper buoy\n"
					"Press OK to restore old name", "Error", MB_ICONEXCLAMATION | MB_OKCANCEL);
...
int CShipEditorDlg::update_ship(int ship)
{
	...
	if (m_team != -1)
		MODIFY(Ships[ship].iff, m_iff);

	// team switched to friendly turn on all the player orders for whatever ship class it is
	/*if (Ships[ship].team == TEAM_FRIENDLY) {
		MODIFY(Ships[ship].orders_accepted, ship_get_default_orders_accepted(&Ship_info[Ships[ship].ship_info_index]));
		test.reset();
	}

	// team hostile turn all the orders off
	if (Ships[ship].team == TEAM_HOSTILE) {
		MODIFY(Ships[ship].orders_accepted, 0);
	}*/

	if (Objects[Ships[ship].objnum].type != OBJ_SHIP){
		if ((Objects[Ships[ship].objnum].type != OBJ_START)){ //mission_type || 
			return 0;
		}
	}
	...
	if (Ships[ship].wingnum < 0) {
		...
		if (m_arrival_target >= 0) {
			...
			if (m_departure_location == DEPART_AT_DOCK_BAY) 
				...
				MODIFY(Ships[ship].jump_anchor, m_departure_target );
	}
...
//	}
	...
	if (!multi_edit && !query_initial_orders_empty(Ai_info[Ships[single_ship].ai_index].goals))
		if ((Ships[single_ship].wingnum >= 0) && (query_initial_orders_conflict(Ships[single_ship].wingnum)))
			MessageBox("This ship's flight group also has initial orders", "Possible conflict");
}
...
void CShipEditorDlg::OnShipReset() 
{
	...
	if (m_ship_class) {
		//m_team = 1 << Ship_info[m_ship_class].species;
...
void CShipEditorDlg::OnTextures() 
{
	...
	// bah - all this rubbish is necessary because single_ship doesn't flag the player ship
	object *objp = GET_FIRST(&obj_used_list);
	while (objp != END_OF_LIST(&obj_used_list))
	{
		if ((objp->type == OBJ_SHIP) || (objp->type == OBJ_START))
		{
			if (objp->flags & OF_MARKED)	// get the first marked ship - it should be the only selected one
			{
				dlg_textures.self_ship = objp->instance;
...
void CShipEditorDlg::OnSelchangeTeam()
{
	int index = 1;

	index = ((CComboBox *)GetDlgItem(IDC_SHIP_TEAM))->GetCurSel();

	switch (1 << index)
	{
	// team hostile turn all the orders off
	case TEAM_HOSTILE:
		MODIFY(Ships[cur_ship].orders_accepted, 0);

		Ships[cur_ship].flags &= ~(SF_CARGO_REVEALED);

		break;
	
	// team switched to friendly turn on all the player orders for whatever ship class it is
	case TEAM_FRIENDLY:
		MODIFY(Ships[cur_ship].orders_accepted, ship_get_default_orders_accepted(&Ship_info[Ships[cur_ship].ship_info_index]));
