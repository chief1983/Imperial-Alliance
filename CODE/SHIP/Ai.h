 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:14:52  Darkhill
 * no message
 *
...
typedef struct ai_goal {
	...
	int jp_index;			// index inot jumppoints list that ship might exit with
...
#define AIM_DISABLE				21     // disable an object
...
#define MAX_JUMPPOINTS_PER_LIST 1
#define MAX_JUMPPOINT_LISTS	    MAX_JUMP_NODES
...
typedef struct jumppoint_list {
	char		name[NAME_LENGTH];
	int		count;
	vector	waypoints[MAX_JUMPPOINTS_PER_LIST];
} jumppoint_list;
...
#define SM_DISABLE			18     // trys to disable target ship
...
extern jumppoint_list Jumppoint_lists[MAX_JUMPPOINT_LISTS];
extern int Num_jumppoint_lists;
...
extern void ai_disable_object(object *attacker, object *attacked, int priority);
...
extern void ai_warp_out(object *objp);
extern void ai_fly_to_jump(object *objp, vector *vp);
