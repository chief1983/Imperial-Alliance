 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:43 $
 * $Author: Darkhill $
 *
 * Ship (and other object) handling functions
 *
 * $Log: Ship.cpp,v $
 * Revision 1.2  2004/09/07 22:56:43  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:13:30  Darkhill
 * no message
 *
...
int enter_hangar;							// used to tell if player is close enough to enter hangar

int cloak_map[MAX_NUM_CLOAKMAP];
...
void ship_iff_init_colors()
{
	...
	for ( i=0; i<2; i++ ) {
		...
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_REBEL][i],	0x00, 0xff, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_IMPERIAL][i],	0xff, 0x00, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_BLUE][i],	0x00, 0x00, 0xff, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_PURPLE][i],	0xff, 0x00, 0xff, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_YELLOW][i],	0xff, 0xff, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_RED][i],	0xff, 0x00, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_ORANGE][i],	0xff, 0x80, 0x00, alpha );
        ...
        gr_init_alphacolor( &IFF_colors[IFF_COLOR_WARHEAD][i],  0x7f, 0x7f, 0x00, alpha );
		/*gr_init_alphacolor( &IFF_colors[IFF_COLOR_HOSTILE][i],	0x00, 0xff, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_FRIENDLY][i],	0xff, 0x00, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_NEUTRAL][i],	0x00, 0x00, 0xff, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_UNKNOWN][i],	0xff, 0xff, 0x00, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_SELECTION][i], 0xff, 0xff, 0xff, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_MESSAGE][i],	0x7f, 0x7f, 0x7f, alpha );
		gr_init_alphacolor( &IFF_colors[IFF_COLOR_TAGGED][i],		0xff, 0xff, 0x00, alpha );*/
	}
...
//		subsystems[idx].ai_rotation = NULL;
	...
	// load in the nifty 2D cockpit file name
	if (optional_string("$Cockpit:")) {
		stuff_string(buf, F_MESSAGE, NULL);
		strcpy(sip->cockpit, strdup(buf));
	}
...
strcpy(parse_error_text, temp_error);
	...
	// does the craft have a hyperdrive?
	required_string("$Hyperdrive:");
	stuff_boolean(&sip->hyperdrive);
	
	// does the craft have s-foils?
	sip->sfoils = 0;

	if (optional_string("$S-Foils:")) {
		stuff_boolean(&sip->sfoils);
	}
	
	// read in flyby sound for craft
	if (optional_string("$Flyby:")) {
		stuff_string(buf, F_MESSAGE, NULL);
		// add the '.wav' extension
		strcat(buf, ".wav");
		strcpy(sip->flyby, strdup(buf));
	}
	//stuff_string(&sip->flyby, F_NAME, NULL);
	...
	// system strength is just a value based off the hull strength of the ship type
	sip->initial_system_strength = sip->initial_hull_strength * 0.9f;
	...
	for ( i=0; i<num_strings; i++ )
	{
		...
		else if (!stricmp( NOX("platform"), ship_strings[i])) 
		...
		else if ( !stricmp( NOX("space object"), ship_strings[i]))
			sip->flags |= SIF_SPACE_OBJECT;
...
//	strcpy(sip->thruster_particle_bitmap01_name,"thrusterparticle");
	...
	// default thruster glows in case none are specified in table
	i = sip->species*2;
...
//	CLOAKMAP = bm_load_animation("cloakmap",&CLOAKFRAMES, &CLOAKFPS,1);
	...
	for (int i=0; i<MAX_NUM_CLOAKMAP; i++) {
		char buf[30];
		sprintf(buf, "cloakmap0%d", i);
		cloak_map[i] = -1;
	
		cloak_map[i] = bm_load(buf);
	}
...
void ship_level_init()
{
	...
	// this is for ImpED.  pod weapon needs to be reset when new mission is created
	for (i=0; i<MAX_SHIPS; i++ )
	{
		Ships[i].pod_weapon = 0;
	}
...
void physics_ship_init(object *objp)
{
	...
	pi->afterburner_forward_accel_time_const=Ships[objp->instance].afterburner_forward_accel;
...
void ship_set(int ship_index, int objnum, int ship_type)
{
	...
	if ( sip->num_primary_banks > 0 ) {
		if ( swp->primary_bank_weapons[BANK_1] >= 0 ) {
			...
			for (int i=0; i<MAX_SHIP_PRIMARY_BANKS; i++)
				shipp->primary_link_status[i] = WEAPON_LINK_SINGLE;
	...
	if ( sip->num_secondary_banks > 0 ) {
		if ( swp->secondary_bank_weapons[BANK_1] >= 0 ) {
			...
			for (int i=0; i<MAX_SHIP_SECONDARY_BANKS; i++)
				shipp->secondary_link_status[i] = WEAPON_LINK_SINGLE;
		} else {
			swp->current_secondary_bank = -1;
		}
	}
	...
	if (Fred_running) {
		...
		shipp->ship_initial_system_strength = 100.0f;
	} else {
		shipp->ship_initial_shield_strength = sip->initial_shield_strength;
		shipp->ship_initial_hull_strength = sip->initial_hull_strength;
		shipp->ship_initial_system_strength = sip->initial_system_strength;
		set_shield_strength(objp, shipp->ship_initial_shield_strength);
		objp->system_strength = shipp->ship_initial_system_strength;
	}
...
//	}
	...
	shipp->cloakmap_index = 0;
	shipp->next_cloakmap_timestamp = timestamp(0);
	shipp->cloak_transparent_timestamp = timestamp(0);
	shipp->texture_translation_key.xyz.x = 10;
	shipp->texture_translation_key.xyz.y = 10;
	shipp->texture_translation_key.xyz.z = 10;
	//=vmd_zero_vector;
	shipp->current_translation.xyz.x = 5;
	shipp->current_translation.xyz.y = 5;
	shipp->current_translation.xyz.z = 5;//=vmd_zero_vector;
	shipp->time_until_full_cloak=timestamp(0);
	shipp->cloak_alpha=255;

	shipp->pod_engaged = 0;

	shipp->hyper_timestamp = timestamp(-1);
...
void ship_recalc_subsys_strength( ship *shipp )
{
	...
	shipp->flags &= ~SF_ENGINES_DISABLED;
...
{	
	...
	for ( i = 0; i < sinfo->n_subsystems; i++ )
	{
		...
		// this particular nasty little hack needs some explaining.  because of limitations of the
		// game it is easier to let model creators make s-foil enabled ships with the s-foils in 
		// the "open" position.  but when craft launch we want them in the "closed" position.  what
		// i'm doing here is if the system happens to be a s-foil then i'm moving it to its "closed"
		// position by reversing the range of movement on on whatever side/direction its going.
		if ((sp->type == SUBSYSTEM_SFOIL) && (shipp->sfoil_status & SFOIL_CLOSED)) {
			if ((sp->sfoil_direction == 1) && (sp->sfoil_side == 1)) 
				ship_system->submodel_info_1.angs.b = ANG_TO_RAD(sp->sfoil_rom);

			if ((sp->sfoil_direction == 0) && (sp->sfoil_side == 1)) 
				ship_system->submodel_info_1.angs.b = ANG_TO_RAD(-sp->sfoil_rom);

			if ((sp->sfoil_direction == 1) && (sp->sfoil_side == 0)) 
				ship_system->submodel_info_1.angs.b = ANG_TO_RAD(-sp->sfoil_rom);

			if ((sp->sfoil_direction == 0) && (sp->sfoil_side == 0)) 
				ship_system->submodel_info_1.angs.b = ANG_TO_RAD(sp->sfoil_rom);
		}
	}
...
extern int on_hangar_approach;
extern float cloak_alpha;
...
void ship_render(object * obj)
{
	...
	if ( vm_vec_dist_quick( &obj->pos, &Eye_position ) < obj->radius*50.0f )	{
		...
		// don't display model if ship is cloaked
	//if (Ships[num].cloak_stage == CLOAK_ON)
	//	return;
...
//	if(splode_level<=0.0f)shipp->end_death_time = 0;
	...
	if ( shipp->large_ship_blowup_index > -1 )	{
		...
		if ( (shipp->thruster_bitmap > -1) && (!(shipp->flags & SF_ENGINES_DISABLED) || !(shipp->flags2 & SF2_DISABLED) ) && (!ship_subsys_disrupted(shipp, SUBSYSTEM_ENGINE)) ) {
...
/*			if (ft.xyz.x || ft.xyz.y || ft.xyz.z) {
		...
		if (timestamp_elapsed(shipp->hyper_timestamp))
			render_flags |= MR_NO_RENDER;

		if (shipp->cloak_stage>CLOAK_OFF) {
			//cloaking
			if (shipp->cloak_stage==CLOAK_WARMUP || shipp->cloak_stage==CLOAK_COOLDOWN) {
				model_set_forced_texture(cloak_map[shipp->cloakmap_index]);
				model_set_alpha(cloak_alpha);
								
				if (shipp->cloak_stage == CLOAK_WARMUP ) {
					if (timestamp_elapsed(shipp->cloak_transparent_timestamp)) 				
						render_flags &= ~MR_SHOW_THRUSTERS;
					
					render_flags |= MR_FORCE_TEXTURE | MR_ALL_XPARENT;
				} 
				
				if (shipp->cloak_stage == CLOAK_COOLDOWN) {
					if (!timestamp_elapsed(shipp->cloak_transparent_timestamp)) 
						render_flags |= MR_FORCE_TEXTURE | MR_ALL_XPARENT;
				}
			}

			if (shipp->cloak_stage == CLOAK_ON) {
				render_flags &= ~MR_SHOW_THRUSTERS;
				render_flags |= MR_NO_RENDER;
			}
		}
...
void ship_init_thrusters()
{
	...
	strncpy(Species_names[0],					"Empire",			SPECIES_NAME_MAXLEN);
//	strncpy(Debris_texture_files[0],			"debris",		MAX_DEBRIS_TNAME_LEN);
//	strcpy(Shield_ani[0].filename, "shieldhit01a");
	...
	strncpy(Species_names[1],					"Rebel Alliance",			SPECIES_NAME_MAXLEN);
	//strncpy(Debris_texture_files[1],			"debris01b",		MAX_DEBRIS_TNAME_LEN);
	//strcpy(Shield_ani[1].filename, "shieldhit01a");

	// species*2 ? afterburning?1:0
	strncpy(Thrust_anim_names[2],				"thruster01",		MAX_FILENAME_LEN);
	strncpy(Thrust_anim_names[3],				"thruster01a",		MAX_FILENAME_LEN);
	strncpy(Thrust_secondary_anim_names[2],		"thruster02-01",	MAX_FILENAME_LEN);
	strncpy(Thrust_secondary_anim_names[3],		"thruster02-01a",	MAX_FILENAME_LEN);
	strncpy(Thrust_tertiary_anim_names[2],		"thruster03-01",	MAX_FILENAME_LEN);
	strncpy(Thrust_tertiary_anim_names[3],		"thruster03-01",	MAX_FILENAME_LEN);
	strncpy(Thrust_glow_anim_names[2],			"thrusterglow01",	MAX_FILENAME_LEN);
	strncpy(Thrust_glow_anim_names[3],			"thrusterglow01a",	MAX_FILENAME_LEN);



	// -------------- Shivan -------------- 

	strncpy(Species_names[2],					"Neutral",			SPECIES_NAME_MAXLEN);
...
#endif	
		//i = 0;
			/*strncpy(Thrust_anim_names[i],				"thruster01",		MAX_FILENAME_LEN);
	strncpy(Thrust_anim_names[i+1],				"thruster01a",		MAX_FILENAME_LEN);
	strncpy(Thrust_secondary_anim_names[i],		"thruster02-01",	MAX_FILENAME_LEN);
	strncpy(Thrust_secondary_anim_names[i+1],		"thruster02-01a",	MAX_FILENAME_LEN);
	strncpy(Thrust_tertiary_anim_names[i],		"thruster03-01",	MAX_FILENAME_LEN);
	strncpy(Thrust_tertiary_anim_names[i+1],		"thruster03-01",	MAX_FILENAME_LEN);
*/
		ta = &Thrust_anims[i];
...
#endif
		//strncpy(Thrust_glow_anim_names[i],			"thrusterglow01",	MAX_FILENAME_LEN);
		//strncpy(Thrust_glow_anim_names[i+1],			"thrusterglow01a",	MAX_FILENAME_LEN);
...
void ship_subsys_disrupted_check(ship *sp)
{
	...
	if ( engines_disabled ) {
		if ( !(sp->subsys_disrupted_flags & (1<<SUBSYSTEM_ENGINE)) ) {
			if ( !(sp->flags & SF_ENGINES_DISABLED) || !(sp->flags2 & SF2_DISABLED) ) {
...
void ship_process_post(object * obj, float frametime)
{
	...
	// This is so horrible, I can't even believe it.  In MP the physical info seems to be getting zeroed out
	// somewhere so when the turbo stuff is set when the mission is parsed it gets erased and craft with
	// turbos can't do anything.  So instead of figure out what the real problem is I'm just going to set the 
	// info everytime we go through here.  This makes sure that AI craft will have turbos working too.....
	
	if ((Game_mode & GM_MULTIPLAYER) && (shipp->pod_weapon == TWT_TURBOCHARGER)) {
		obj->phys_info.turbo_forward_accel_time_const = shipp->turbo_accleration;
		obj->phys_info.turbo_max_vel.xyz.z = shipp->turbo_top_speed;
	}
	...
	pod_update(obj, frametime);

	maybe_disable_pod(obj);

	// no use of calling this function for every object.  the player is the only one it has any
	// use for
	if (obj == Player_obj)
		maybe_player_enter_hanger();

	
	is_in_front_in_box(obj);
	...
	if ( shipp->flags & SF_ARRIVING && Ai_info[shipp->ai_index].mode != AIM_BAY_EMERGE )	{
		...
		if ( obj->flags & OF_PLAYER_SHIP ) {
			...
			for ( pss = GET_FIRST(&shipp->subsys_list); pss !=END_OF_LIST(&shipp->subsys_list); pss = GET_NEXT(pss) )
			{
				...
				ship_do_submodel_rotation(shipp, psub, pss, obj);
			}
		}
...
/*#ifndef NO_NETWORK
		...
		// submodel rotating code used to be here.  but here clients won't be able to do anything
		// because the return statement above
		if ( obj->flags & OF_PLAYER_SHIP ) 
...
int ship_create(matrix *orient, vector *pos, int ship_type, char *ship_name)
{
	...
	Assert( sip->flags & (SIF_SMALL_SHIP | SIF_MEDIUM_SHIP | SIF_BIG_SHIP | SIF_CAPITAL | SIF_NO_SHIP_TYPE | SIF_NOT_FLYABLE | SIF_ESCAPEPOD | SIF_SUPERCAP | SIF_DRYDOCK /*| SIF_KNOSSOS_DEVICE*/) );
	...
	// just set the status closed for every ship if it has s-foils or not.  there is a flag in the
	// ship_info struct that tells if a craft actually has s-foils and is checked before this
	// updated for now that s-foils are only close when in single player, mp everything is left open
...
void change_ship_type(int n, int ship_type, int by_sexp)
{
	...
	if (Fred_running) {
		...
		// this is in case the ship being switched to has no shields.
		if (sp->ship_initial_shield_strength <= 0)
			Objects[sp->objnum].flags |= OF_NO_SHIELDS;
		//else
		//	Objects[sp->objnum].flags &= ~(OF_NO_SHIELDS);
	...
	// set the correct system strength
	if (Fred_running) {
		sp->ship_initial_system_strength = 100.0f;
		objp->system_strength = 100.0f;
	} else {
		sp->ship_initial_system_strength = sip->initial_system_strength;
	
		// don't set system strength if called by sexp
		if (!by_sexp)
		{
			objp->system_strength = sp->ship_initial_system_strength;
		}
	}
	...
	Assert(sp->ship_initial_system_strength > 0.0f);
	Assert(objp->system_strength > 0.0f);
	...
	sp->afterburner_fuel = sp->afterburner_fuel_capacity;

	sp->pod_energy = sp->pod_energy_capacity;
...
int ship_fire_primary(object * obj, int stream_weapons, int force)
{
	...
	// if ship has s-foils make sure they are open
	if (sip->sfoils) {
...
//								HUD_sourced_printf(HUD_SOURCE_HIDDEN, "No %s ammunition left in bank", Weapon_info[swp->primary_bank_weapons[bank_to_fire]].name);
				...
				if (shipp->last_fired_point[bank_to_fire] == 0)
				...
				numtimes = winfo_p->shots;
				points = shipp->primary_link_status[bank_to_fire];//num_slots;
				
				// so rate of fire is rather constant for all link types we do a little math on the next primary
				// timestamp
				switch (points)
				{
				case WEAPON_LINK_SINGLE:
					swp->next_primary_fire_stamp[bank_to_fire] = timestamp((int)(next_fire_delay / po->gun_banks[bank_to_fire].num_slots));
					break;
				case WEAPON_LINK_DUAL:
					swp->next_primary_fire_stamp[bank_to_fire] = timestamp((int)(next_fire_delay / (po->gun_banks[bank_to_fire].num_slots/2)));
					break;
				case WEAPON_LINK_ALL:
					polymodel *po = model_get(shipp->modelnum);

					// we override points here in case the ship has 3 lasers or over 4. gives the ship the proper
					// number of lasers to fire
					points = po->gun_banks[shipp->weapons.current_primary_bank].num_slots;
...
//mprintf(("fireing from %d\n",shipp->last_fired_point[bank_to_fire]+j%num_slots));
						...
						// i should probably just remove the whole cycle thing for weapons but for now i'm just
						// going to keep it here.  its the same thing as above so that it fires the points that
						// are needed for whatever link type the player has selected
...
//mprintf(("fireing from %d\n",j));
						...
						// stuff for having player fire from turrets also comment out player_maybe_fire_turret
						/*ship_subsys	*pss;
					...
					pnt = psub->turret_firing_point[0];*/
					pnt = po->gun_banks[bank_to_fire].pnt[pt];
					
					vm_vec_unrotate(&gun_point, &pnt, &obj->orient);
					vm_vec_add(&firing_pos, &gun_point, &obj->pos);

					// thanks to Spearhawk for telling me what to do to make this work
					vector temp, temp1, pt1, laser_vector;
					matrix temp_matrix;
					float convergence_distance;
					object *obj2;

					obj2 = &Objects[aip->target_objnum];

					// rotate the cordinate system to have origin at the center of the obj and
					// looking down the positive z-axis
					vm_vec_sub(&temp, &obj2->pos, &obj->pos);
					vm_vec_rotate(&temp1, &temp, &obj->orient);

					// check if target is in front and then set the convergence distance of the
					// laser to the distance of the craft.
					if (temp1.xyz.z > 0)
						convergence_distance = vm_vec_dist(&obj->pos, &obj2->pos);
					// not in front so range is max distance laser travels
					else
						convergence_distance = winfo_p->lifetime*winfo_p->max_speed;

					// assign and normalize the forward vector of the firing craft
					temp.xyz.x = obj->orient.vec.fvec.xyz.x;
					temp.xyz.y = obj->orient.vec.fvec.xyz.y;
					temp.xyz.z = obj->orient.vec.fvec.xyz.z;
					vm_vec_normalize(&temp);

					// modify for the convergence point and center of ship
					pt1.xyz.x = temp.xyz.x * convergence_distance + obj->pos.xyz.x;
					pt1.xyz.y = temp.xyz.y * convergence_distance + obj->pos.xyz.y;
					pt1.xyz.z = temp.xyz.z * convergence_distance + obj->pos.xyz.z;

					laser_vector.xyz.x = pt1.xyz.x - firing_pos.xyz.x;
					laser_vector.xyz.y = pt1.xyz.y - firing_pos.xyz.y;
					laser_vector.xyz.z = pt1.xyz.z - firing_pos.xyz.z;
					vm_vec_normalize(&laser_vector);
					
					// construct matrix to pass to weapon_create with our new laser firing vector
					// and the orignal up/right vector of the craft
					temp_matrix.vec.fvec = laser_vector;
					temp_matrix.vec.uvec = obj->orient.vec.uvec;
					temp_matrix.vec.rvec = obj->orient.vec.rvec;
					
					// create the weapon -- the network signature for multiplayer is created inside
					// of weapon_create
					weapon_objnum = weapon_create( &firing_pos, &temp_matrix/*&obj->orient*/, weapon, OBJ_INDEX(obj),0, new_group_id );
				...
				shipp->last_fired_point[bank_to_fire] = (shipp->last_fired_point[bank_to_fire] + points) % num_slots; // +1
				}
			}
...
done_secondary:
	...
	if(num_fired > 0){
		...
		if ((shipp->cloak_stage > CLOAK_OFF) && (shipp->cloak_stage < CLOAK_COOLDOWN))
		{
			shipfx_start_cloak(shipp,500);
		}
...
int ship_do_rearm_frame( object *objp, float frametime )
{
	...
	if ( shipp->flags & SF_WARP_BROKEN ) {
		...
		HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR( "Hyperdrive repaired.", -1));
...
//	Comments removed by PhReAk; Note that this is toggled on/off with a mission flag
	...
	while ( ssp != END_OF_LIST( &shipp->subsys_list ) ) {
		...
		if ( ssp->current_hits < max_subsys_repair && repair_allocated > 0 ) {
			...
			// reset the flags
			if ( (ssp->system_info->type == SUBSYSTEM_ENGINE) && (shipp->flags & SF_ENGINES_DISABLED) ) {
				shipp->flags &= ~SF_ENGINES_DISABLED;
...
void ship_close()
{
	...
	for (i=0; i<MAX_NUM_CLOAKMAP; i++) {
		if (cloak_map[i] != -1)
			bm_release(cloak_map[i]);
	}
}	
		...
		if ( !stricmp( Dc_arg, "weapons" ))	{
			...
			if ( (Dc_arg_float < 0.0f) || (Dc_arg_float > 1.0f) )	{
				...
				if ( Dc_arg_float < ENGINE_MIN_STR )	{
					Player_ship->flags |= SF_ENGINES_DISABLED;				// add the disabled flag
				} else {
					Player_ship->flags &= (~SF_ENGINES_DISABLED);				// add the disabled flag
				}
			} 
...
/*	else if ( ship_info_flag & SIF_GAS_MINER )
	...
	} else if ( ship_info_flag & SIF_SPACE_OBJECT ) {
		Ship_counts[SHIP_TYPE_SPACE_OBJECT].total += num;
...
/*	else if ( ship_info_flag & SIF_AWACS )
		...
		Ship_counts[SHIP_TYPE_SPACE_OBJECT].killed++;
	else if ( ship_info_flag & SIF_ESCAPEPOD )
...
int ship_query_general_type(ship *shipp)
{
	...
	switch (flags & SIF_ALL_SHIP_TYPES) {
		...
		case SIF_SPACE_OBJECT:
			return SHIP_TYPE_SPACE_OBJECT;
	...
	// used to be Error() but was tripping on the SIF_DRYDOCK flag even though it was right
	Warning(LOCATION, "Ship type flag is unknown.  Flags value is 0x%x", flags);
	return SHIP_TYPE_NONE;
}
...
int ship_docking_valid(int docker, int dockee)
{
	...
	if (docker_type == SHIP_TYPE_FREIGHTER)
	{
		if ( (dockee_type == SHIP_TYPE_CARGO) || (dockee_type == SHIP_TYPE_CRUISER)
			...
			|| (dockee_type == SHIP_TYPE_SPACE_OBJECT))
		{
			return 1;
		}
	}
	...
	if (docker_type == SHIP_TYPE_TRANSPORT)
	{
		if ( (dockee_type == SHIP_TYPE_CARGO) || (dockee_type == SHIP_TYPE_CRUISER)
			...
			|| (dockee_type == SHIP_TYPE_SENTRYGUN) || (dockee_type == SHIP_TYPE_SPACE_OBJECT))
		{
				return 1;
		}
	}
	...
	// supply ships
	// let supply ships dock with anything
...
// see if the player ship has a hyperdrive.
//returns 1 if drive present, otherwise returns 0
int ship_has_hyperdrive(object *sp)
{
	ship_info	*sip;
	ship		*shipp;
	
	shipp = &Ships[sp->instance];
	sip = &Ship_info[shipp->ship_info_index];
...
//				1 =>	small ship is firing
//				2 =>    large ship is firing
//
...
int ship_dumbfire_threat(ship *sp)
{
	...
	if (ai_endangered_by_weapon(&Ai_info[sp->ai_index]) > 0) {
		weapon_objp = &Objects[Ai_info[sp->ai_index].danger_weapon_objnum];

		if (Ship_info[Ships[weapon_objp->parent].ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) 
			return 2;
		else
			return 1;
	}
	...
	switch (aigp->ai_mode ) {
		...
		case AI_GOAL_CHASE_WING:
			if ( aigp->ship_name ) {
				...
				strcat(outbuf, " flight group");
			} else {
				strcpy(outbuf, "no orders");
			}
			break;
		...
		case AI_GOAL_DESTROY_SUBSYSTEM: {
			...
			if ( aip->targeted_subsys != NULL ) {
				sprintf(outbuf, "attack %s %s", aigp->ship_name, hud_targetbox_truncate_subsys_name(aip->targeted_subsys->system_info->name));
				strcat(outbuf, name);
			} else {
				strcpy(outbuf, "no orders" );
			}
		...
		case AI_GOAL_WARP:
			if ( aigp->ai_mode == AIM_BAY_DEPART) {
				char *name;
				char buf[50];

				Assert( sp->departure_anchor >= 0 );
				name = Parse_names[sp->departure_anchor];
	
				sprintf(buf, " to %s", name);
				strcat(outbuf, buf);				
			}
			break;
...
char *ship_return_time_to_goal(char *outbuf, ship *sp)
{
	...
	} else if ( aip->mode == AIM_WARP_OUT) {
		min_speed = 0.9f * sp->current_max_speed;
		if (sp->jump_anchor >= 0) {
			jumppoint_list *jpl;
			jpl = &Jumppoint_lists[sp->jump_anchor];

			dist += vm_vec_dist_quick(&objp->pos, &jpl->waypoints[0]);
			...
			return NULL;
	} else if (aip->mode == AIM_BAY_DEPART) {
		if (sp->departure_anchor >= 0) {
			object* targetp;
			ship_subsys	*subsys;
			vector subobj_pos;
			int anchor_shipnum;
				
			anchor_shipnum = ship_name_lookup(Parse_names[sp->departure_anchor]);
		
			targetp = &Objects[anchor_shipnum];

			if ( targetp->type != OBJ_SHIP ) 
				time = -1;

			for (subsys = GET_FIRST(&Ships[anchor_shipnum].subsys_list); subsys != END_OF_LIST(&Ships[anchor_shipnum].subsys_list); subsys = GET_NEXT( subsys ) ) {
				if (ship_subsys_is_fighterbay(subsys)) {
					get_subsystem_world_pos(targetp, subsys, &subobj_pos);

					dist += vm_vec_dist_quick(&objp->pos, &subobj_pos);
				}
			}
	...
	while(cargo_so != END_OF_LIST(&Ship_obj_list)){
		...
		if ( (Ship_info[cargo_sp->ship_info_index].flags & SIF_CARGO) && !(cargo_sp->team & TEAM_FRIENDLY) ) {
			...
			for ( ship_so=GET_FIRST(&Ship_obj_list); ship_so != END_OF_LIST(&Ship_obj_list); ship_so=GET_NEXT(ship_so) ) {
				...
				// if this ship is a small, medium or big ship
				if ( Ship_info[ship_sp->ship_info_index].flags & (SIF_SMALL_SHIP|SIF_MEDIUM_SHIP|SIF_BIG_SHIP) ) {
...
void awacs_maybe_ask_for_help(ship *sp, int multi_team_filter)
{
	...
	if (stricmp(Campaign.filename, "imperialalliance") || !(Game_mode & GM_CAMPAIGN_MODE))
		return;
...
void ship_maybe_tell_about_rearm(ship *sp)
{
	...
	if ( heavily_damaged || (sp->flags & SF_ENGINES_DISABLED) || (sp->flags2 & SF2_DISABLED) ) {
...
void ship_page_in_model_textures(int modelnum)
{
	...
	for (i=0; i<pm->n_textures; i++ )
	{
		...
		bitmap_num = pm->normal_original_textures[i];
...
int ship_is_team(int ship_num, int check_team)
{
	Assert(ship_num >= 0 && ship_num < MAX_SHIPS);
...
// Goober5000: change ship's team, currently only called from change-team in sexp.cpp
void ship_change_team(int ship_num, int new_team)
{
	Assert(ship_num >= 0 && ship_num < MAX_SHIPS);
	...
	if((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL) && (Net_player->flags & NETINFO_FLAG_AM_MASTER) && (Ships[ship_num].objnum >= 0)){
		send_change_team_packet(Objects[Ships[ship_num].objnum].net_signature, new_team);
	}
#endif
}

int ship_is_iff(int ship_num, int check_iff)
{
	Assert(ship_num >= 0 && ship_num < MAX_SHIPS);

	return (Ships[ship_num].iff == check_iff);
}

// change ship's iff, currently only called from change-iff in sexp.cpp
void ship_change_iff(int ship_num, int new_iff)
{
	Assert(ship_num >= 0 && ship_num < MAX_SHIPS);

	Ships[ship_num].iff = new_iff;

#ifndef NO_NETWORK
	// send a network packet if we need to
	if((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL) && (Net_player->flags & NETINFO_FLAG_AM_MASTER) && (Ships[ship_num].objnum >= 0)){
		send_change_iff_packet(Objects[Ships[ship_num].objnum].net_signature, new_iff);
	}
#endif
}
...
int ship_subsys_is_fighterbay(ship_subsys *ss)
{
	...
	if ( !strnicmp(NOX("hangar"), ss->system_info->name, 6) ) {
		return 1;
	}
...
int ship_fire_tertiary(object *objp)
{
	Assert(objp->type == OBJ_SHIP);
	ship *shipp=&Ships[objp->instance];
	ship_info *sip = &Ship_info[objp->instance];
...
//	Assert(sw);
	...
	// if subsystem has taken to much damage then can't use it until
	// repaired
	if (ship_get_subsystem_strength(shipp, SUBSYSTEM_POD) < 0.2) {
		HUD_printf("P.O.D. has taken to much damage");

		return 0;
	}

	// stop any looping sounds
	if (shipp->pod_engaged == 1 || shipp->cloak_stage==CLOAK_ON)
		pod_stop_sounds();

	switch (shipp->pod_weapon) //
	{
		case TWT_TURBOCHARGER:
			if ( shipp->current_max_speed == 0 ) { 
				percent_throttle = objp->phys_info.fspeed / sip->max_speed;
			} else {
				percent_throttle = objp->phys_info.fspeed / shipp->current_max_speed;
			}

			percent_throttle *= 100;
			if (percent_throttle > 100)
				percent_throttle = 100;
			
			HUD_printf("Turbocharger operating at %d%% efficiency.", fl2i(percent_throttle));
		...
		case TWT_CLOAK:
			if (shipp->cloak_stage==CLOAK_OFF) {
				if (!pod_start(objp)) {
					shipfx_start_cloak(shipp,twip->cloak_warmup);
					sprintf(buf, "Cloak engaged.");
					//hud_target_next(shipp->team);
					Objects[shipp->objnum].flags |= OF_PROTECTED;
					send_ship_pod_update_packet(shipp, 1, SF2_CLOAK_ON);
				} else
					break;
			...
			if (shipp->cloak_stage==CLOAK_ON) {
				...
				sprintf(buf, "Cloak disenganged.");
				Objects[shipp->objnum].flags &= ~(OF_PROTECTED);
				send_ship_pod_update_packet(shipp, 0, SF2_CLOAK_ON);
				pod_stop_sounds();
			}
		...
		case TWT_BOOST_POD:
		{
			if (shipp->pod_engaged)
			{
				return 1;
			}

			if (shipp->boost_shots_left == 0)
			{
				sprintf(buf, "No booster shots remaining");
				return 1;
			}
			if (shipp->pod_engaged == 0) {
			shipp->pod_engaged=1;
			shipp->boost_shots_left--;
		...
		case TWT_DECOY:
			if (shipp->pod_engaged == 0) {
				pod_start(objp);
				shipp->pod_engaged = 1;
				sprintf(buf, "Decoy engaged.");
				shipp->flags2 |= SF2_DECOY_ON;
				Objects[shipp->objnum].flags |= OF_PROTECTED;
				send_ship_pod_update_packet(shipp, 1, SF2_DECOY_ON);
			}
			else { 				
				shipp->pod_engaged = 0;
				sprintf(buf, "Decoy disengaged.");
				shipp->flags2 &= ~(SF2_DECOY_ON);
				Objects[shipp->objnum].flags &= ~(OF_PROTECTED);
				send_ship_pod_update_packet(shipp, 0, SF2_DECOY_ON);
			}
		...
		case TWT_TRACTOR:
			if (shipp->pod_engaged == 0) {
				pod_start(objp);
				shipp->pod_engaged = 1;
				sprintf(buf, "Tractor engaged.");
				shipp->flags2 |= SF2_TRACTOR_ON;
				send_ship_pod_update_packet(shipp, 1, SF2_TRACTOR_ON);
			}
			else { 				
				shipp->pod_engaged = 0;
				sprintf(buf, "Tractor disengaged.");
				shipp->flags2 &= ~(SF2_TRACTOR_ON);
				send_ship_pod_update_packet(shipp, 0, SF2_TRACTOR_ON);
			}
		...
		case TWT_JAMMER:
			if (shipp->pod_engaged == 0) {
				pod_start(objp);
				shipp->pod_engaged = 1;
				sprintf(buf, "Jammer engaged.");
				shipp->flags2 |= SF2_JAMMER_ON;
				send_ship_pod_update_packet(shipp, 1, SF2_JAMMER_ON);
			}
			else { 				
				shipp->pod_engaged = 0;
				sprintf(buf, "Jammer disengaged.");
				shipp->flags2 &= ~(SF2_JAMMER_ON);
				send_ship_pod_update_packet(shipp, 0, SF2_JAMMER_ON);
			}
		...
		case TWT_SLAM:
			...
			if (percent_throttle < 0.66) {
				HUD_printf("Need 66%% power to engage S.L.A.M.");
			...
			if (shipp->pod_engaged == 0) {
				...
				shipp->pod_engaged = 1;
				sprintf(buf, "S.L.A.M. engaged.");
			}
			else { //(shipp->pod_engaged == 1) {
				afterburners_stop(objp, 1);
				shipp->pod_engaged = 0;
				sprintf(buf, "S.L.A.M. disengaged.");
			}
		
			break;

		default:
			HUD_printf("P.O.D. not installed.");

			break;
	}

	if ((objp == Player_obj) && ((shipp->pod_weapon != TWT_TURBOCHARGER) || (shipp->pod_weapon != TWT_SLAM)))
		HUD_printf(buf);
...
// Goober5000
void ship_do_submodel_rotation(ship *shipp, model_subsystem *psub, ship_subsys *pss, object *objp)
{
	Assert(shipp);
	Assert(psub);
	Assert(pss);
...
//pss->submodel_info_1.angs.b;
	...
	if (psub->flags & MSS_FLAG_STEPPED_ROTATE	) {
		...
		submodel_rotate(shipp->objnum, psub, &pss->submodel_info_1, objp );
	}
}
...
// checks to see if the player is near the hangar subsystem of its mothership.
// set the enter_hanger variable so that the key stroke to enter the 
// hangar is valid in the key checking code
void maybe_player_enter_hanger()
{
	...
	anchor_shipnum = ship_name_lookup(Parse_names[Ships[Player_obj->instance].departure_anchor]);
	...
	for (subsys = GET_FIRST(&Ships[anchor_shipnum].subsys_list); subsys != END_OF_LIST(&Ships[anchor_shipnum].subsys_list); subsys = GET_NEXT( subsys ) ) {
		if (ship_subsys_is_fighterbay(subsys)) {
			...
			dist = vm_vec_dist_quick(&Player_obj->pos, &subobj_pos);

			if (dist < 500.0f) {
				HUD_fixed_printf(0.1f, "Press Spacebar to Enter hangar");
...
int ship_has_ions(ship *shipp)
{
	...
	for (int i=0; i<swp->num_primary_banks; i++) {
		...
		if (weapon_info_index > -1) {
			if (Weapon_info[weapon_info_index].wi_flags & WIF_ELECTRONICS) {
				return 1;
