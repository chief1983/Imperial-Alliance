 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:18:24  Darkhill
 * no message
 *
...
// radius.
#define SHIPFX_WARP_DELAY	(1.5f)		// time for warp effect to ramp up before ship moves into it.
...
void shipfx_warpin_frame( object *objp, float frametime )
{
	...
	if ( shipp->flags & SF_ARRIVING_STAGE_1 )	{
		if ( timestamp_elapsed(shipp->final_warp_time) ) {
			...
			// just hard code these because they work
			float warp_time = 0.0f;//shipfx_calculate_warp_time(objp);
			float speed = 2500.0f;//shipfx_calculate_warp_speed(objp);			// How long it takes to move through warp effect
			...
			// times by 0, who cares?
			shipp->final_warp_time = timestamp(fl2i(warp_time*1.0f));
		...
		if ( timestamp_elapsed(shipp->final_warp_time) ) {
			...
			// when the craft is actually in space slow it down as fast
			// as possible.  gives the nifty "appearing" effect of the
			// movies
			objp->phys_info.prev_ramp_vel.xyz.z = 0;
...
void shipfx_warpout_start( object *objp )
{
	...
	//return if disabled
	if ( shipp->flags & SF_ENGINES_DISABLED || shipp->flags2 & SF2_DISABLED ){
		return;
	}
...
//		HUD_printf(XSTR( "Subspace node activated", 498) );
		HUD_printf(NOX("Hyperdrive engaged"));
	}
...
//		warp_time = fireball_lifeleft(&Objects[warp_objnum]);
		shipp->final_warp_time = timestamp(0);//fl2i(warp_time*1000.0f));
	} else {
		shipp->final_warp_time = timestamp(fl2i(0));//warp_time*2.0f*1000.0f
	}
	shipp->flags |= SF_DEPART_WARP;
...
void shipfx_emit_spark( int n, int sn )
{
	...
	if ( create_spark )	{
		...
		} else if (!(si->flags & SIF_FIGHTER)) {

			pe.min_rad = 0.1f;//0.7f;				// Min radius
			pe.max_rad = 0.3f;//1.3f;				// Max radius
			pe.num_low  = int (10 * spark_num_scale);//int (20 * spark_num_scale);		// Lowest number of particles to create (hardware)
			pe.num_high = int (15 * spark_num_scale);//int (50 * spark_num_scale);		// Highest number of particles to create (hardware)
...
void shipfx_do_damaged_arcs_frame( ship *shipp )
{
	...
	if (!timestamp_valid(shipp->arc_next_time))	{
		...
		// if ship is cloaked and a small ship throw a spark every once in awhile
		else if (shipp->cloak_stage == CLOAK_ON && (Ship_info[shipp->ship_info_index].flags & SIF_SMALL_SHIP)) {
			freq = 1000;
		} else if ((shipp->cloak_stage == CLOAK_WARMUP || shipp->cloak_stage == CLOAK_COOLDOWN) && (Ship_info[shipp->ship_info_index].flags & SIF_SMALL_SHIP)) {
			freq = 100; 
		} else {
		...
		for (i=0; i<MAX_SHIP_ARCS; i++ )	{
			if ( !timestamp_valid( shipp->arc_timestamp[i] ) )	{
				...
				if((shipp->emp_intensity > 0.0f) || (disrupted_arc)){
					...
					shipp->arc_type[i] = MARC_TYPE_CLOAK;
				}else {
...
void shipfx_start_cloak(ship *shipp, int warmup)
{
	shipp->time_until_full_cloak=timestamp(warmup);
	shipp->cloak_stage=CLOAK_WARMUP;
	
	shipp->next_cloakmap_timestamp = timestamp(100);
	shipp->cloak_transparent_timestamp = timestamp(warmup/2);
	cloak_alpha = 255.0f;

	shipp->time_until_uncloak=timestamp(Tertiary_weapon_info.cloak_lifetime); //[shipp->tertiary_weapon_info_idx]
...
void shipfx_cloak_frame(ship *shipp, float frametime)
{
	...
	switch (shipp->cloak_stage)
	{
		case 1:
			...
			{
				shipp->cloak_stage=CLOAK_ON;
				shipp->flags2 |= SF2_STEALTH;
				shipp->flags2 |= SF2_CLOAK_ON;
				shipp->arc_next_time = timestamp(-1);

			}
			cloak_alpha -= 25.0f;
			if (cloak_alpha < 0)
				cloak_alpha = 0.0f;
			if (timestamp_elapsed(shipp->next_cloakmap_timestamp)) {
				shipp->cloakmap_index++;
				if (shipp->cloakmap_index > MAX_NUM_CLOAKMAP-1)
		...
		case 3:
			...
			if (timestamp_elapsed(shipp->time_until_full_cloak))
			{
				...
				shipp->flags2 &= ~(SF2_CLOAK_ON);
				shipp->arc_next_time = timestamp(-1);
				//obj->phys_info.flags &= ~PF_CLOAK_ON;
			}
			break;
...
{
	shipp->cloak_stage=CLOAK_COOLDOWN;
	shipp->time_until_full_cloak=timestamp(warmdown);
	shipp->cloak_transparent_timestamp = timestamp(warmdown/4);
