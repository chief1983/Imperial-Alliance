 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:18:47  Darkhill
 * no message
 *
...
void do_subobj_destroyed_stuff( ship *ship_p, ship_subsys *subsys, vector* hitpos )
{
	...
	if ( psub->type == SUBSYSTEM_TURRET ) {
		...
		if ( ship_p->subsys_info[type].current_hits == 0.0f ) {
//			mission_log_add_entry(LOG_SHIP_DISABLED_ENGINES, ship_p->ship_name, NULL );
			ship_p->flags |= SF_ENGINES_DISABLED;				// add the disabled flag
		}
	}
	...
	} else if ( Ship_info[ship_p->ship_info_index].flags & (SIF_BIG_SHIP | SIF_MEDIUM_SHIP)) {
...
float do_subobj_hit_stuff(object *ship_obj, object *other_obj, vector *hitpos, float damage)
{
	...
	{
		// don't want s-foils taking damage
...
void ship_hit_kill(object *ship_obj, object *other_obj, float percent_killed, int self_destruct)
{
	...
	// if the ship happens to be cloaked then turn the cloak off
	if (sp->flags2 & SF2_CLOAK_ON)
		shipfx_stop_cloak(sp, Tertiary_weapon_info.cloak_cooldown);
...
{	
	...
	if (objp->type == OBJ_SHIP) {
		...
		if (Ships[objp->instance].flags2 & SF2_CLOAK_ON)
			shipfx_stop_cloak(&Ships[objp->instance], Tertiary_weapon_info.cloak_cooldown);
	}
...
//	mprintf(("doing damage\n"));
	...
	if (other_obj)
	{
		...
		other_obj_is_mine = ((other_obj->type == OBJ_SHIP) && (other_obj->instance >=0) && (Ship_info[Ships[other_obj->instance].ship_info_index].flags & SIF_SENTRYGUN));
	}
	else
	{
...
//mprintf(("lethality updated\n"));
	...
	if (other_obj_is_mine) {
		// small ships just get nuked...
		if (sip->flags & (SIF_SMALL_SHIP | SIF_HARMLESS)) {
			ship_hit_kill(ship_obj, other_obj, 100.0f, 0);
			return;
		// other ships get a flat damage, maybe they make it maybe they don't
		} else
			damage *= 2000.0f;
	}
...
// mines and little space objects get destroyed if ran into
#ifndef NO_NETWORK
	if ( (sip->flags & (SIF_SENTRYGUN | SIF_SPACE_OBJECT)) && !(shipp->flags & SF_DYING) && !MULTIPLAYER_CLIENT && !(Game_mode & GM_DEMO_PLAYBACK)){  // if not killed, then kill
#else
	if ( (sip->flags & (SIF_SENTRYGUN | SIF_SPACE_OBJECT)) && !(shipp->flags & SF_DYING) && !(Game_mode & GM_DEMO_PLAYBACK)) {  // if not killed, then kill
#endif
	if ((other_obj->parent < 0) || (other_obj->parent >= MAX_OBJECTS)){
		...
		ship_hit_kill(ship_obj, other_obj, 1.0f, 0);
		...
		if(damage > 0.0){
			...
			if ( weapon_info_index >= 0 ) {
				...
				if (Weapon_info[weapon_info_index].wi_flags & WIF_ELECTRONICS) {
					// if its a little ship
					if (sip->flags & (SIF_SMALL_SHIP | SIF_NOT_FLYABLE | SIF_MEDIUM_SHIP)) {
						ship_obj->system_strength -= Weapon_info[weapon_info_index].damage;
						if (ship_obj->system_strength <= 0.0f) {
							ship_obj->system_strength = 0.0f;

							//#ifndef NO_NETWORK
							//	if ( MULTIPLAYER_MASTER ) {
									send_ship_disabled_packet(shipp);
							//	}	
							//#endif
							shipp->flags2 |= SF2_DISABLED;
							// i'm not quite sure where to put this at.  it will work here but have to check
							// if the log message has already been posted so it doesn't get reposted every
							// time the ship is hit.  the only problem with this is if the ship got repaired
							// and then disabled again the log for the second disabling probably won't show
							// up.  i know sometime in the future this is going to come back and bite me in
							// the ass
							if (! (mission_log_get_time(LOG_SHIP_DISABLED, shipp->ship_name, NULL, NULL) ) )
								mission_log_add_entry(LOG_SHIP_DISABLED, shipp->ship_name, NULL );
						}
					// do stuff for bigger ships
					} else if (sip->flags & (SIF_CAPITAL | SIF_CRUISER | SIF_CORVETTE | SIF_DRYDOCK)){
						// if system percent is lower then 10 for ship then a ion type bomb
						// is need to take it down any lower
						if (get_system_pct(ship_obj) <= 0.1f) {
							if (Weapon_info[weapon_info_index].wi_flags & WIF_BOMB)
								ship_obj->system_strength -= (Weapon_info[weapon_info_index].damage/2.0f);
						} else
							ship_obj->system_strength -= (Weapon_info[weapon_info_index].damage/2.0f);

						if (ship_obj->system_strength <= 0.0) {
							ship_obj->system_strength = 0.0;
					...
					// can't disable the really big suckers
					} else {
						if (get_system_pct(ship_obj) >= 0.5f) {
							ship_obj->system_strength -= (Weapon_info[weapon_info_index].damage/4.0f);
						}		
					}
				}
			...
			// don't want electronics stuff making hull damage
			if (Weapon_info[weapon_info_index].wi_flags & WIF_ELECTRONICS) 
				damage = 0.0f;
