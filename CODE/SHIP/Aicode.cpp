 ...
 * $Revision: 1.2 $
 * $Date: 2004/08/29 23:12:56 $
 * $Author: Spearhawk $
 * 
 * AI code that does interesting stuff
 *
 * $Log: AICODE.CPP,v $
 * Revision 1.3  2004/09/07 23:15:31  Spearhawk
 * AI now won't hyper out untill path is clear
 *
 * Revision 1.2  2004/08/29 23:12:56  Spearhawk
 * Made the AI's jump to hyperspeed more SW:ish
 *
 * Revision 1.1.1.1  2004/08/13 22:47:43  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:17:17  Darkhill
 * no message
 *
...
// Speed to be ahcived for hyperspace
const float ai_hyper_speed = 1500.0f;

// Time to achieve hyperspace speed before forcing jump
const float ai_hyper_speed_time = 500.0f;

// Force a jump to hyperspace after x seconds of trying to get into position
const float force_ai_jump_after     = 50.0f;
const float force_big_ai_jump_after = 100.0f;

// Diffrent jump points for diffrent factions
const vector jump_point_enemy	 = { 9999.9f, 9999.9f, 9999.9f };
vector jump_point_friendly = { -9999.9f, 9999.9f, 9999.9f };
const vector jump_point_neutral	 = { 9999.9f, -9999.9f, 9999.9f };
const vector jump_point_unknown	 = { 9999.9f, 9999.9f, -9999.9f };
...
jumppoint_list Jumppoint_lists[MAX_JUMPPOINT_LISTS];
...
*/
	...
	*/
	...
	//if (Ships[objnum].flags2 & SF2_CLOAK_ON)
	//	return aip->previous_target_objnum;
...
int get_enemy_team_mask(int objnum)
{
	...
	// sentry guns attack any team but their own
	if (Ship_info[Ships[Objects[objnum].instance].ship_info_index].flags & SIF_SENTRYGUN) {
		switch (my_team) {
		case TEAM_FRIENDLY:
			enemy_team_mask = TEAM_HOSTILE | TEAM_NEUTRAL | TEAM_TRAITOR | TEAM_UNKNOWN;
			break;
		case TEAM_HOSTILE:
			enemy_team_mask = TEAM_FRIENDLY | TEAM_NEUTRAL | TEAM_TRAITOR | TEAM_UNKNOWN;
			break;
		case TEAM_NEUTRAL:
			enemy_team_mask = TEAM_FRIENDLY | TEAM_HOSTILE | TEAM_TRAITOR | TEAM_UNKNOWN;
			break;
		case TEAM_UNKNOWN:
			enemy_team_mask = TEAM_FRIENDLY | TEAM_HOSTILE | TEAM_TRAITOR | TEAM_HOSTILE;
			break;
		case TEAM_TRAITOR:
			enemy_team_mask = TEAM_FRIENDLY | TEAM_NEUTRAL | TEAM_HOSTILE | TEAM_TRAITOR | TEAM_UNKNOWN;
			break;
		default:
...
void evaluate_object_as_nearest_objnum(eval_nearest_objnum *eno)
{
	...
	// if target is cloaked then we can't see to shoot it right?
	if (shipp->flags2 & SF2_CLOAK_ON) {
		return;
	}

	// if the target has the decoy/jammer on then we don't fire on it.  only
	// exception is jedi, they fire wether the ship has the decoy on
	// or not
	if (shipp->flags2 & SF2_NO_RADAR) {
		if (aip->ai_class < (Num_ai_classes-1))
			return;
	}
...
int get_nearest_objnum(int objnum, int enemy_team_mask, int enemy_wing, float range, int max_attackers, int except)
{
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {
		if (Ships[so->objnum].flags2 & SF2_NO_RADAR)
			continue;
		...
		if (aip->target_objnum != -1) {
			...
			if ( (Objects[target_objnum].signature == aip->target_signature) ) {
				if (Ships[Objects[target_objnum].instance].team & enemy_team_mask) {
					if (!(Objects[target_objnum].flags & OF_PROTECTED) || !(Ships[Objects[target_objnum].instance].flags2 & SF2_NO_RADAR)) {
...
void ai_attack_object(object *attacker, object *attacked, int priority, ship_subsys *ssp, int except)
{
	...
	// if target is cloaked then we can't see to shoot it right?
	if ((attacked != NULL) && (Ships[attacked->instance].flags2 & SF2_CLOAK_ON)) {
		return;
	}

	// if the attacking craft has s-foils we need to open them up so he can bust a cap in that
	// punks ass
	if (sip->sfoils) {
	...
	// exception is jedi, they fire wether the ship has the decoy on
	// or not
	if ((attacked != NULL) && (Ships[attacked->instance].flags2 & SF2_NO_RADAR)) {
		if (aip->ai_class < (Num_ai_classes-1))
			return;
	}
...
void ai_disable_object(object *attacker, object *attacked, int priority)
{
	ai_info	*aip;
	ship_info	*sip;
	ship		*shipp;
...
//		return;
	...
	if (attacked == NULL) {
		aip->choose_enemy_timestamp = timestamp(0);
		// nebula safe
		set_target_objnum(aip, find_enemy(attacker-Objects, 99999.9f, 4, NULL));
	} else {
		// check if we can see atacked in nebula
		if (aip->target_objnum != attacked - Objects) {
			aip->aspect_locked_time = 0.0f;
	...
	//aip->mode = AIM_CHASE;
	aip->mode = AIM_DISABLE;
	//aip->submode = SM_DISABLE;	
...
void set_accel_for_docking(object *objp, ai_info *aip, float dot, float dot_to_next, float dist_to_next, float dist_to_goal, ship_info *sip)
{
	...
	if (objp->phys_info.speed < 0.0f) {
		...
		if ((aip->mode == AIM_DOCK) && (dist_to_next < 150.0f) && (aip->path_start + aip->path_length - 2 == aip->path_cur)) {
			set_accel_for_target_speed(objp, sip->max_speed * max(dist_to_next/500.0f, 0.0f));
...
//	if (IS_VEC_NULL(&Pl_objp->phys_info.vel)) {
	...
	if (shipp->wingnum != -1) {
		...
		max_allowed_speed = shipp->current_max_speed;
...
//	ship_info	*sip = &Ship_info[shipp->ship_info_index];
	...
	if ((Missiontime - aip->submode_start_time > F1_0/2) && (shipp->afterburner_fuel_capacity > 0.0f)) {
		float percent_left = 100.0f * shipp->afterburner_fuel / shipp->afterburner_fuel_capacity;
...
//	if (flags & WIF_PUNCTURE) 
	...
	// no shields and disabling only select a ion weapon
	if (Ai_info[shipp->ai_index].mode == AIM_DISABLE) {
	//if (Ai_info[shipp->ai_index].goals[0].ai_mode == AI_GOAL_DISABLE) {
		if (swp->current_primary_bank >= 0) 
		{
			int	bank_index;
			...
			if (Weapon_info[swp->primary_bank_weapons[bank_index]].wi_flags & WIF_ELECTRONICS) 
			{
				return swp->current_primary_bank;
			}
		}
...
void set_primary_weapon_linkage(object *objp)
{
	...
	if ( (aip->active_goal != AI_GOAL_NONE) && (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC) )
	{
		if ( aip->goals[aip->active_goal].ai_mode & (AI_GOAL_DISARM_SHIP | AI_GOAL_DISABLE_ENGINES))
		{
...
//	if (!IS_VEC_NULL(&G_predicted_pos)) {
	...
	// if we are disabling and shields are gone don't fire unless we have an ion weapon
	/*//if (aip->mode == AIM_DISABLE && enemy_objp != NULL) {
		//float shield = get_shield_pct(enemy_objp);

		//if (shield <= 0) {
			if (!(Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]].wi_flags & WIF_ELECTRONICS)) {
			//nprintf(("AI", "Ship %s not firing at protected ship %s because not using disruptor.\n", Ships[objp->instance].ship_name, Ships[enemy_objp->instance].ship_name));
				swp->next_primary_fire_stamp[swp->current_primary_bank] = timestamp(1000);
				return 0;
			}
	...
	// if target is cloaked then we can't see to shoot it right?
	if ((enemy_objp != NULL) && (Ships[enemy_objp->instance].flags2 & SF2_CLOAK_ON)) {
		return 0;
	}
	...
	// exception is jedi, they fire wether the ship has the decoy on
	// or not
	if ((enemy_objp != NULL) && (Ships[enemy_objp->instance].flags2 & SF2_NO_RADAR)) {
		if (aip->ai_class < (Num_ai_classes-1))
			return 0;
	}
		...
		if (ai_maybe_fire_afterburner(Pl_objp, aip)) {
			if (dist_to_enemy > 800.0f) {
				if (!( Pl_objp->phys_info.flags & PF_AFTERBURNER_ON )) {
					...
					if (shipp->afterburner_fuel_capacity > 0.0f) {
						percent_left = 100.0f * shipp->afterburner_fuel / shipp->afterburner_fuel_capacity;
						if (percent_left > 30.0f + ((Pl_objp-Objects) & 0x0f)) {
							afterburners_start(Pl_objp);
							aip->afterburner_stop_time = Missiontime + F1_0 + static_rand(Pl_objp-Objects)/4;
						}
...
void ai_chase_attack(ai_info *aip, ship_info *sip, vector *predicted_enemy_pos, float dist_to_enemy, int modelnum)
{
	...
	if (En_objp->radius > En_objp->phys_info.speed) {
		...
		scale *= 0.2f * En_objp->radius/(En_objp->phys_info.speed + En_objp->radius);	//  scaled down to 1/5 since 1/2 to much
...
void ai_disable_ship(ai_info *aip, ship_info *sip, vector *predicted_enemy_pos, float dist_to_enemy, int modelnum)
{
	int		start_bank;
	float		dot_to_enemy, dot_from_enemy; //, time_to_hit;
	float		bank_override = 0.0f;
...
void ai_chase_ga(ai_info *aip, ship_info *sip)
{
	...
	if (ai_maybe_fire_afterburner(Pl_objp, aip)) {
		if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON )) {
			float percent_left = 100.0f * Ships[Pl_objp->instance].afterburner_fuel / Ships[Pl_objp->instance].afterburner_fuel_capacity;
			if (percent_left > 30.0f + ((Pl_objp-Objects) & 0x0f)) {
				afterburners_start(Pl_objp);
				aip->afterburner_stop_time = Missiontime + 3*F1_0/2;
			}
...
void ai_choose_secondary_weapon(object *objp, ai_info *aip, object *en_objp)
{
	...
	// if we are disabling and the shields are down then no more secondaries
	/*if (aip->mode == AIM_DISABLE) {
		float shield = get_shield_pct(en_objp);
	...
	if (preferred_secondary != -1) {
		...
		if (is_big_ship) {
			...
			priority1 = 0;								
			priority2 = 0;
		} else if (aip->mode == AIM_DISABLE) {
			priority1 = WIF_ELECTRONICS;
			priority2 = 0;
		} else {
...
void ai_chase()
{
	...
	// removed so other ship classes attack
		...
		if (dot_to_enemy > 0.95f - 0.5f * En_objp->radius/max(1.0f, En_objp->radius + dist_to_enemy)) {
			...
			if (dot_to_enemy > max(0.5f, 0.90f + aip->ai_accuracy/10.0f - En_objp->radius/max(1.0f,dist_to_enemy))) {
				...
				if ( tswp->num_primary_banks > 0 ) {
					...
					// if target is cloaked then we can't see to shoot it right?
					if (Ships[En_objp->instance].flags2 & SF2_CLOAK_ON) {
						return;
					}

					// if the target has the decoy/ajmmer on then we don't fire on it.  only
					// exception is jedi, they fire wether the ship has the decoy on
					// or not
					if (Ships[En_objp->instance].flags2 & SF2_NO_RADAR) {
						if (aip->ai_class < (Num_ai_classes-1))
							return;
					}
...
void ai_disable()
{
	...
	ai_disable_ship(aip, sip, &predicted_enemy_pos, dist_to_enemy, shipp->modelnum);
			...
			if ( tswp->num_primary_banks > 0 ) {
				...
				if (!(En_objp->flags & OF_PROTECTED)) {
					...
					if (current_bank > -1) {
						...
						if ( !(swip->wi_flags & WIF_ELECTRONICS) && ( (get_hull_pct(En_objp) >= 50.0f) || (get_shield_pct(En_objp) != 0)) ) {
						
						//	if (swip->wi_flags & WIF_ELECTRONICS) {
						if (timestamp_elapsed(swp->next_secondary_fire_stamp[current_bank])) {
							if (tswp->current_secondary_bank >= 0) {
								weapon_info	*swip = &Weapon_info[tswp->secondary_bank_weapons[tswp->current_secondary_bank]];
								float firing_range;
	...
	// ship is disabled so remove the order
	if (En_objp != NULL && Ships[En_objp->instance].flags2 & SF2_DISABLED) {
		int treat_as_ship;
...
void ai_dock()
{
	...
	case AIS_DOCK_0:
		...
		aip->path_start = -1;
		aip->submode_start_time = Missiontime;
		// nasty hack to make sure ship doesn't overshoot path because it naturally has a slow decel time
		Pl_objp->phys_info.forward_decel_time_const = 0.25f;
		break;
	...
	case AIS_UNDOCK_0: {
		...
		// hack to set decel back to ship specific number when starting to undock
		Pl_objp->phys_info.forward_decel_time_const = sip->forward_decel;
		break;
							 }
	case AIS_UNDOCK_1: {
...
ship_subsys *aifft_find_turret_subsys(object *objp, ship_subsys *ssp, object *enemy_objp, float *dot_out)
{
	...
	*dot_out = best_dot;
	...
	if (!(esip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP | SIF_MEDIUM_SHIP)))
		return best_subsysp;
...
void turret_fire_weapon(ship_subsys *turret, int parent_objnum, vector *turret_pos, vector *turret_fvec, vector *predicted_pos = NULL)
{
	...
	if (check_ok_to_fire(parent_objnum, turret->turret_enemy_objnum, &Weapon_info[turret_weapon_class])) {
		...
		if(wip->wi_flags & WIF_BEAM){
			...
			if (parent_ship->cloak_stage == CLOAK_ON)
			{
			...
			{		
				...
				if ((parent_ship->cloak_stage > CLOAK_OFF) && (parent_ship->cloak_stage < CLOAK_COOLDOWN))
				{
...
void ai_fire_from_turret(ship *shipp, ship_subsys *ss, int parent_objnum)
{
	...
	// disabled can't shoot turrets
	if (shipp->flags2 & SF2_DISABLED)
		return;

	// if the system is less then 20% then have the turret cut out firing from time to time
	if (get_system_pct(objp) <= 0.2) {
		int x;

		srand(time(NULL));

		//25% chance?  sounds good to me
		x = rand()%4+1;
	...
	if ( (ss->turret_enemy_objnum >= 0 && ss->turret_enemy_objnum < MAX_OBJECTS) && (ss->turret_enemy_sig == Objects[ss->turret_enemy_objnum].signature)) {
		...
		if ( Weapon_info[turret_weapon_class].wi_flags & WIF_HUGE ) {
			...
			if ( !(Ship_info[Ships[lep->instance].ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP | SIF_MEDIUM_SHIP)) ) {
				return;
			}
		}
...
void process_subobjects(int objnum)
{
	...
	for ( pss = GET_FIRST(&shipp->subsys_list); pss !=END_OF_LIST(&shipp->subsys_list); pss = GET_NEXT(pss) ) {
		...
		ship_do_submodel_rotation(shipp, psub, pss, objp);
	}
...
void ai_warp_out(object *objp)
{
	...
	//if ship doesn't have hyperdrive can't warp
	if (sip->hyperdrive == 0) { 
		return;
	}
	...
	case AIS_WARP_1:
		...
		// Big ships get longer time to manuver
		if (Ship_info[Ships[objp->instance].ship_info_index].flags & SIF_HUGE_SHIP) 
		{
			aip->force_warp_time = timestamp((int)force_big_ai_jump_after*1000);
		}
		else 
		{
			aip->force_warp_time = timestamp((int)force_ai_jump_after*1000);
		}
	...
	//if ((timestamp_elapsed(aip->force_warp_time)) && (!collide_predict_large_ship(objp, objp->radius*2.0f + 500.0f))) 
	if (timestamp_elapsed(aip->force_warp_time) && !collide_predict_large_ship(objp, 1000000.0f)) // || !collide_predict_large_ship(objp, objp->radius*2.0f + 100.0f)) {
		{ 
			...
			if (ai_hyper_speed > ship_get_max_speed(&Ships[objp->instance])) {
				// recalculate collision pairs
				OBJ_RECALC_PAIRS(objp);	
			}

			// changed to about a half a second
			aip->force_warp_time = timestamp((int)ai_hyper_speed_time);		//	Try to attain target speed for up to x seconds.
		} 
		else {
			// fly ship towards jump point then hyper out
			if (shipp->jump_anchor >= 0) {
				float	dist;
				float	dot;
				vector	v2v;
				vector *pos = &Jumppoint_lists[shipp->jump_anchor].waypoints[0];
			
				dist = vm_vec_normalized_dir(&v2v, pos, &Pl_objp->pos);

				if (dist < objp->radius + 50.0f) {
					accelerate_ship(aip, 0.0f);
					aip->force_warp_time = timestamp(0);
		
			} else {
				dot = vm_vec_dot(&objp->orient.vec.fvec, &v2v);

				accelerate_ship(aip, 1.0f);
			}
			turn_towards_point(objp, pos, NULL, 0.0f);
			// just hyper ship out where its at
			} else {		
				vector goal_point;
				vector aim_vect;
			
				// Set the direction of the hyperjump
				if(Ships[objp->instance].team == TEAM_HOSTILE) {
					goal_point = jump_point_enemy;
				}
				else if (Ships[objp->instance].team == TEAM_FRIENDLY) {
					goal_point = jump_point_friendly;
				}
				else if (Ships[objp->instance].team == TEAM_NEUTRAL) {
					goal_point = jump_point_neutral;
				}
				else {
					goal_point = jump_point_unknown;
				}

				// Turn the ship throrwards the jumpout point
				turn_towards_point(objp, &goal_point, NULL, 0.0f);
				accelerate_ship(aip, 0.0f);

				// Calculate the jump vector
				vm_vec_sub(&aim_vect, &goal_point, &(objp->pos));

				// Check to see if jump vector is achieved
				if(is_vect_direction_equal(aim_vect, objp->orient.vec.fvec)) 
				{
					// Make the jump now
					aip->force_warp_time = timestamp(0);
				}
	...
	case AIS_WARP_3:
		...
		goal_speed = ai_hyper_speed; //shipfx_calculate_warp_speed(objp);
...
//	validate_mode_submode(aip);
	...
	if (aip->mode == AIM_WARP_OUT ) {
		// if a craft is in the way of the jump point have the jumper
		// evade until the path is clear
		if (collide_predict_large_ship(Pl_objp, Pl_objp->radius*2.0f + 200.0f))
		{
			aip->active_goal = AI_ACTIVE_GOAL_DYNAMIC;
			aip->previous_mode = aip->mode;
			aip->previous_submode = aip->submode;
			...
			aip->submode = -1;
			aip->submode_start_time = Missiontime;
			aip->mode_time = timestamp(MAX_EVADE_TIME);

			// set the craft at half throttle so its not a complete
			// sitting duck
			accelerate_ship(aip, 50);
		}
...
*/
	...
	if ((En_objp != NULL) && ( aip->targeted_subsys != NULL )) {
		...
		if ( aip->targeted_subsys->current_hits <= 0.0f ) {
			...
			if ( aip->goals[0].ai_mode == AI_GOAL_DISABLE_ENGINES ) {
...
void ai_ship_hit(object *objp_ship, object *hit_objp, vector *hitpos, int shield_quadrant, vector *hit_normal)
{
	...
	// if the ship happens to be cloaked then turn the cloak off
	if (shipp->flags2 & SF2_CLOAK_ON)
	...
	if (hit_objp->type == OBJ_WEAPON) {
		...
		if ((Objects[hit_objp->parent].flags & OF_PROTECTED) ||
			(Ships[Objects[hit_objp->parent].instance].flags2 & SF2_DECOY_ON)) { 
	...
	if (Ships[hit_objp->instance].flags2 & SF2_DECOY_ON)
		return;
...
//	Make object *objp fly to point *vp and jump out.
void ai_fly_to_jump(object *objp, vector *vp)
{
	ai_info	*aip = &Ai_info[Ships[Pl_objp->instance].ai_index];
	float	dist;
	float	dot;
	...
	dist = vm_vec_normalized_dir(&v2v, vp, &Pl_objp->pos);

	if (dist < Pl_objp->radius + 50.0f) {
		...
		dot = vm_vec_dot(&Pl_objp->orient.vec.fvec, &v2v);

		aip = &Ai_info[Ships[Pl_objp->instance].ai_index];

		accelerate_ship(aip, 1.0f);

		turn_towards_point(Pl_objp, vp, NULL, 0.0f);
	}
}
