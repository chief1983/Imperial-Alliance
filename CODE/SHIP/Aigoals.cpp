 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:17:38  Darkhill
 * no message
 *
...
#define PURGE_GOALS		(AI_GOAL_IGNORE | AI_GOAL_DISARM_SHIP | AI_GOAL_DISABLE_ENGINES)
...
char *Ai_goal_text(int goal)
{
	...
	case 11:
		return "disable engines ";
	case 12:
		return XSTR( "disarm ", 482);
	case 13:
		return XSTR( "attack ", 482);
	case 15:
		return XSTR( "guard ", 480);
	case 16:
...
void ai_clear_ship_goals( ai_info *aip )
{
	...
	if (!(Player_ship != NULL) || (Player_use_ai))	
		  ai_do_default_behavior( &Objects[Ships[aip->shipnum].objnum] );
	//}
}
...
int ai_get_subsystem_type( char *subsystem )
{
	...
	} else if ( !strnicmp(subsystem, "pod", 3) )  {
		return SUBSYSTEM_POD;
	} else if ( !strnicmp(subsystem, "sfoil", 4) ) {
		return SUBSYSTEM_SFOIL;
...
void ai_goal_purge_invalid_goals( ai_goal *aigp, ai_goal *goal_list )
{
	...
	for ( i = 0; i < MAX_AI_GOALS; purge_goal++, i++ ) {
		...
		case AI_GOAL_IGNORE:
			if ( purge_ai_mode & (AI_GOAL_DISARM_SHIP | AI_GOAL_DISABLE | AI_GOAL_DISABLE_ENGINES | AI_GOAL_CHASE | AI_GOAL_CHASE_WING | AI_GOAL_DESTROY_SUBSYSTEM) )
		...
		case AI_GOAL_DISARM_SHIP:
			if ( purge_ai_mode & (AI_GOAL_CHASE | AI_GOAL_CHASE_WING | AI_GOAL_DISABLE) )
				purge_goal->flags |= AIGF_PURGE;
			break;
		}
	}
...
extern int ship_has_ions(ship *shipp);
...
/*	if ( (mode == AI_GOAL_DISABLE) && !(ship_has_ions(&Ships[aip->shipnum])) ) {
		//FILE *fp = fopen("aa.txt", "a");
		//fprintf(fp, "%s\n", Ships[aip->shipnum].ship_name);
		//fclose(fp);
		mode = AI_GOAL_DISABLE_ENGINES;
		submode = -SUBSYSTEM_ENGINE;
...
void ai_add_goal_sub_sexp( int sexp, int type, ai_goal *aigp )
{
	...
	case OP_AI_DISABLE_SHIP:
		aigp->ai_mode = AI_GOAL_DISABLE;
		aigp->ship_name = ai_get_goal_ship_name( CTEXT(CDR(node)), &aigp->ship_name_index );
		aigp->priority = atoi( CTEXT(CDR(CDR(node))) );
		break;

	case OP_AI_DISABLE_ENGINES:
		aigp->ai_mode = AI_GOAL_DISABLE_ENGINES;
		aigp->ship_name = ai_get_goal_ship_name( CTEXT(CDR(node)), &aigp->ship_name_index );
...
int ai_mission_goal_achievable( int objnum, ai_goal *aigp )
{
	...
	case AI_GOAL_DISABLE_ENGINES:
		status = mission_log_get_time( LOG_SHIP_DISABLED_ENGINES, aigp->ship_name, NULL, NULL );
		break;
	...
	if ( (aigp->ai_mode == AI_GOAL_CHASE_WING) || (aigp->ai_mode == AI_GOAL_GUARD_WING) ) {
		...
		if ( ship_find_exited_ship_by_name(aigp->ship_name) != -1 ) 
			status = SHIP_STATUS_GONE;
		else if ( ship_name_lookup( aigp->ship_name ) != -1 )
	...
	if ( (aigp->ai_mode == AI_GOAL_DOCK) && (status == SHIP_STATUS_ARRIVED) ) {
		...
		if ( Ships[objp->instance].flags & SF_ENGINES_DISABLED || Ships[objp->instance].flags2 & SF2_DISABLED)
			return AI_GOAL_NOT_KNOWN;
...
*/
	...
	case AI_GOAL_CHASE:
		...
		/*if (Ships[shipnum].flags2 & SF2_CLOAK_ON)
		{
			current_goal->ai_mode = AI_GOAL_STAY_STILL;
			break;
		}*/
	...
	case AI_GOAL_DISABLE:
		...
		if ( current_goal->ship_name ) {
			...
			ai_disable_object(objp, other_obj, current_goal->priority);
		break;
