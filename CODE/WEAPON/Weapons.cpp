 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:36:25  Darkhill
 * no message
 *
...
void parse_pod_cloak(tertiary_weapon_info* twip)
{
	...
	required_string("+Recharge:");
	stuff_float(&twip->cloak_recover_rate);

	required_string("+Burn Rate:");
	stuff_float(&twip->cloak_burn_rate);

	required_string("+Energy:");
	stuff_float(&twip->cloak_energy_capacity);

	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->cloak_sound_engage);
		required_string("+Loop:");
		stuff_int(&twip->cloak_sound_loop);
	}

}

void parse_pod_boost(tertiary_weapon_info* twip)
{
	...
	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->boost_sound_engage);
		required_string("+Loop:");
		stuff_int(&twip->boost_sound_loop);
	}
}

void parse_pod_jammer(tertiary_weapon_info* twip)
{
	required_string("+Range:");
	stuff_float(&twip->jammer_range);
	
	required_string("+Recharge:");
	stuff_float(&twip->jammer_recover_rate);

	required_string("+Burn Rate:");
	stuff_float(&twip->jammer_burn_rate);

	required_string("+Energy:");
	stuff_float(&twip->jammer_energy_capacity);

	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->jammer_sound_engage);
		required_string("+Loop:");
		stuff_int(&twip->jammer_sound_loop);
	}
}

void parse_pod_turbo(tertiary_weapon_info* twip)
{
	required_string("+Overdrive:");
	stuff_float(&twip->turbo_speed_multiplier);

	required_string("+Acceleration:");
	stuff_float(&twip->turbo_speed_accel_mult);

	if (optional_string("$Sound:")) {
		required_string("+Loop:");
		stuff_int(&twip->turbo_sound_loop);
	}
}

void parse_pod_decoy(tertiary_weapon_info* twip)
{
	required_string("+Range:");
	stuff_float(&twip->decoy_range);
	
	required_string("+Recharge:");
	stuff_float(&twip->decoy_recover_rate);

	required_string("+Burn Rate:");
	stuff_float(&twip->decoy_burn_rate);

	required_string("+Energy:");
	stuff_float(&twip->decoy_energy_capacity);

	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->decoy_sound_engage);
		required_string("+Loop:");
		stuff_int(&twip->decoy_sound_loop);
	}
}

void parse_pod_slam(tertiary_weapon_info* twip)
{
	required_string("+Acceleration:");
	stuff_float(&twip->slam_acceleration);

	required_string("+Recharge:");
	stuff_float(&twip->slam_recharge);

	required_string("+Burn Rate:");
	stuff_float(&twip->slam_burn_rate);

	required_string("+Energy:");
	stuff_float(&twip->slam_fuel);

	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->slam_sound_engage);
		required_string("+Disengage:");
		stuff_int(&twip->slam_sound_disengage);
	}
}

void parse_pod_tractor(tertiary_weapon_info* twip)
{
	required_string("+Range:");
	stuff_float(&twip->tractor_range);
	
	required_string("+Recharge:");
	stuff_float(&twip->tractor_recover_rate);

	required_string("+Burn Rate:");
	stuff_float(&twip->tractor_burn_rate);

	required_string("+Energy:");
	stuff_float(&twip->tractor_energy_capacity);

	if (optional_string("$Sound:")) {
		required_string("+Engage:");
		stuff_int(&twip->tractor_sound_engage);
		required_string("+Loop:");
		stuff_int(&twip->tractor_sound_loop);
	}
}
...
void parse_tertiary()
{
	...
	{
		case TWT_CLOAK:
			parse_pod_cloak(twip);
			break;

		case TWT_BOOST_POD:
			parse_pod_boost(twip);
			break;

		case TWT_DECOY:
			parse_pod_decoy(twip);
			break;

		case TWT_JAMMER:
			parse_pod_jammer(twip);
			break;
		...
		case TWT_TURBOCHARGER:
			parse_pod_turbo(twip);
			break;

		case TWT_TRACTOR:
			parse_pod_tractor(twip);
			break;
...
void parse_weaponstbl(char* longname, bool is_chunk)
{
	...
	if (optional_string("#POD Modules"))
	{
		while (required_string_either("#End", "$Name:")) {
...
void find_homing_object(object *weapon_objp, int num)
{
	...
	for ( objp = GET_FIRST(&obj_used_list); objp !=END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp) ) {
		if ((objp->type == OBJ_SHIP) || (objp->type == OBJ_CMEASURE)) {
			...
			// just to make it interesting a heat seeker will go after 
			// *any* target, not just objects on the hostile team
...
*/
	...
	// if target is using either cloak or decoy then missile lock is lost
	if (wip->wi_flags & WIF_HOMING_ASPECT) {
		if (Ships[Objects[wp->target_num].instance].flags2 & SF2_NO_RADAR) {
			wp->target_num = -1;
			wp->homing_object = &obj_used_list;
			return;
		}
