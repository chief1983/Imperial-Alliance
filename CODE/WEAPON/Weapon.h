 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:35:52  Darkhill
 * no message
 *
...
#define TWT_CLOAK			1			//cloaking device
#define TWT_BOOST_POD		2			//one-shot (or multi-shot) fast afterburner
#define TWT_DECOY			3			//makes your ship untargetable on radar. 
#define TWT_JAMMER			4			//disrupts lasers on all craft in range
#define TWT_SLAM			5			// sublight accleration motor.  doubles speed 
#define TWT_TURBOCHARGER	6			//souped up engine. top speed will increase. afterburner will replenish faster.  now i need a 6" exhaust pipe :p
#define TWT_TRACTOR			7			// keeps target ship from manouvering away	

#define CLOAK_OFF			0
#define CLOAK_WARMUP		1
#define CLOAK_ON			2
#define CLOAK_COOLDOWN		3
...
typedef struct tertiary_weapon_info {
	...
	float cloak_energy_capacity;		// total engery for pod weapon				
	float cloak_burn_rate;				// how fast pod burns energy		
	float cloak_recover_rate;			// how fast pod recovers energy

	int cloak_sound_engage;			// sound for start
	int cloak_sound_loop;			// looping sound
	
	//boost pod info
	int boost_lifetime;				// how long boost shot lasts
	int boost_num_shots;			// how many shots you got
	float boost_speed;				// top speed while on boost
	float boost_acceleration;		// speed modifier while active

	int boost_sound_engage;			// sound for start
	int boost_sound_loop;			// looping sound

	// decoy
	float decoy_range;				// range at which ship is seen on radar
	float decoy_energy_capacity;	// total engery for pod weapon				
	float decoy_burn_rate;			// how fast pod burns energy		
	float decoy_recover_rate;		// how fast pod recovers energy

	int decoy_sound_engage;			// sound for start
	int decoy_sound_loop;			// looping sound
	
	//jammer
	float jammer_range;				//	distance where jammed ship can't fire
	float jammer_energy_capacity;	// total engery for pod weapon				
	float jammer_burn_rate;			// how fast pod burns energy		
	float jammer_recover_rate;		// how fast pod recovers energy

	int jammer_sound_engage;			// sound for start
	int jammer_sound_loop;			// looping sound
	
	//S.L.A.M info
	float slam_acceleration;		// speed modifier while active
	float slam_recharge;			// how fast engery recharges
	float slam_burn_rate;			// how much fuel burned 
	float slam_fuel;				// how fuel slam has

	int slam_sound_engage;			// sound for start
	int slam_sound_disengage;		// sound for end
	...
	int turbo_sound_loop;			// looping sound
	
	//tractor
	float tractor_range;			// max distance beam can stop ship
	float tractor_energy_capacity;	// total engery for pod weapon				
	float tractor_burn_rate;			// how fast pod burns energy		
	float tractor_recover_rate;		// how fast pod recovers energy

	int tractor_sound_engage;			// sound for start
	int tractor_sound_loop;			// looping sound
