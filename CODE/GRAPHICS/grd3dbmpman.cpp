...
bitmap * bm_d3d_lock( int handle, ubyte bpp, ubyte flags )
{
 ...
 //		Assert( bpp == 8 );
	...
	if(be->type > BM_TYPE_32_BIT_FORMATS && be->type != 100)
	{
		if(d3d_bitmap_entry[bitmapnum].tinterface == NULL)
		{
			switch ( be->type ) {
	...
	*/
		...
		case 100: 
			if (be->ref_count > 1)
				be->ref_count = 1;
			if(d3d_bitmap_entry[bitmapnum].tinterface == NULL) {
				mprintf(("Locking Special User Bm: %s %d\n", be->filename, bitmapnum));
				if (ttt < 3) {
					ttt++;
				if (ttt == 3)
					ttt = 0;
				...
				sprintf(be->filename, "test000%d", ttt);
				bm_d3d_free_data(bitmapnum);
				d3d_bitmap_entry[bitmapnum].tinterface = 
						(IDirect3DBaseTexture8 *) d3d_lock_d3dx_types(be->filename, be->type, flags, bitmapnum);	
			}
