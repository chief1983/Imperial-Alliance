 ...
 *
 * $Log: grd3dtexture.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:46:48  Darkhill
 * no message
 *
...
void d3d_tcache_flush()
{
	...
	//if (Textures->bitmap_id < 0);
	//	Warning(LOCATION, "big problem");

	for( i=0; i<MAX_BITMAPS; i++ )	{
		// when quitting the game and this function is called it gives
		// a illegal page fault error.  not real sure why this is happening
		// opengl still gives the error with its function call commented
		// out.  this really needs to be looked at and see what is wrong.
...
IDirect3DTexture8 *d3d_make_texture(void *data, int bitmapnum, int size, int type, int flags) 
{
	...
	switch(source_desc.Format)
	{
		...
		default:
		{
			...
			case 100: use_alpha_format = (source_desc.Format == D3DFMT_A8R8G8B8); break; // 32 Bit TGAs only
	...
	*uscale = *vscale = 1.0;
	...
	// so we can use non power of two sized textures for background have to figure the first
	// largest power of two to create the texture
	// for example with 640x480 the texture size created needs to 1024x512
	int width, height;
	uint scale = 1;
	...
	if (! (Game_mode & GM_IN_MISSION)) {
		while (scale <= source_desc.Width) 
			scale*=2;
		
		width = scale;

		scale = 1;
		while (scale <= source_desc.Height) 
			scale*=2;
...
void *d3d_lock_d3dx_types(char *file, int type, ubyte flags, int bitmapnum)
{
	...
	switch(type)
	{
		...
		case 100: strcat(filename, ".tga"); break;
		default: return NULL;
	}
	...
	byte *tga_data = (byte *)malloc(size);
	...
	int w, h, bpp;
	targa_read_header(file, &w, &h, &bpp, NULL);
	
	// if its a 24 or 32 bit targa we want to hijack it first and read it through to adjust
	// for color keying
	if (type == BM_TYPE_TGA && (bpp == 24 || bpp == 32)) {
		byte *temp = (byte *)malloc(w*h*4+18);
		
		byte *header = (byte *)malloc(sizeof(byte) * 18);
		byte *data = (byte *)malloc(w*h*4);
		memcpy(header, (byte *)tga_data, 18);
		...
		targa_read_bitmap_32(file, data, NULL, 0);
		
		free(tga_data);
		
		memcpy(temp, header, 18);
		memcpy((temp+18), data, w*h*4);
		tga_data = temp;

		size = w*h*4+18;
	}

	IDirect3DTexture8 *ptexture = d3d_make_texture((void *)tga_data, bitmapnum, size, type, flags);
