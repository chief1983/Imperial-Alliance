 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:38:15  Darkhill
 * no message
 *
	...
	if ( closest_dist < FLYBY_MIN_DISTANCE ) {
		...
		if ( relative_speed > FLYBY_MIN_RELATIVE_SPEED ) {
			if ( timestamp_elapsed(Flyby_next_sound) ) {
				...
				// setup the game_snd struct to play the craft specific
				// flyby sound
				flyby.default_volume = 1.0;
				strcpy(flyby.filename, sip->flyby);
				flyby.id = -1;
				flyby.id_sig = -1;
				flyby.flags = 0; // don't set this to 0 first, won't work
				flyby.min = 200;
				flyby.max = 400;
				flyby.preload = 0;
				flyby.flags |= GAME_SND_USE_DS3D; //= Snds_flyby[0][0].flags;
											
				// play da sound
				snd_play_3d(&flyby, &closest_objp->pos, &View_position);
