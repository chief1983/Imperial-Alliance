 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:38:02  Darkhill
 * no message
 *
...
float get_system_pct(object *objp)
{	
	Assert(objp);
	Assert(objp->type == OBJ_SHIP);
	...
	float total_strength = Ships[objp->instance].ship_initial_system_strength;

	if (total_strength == 0.0f)
		return 0.0f;

	if (objp->system_strength < 0.0f)
		return 0.0f;

	return objp->system_strength / total_strength;
}

float get_system_strength(object *objp)
{
	float	strength = 0.0f;

	strength = objp->system_strength;
	
	return strength;
}

void set_system_strength(object *objp, float strength)
{
	Assert(objp);
	...
	objp->system_strength = strength;
}

void add_system_strength(object *objp, float strength)
{
	Assert(objp);
...
void obj_player_fire_stuff( object *objp, control_info ci )
{
	...
	if (ci.fire_pod)
	{
		...
		//	}
			//objp->c  phys_info.speed = 1000.0f;
...
int obj_iff(object *objp)
{
	Assert( objp != NULL );
	
	// only ships should have iff...
	if (objp->type == OBJ_SHIP)
		return Ships[objp->instance].iff;
	//...not anymore
	if (objp->type == OBJ_JUMP_NODE)
		return Jump_nodes[objp->instance].iff;
