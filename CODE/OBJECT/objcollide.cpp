 ...
 *
 * $Log: objcollide.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:37:47  Darkhill
 * no message
 *
...
int collide_predict_large_ship(object *objp, float distance)
{
	...
	for ( objp2 = GET_FIRST(&obj_used_list); objp2 != END_OF_LIST(&obj_used_list); objp2 = GET_NEXT(objp2) ) {
		if ((objp != objp2) && (objp2->type == OBJ_SHIP)) {
			if (Ship_info[Ships[objp2->instance].ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP | SIF_MEDIUM_SHIP)) {
				if (objp2 == dock_objp)
					continue;
		...
		} else if (!(sip->flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP | SIF_MEDIUM_SHIP)) && (objp2->type == OBJ_ASTEROID)) {
