 ...
 *
 * $Log: collideshipweapon.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:37:38  Darkhill
 * no message
 *
			...
			if (!(shipp->flags & SF_DYING) && ship_is_shield_up(ship_obj,quadrant_num) ) {
				...
				if ( Weapon_info[Weapons[weapon_obj->instance].weapon_info_index].wi_flags2 & WIF2_PIERCE_SHIELDS )	{
					...
					add_shield_point(ship_obj-Objects, mc.shield_hit_tri, &mc.hit_point, 
						Weapon_info[Weapons[weapon_obj->instance].weapon_info_index].laser_color_1.red/4,
						Weapon_info[Weapons[weapon_obj->instance].weapon_info_index].laser_color_1.green/4,
						Weapon_info[Weapons[weapon_obj->instance].weapon_info_index].laser_color_1.blue/4);
				} else {
					valid_hit_occured = 1;
					// shield effect
					add_shield_point(ship_obj-Objects, mc.shield_hit_tri, &mc.hit_point,
