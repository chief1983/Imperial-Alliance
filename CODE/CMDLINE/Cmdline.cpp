 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:05 $
 * $Author: Darkhill $
 *
 * $Log: Cmdline.cpp,v $
 * Revision 1.2  2004/09/07 22:56:05  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:06:55  Darkhill
 * no message
 *
...
Flag exe_params[] = 
{
	...
	"-ogg",			  "Use Ogg Vorbis jukebox",			false,	EASY_MEM_ALL_ON, EASY_DEFAULT_MEM,	"Audio",		"", 
	"-radar",		  "Use front/back radar system",	false,	EASY_MEM_ALL_ON, EASY_DEFAULT_MEM,	"Gameplay",		"",

	"-give_all_medals", "Give current player all medals", false, 0,		EASY_DEFAULT,		"Dev Tool",		"", 
	"-make_me_admiral", "Make current player highest rank", false, 0,	EASY_DEFAULT,       "Dev Tool",     "",
	"-debug_hud",		"Show all HUD parts for debuging", false, 0,	EASY_DEFAULT,       "Dev Tool",		"",
...
cmdline_parm use_jukebox("-ogg", NULL);
cmdline_parm use_tworadar("-radar", NULL);

cmdline_parm give_all_medals("-give_all_medals",NULL);
cmdline_parm make_me_admiral("-make_me_admiral",NULL); 

cmdline_parm debug_hud("-debug_hud", NULL);
...
int Cmdline_UseJukebox = 0; // Turn off to use original music, turn on to use the jukebox feature
int Cmdline_UseTwoRadar = 0; // Turn on to use front/back radar system of TIE Fighter
int Cmdline_gimme_all_medals = 0; // Give all medals to current pilot
int Cmdline_make_me_admiral = 0; // Give highest rank to current pilot
int Cmdline_debug_hud = 0;		// Show all parts of the HUD to debug
...
void os_init_cmdline(char *cmdline)
{
	...
	fp = fopen("data\\cmdline_ia.cfg", "rt");
...
// Sets externed variables used for communication cmdline information
{
	...
	//Gimme all the damn medals!
	if (give_all_medals.found()) {
		Cmdline_gimme_all_medals = 1;
	}

	//Gimme some brass!
	if (make_me_admiral.found()) {
		Cmdline_make_me_admiral = 1;
	}
	
	if (use_jukebox.found())
	{
		Cmdline_UseJukebox = 1;
	}
	
	if (use_tworadar.found())
	{
		Cmdline_UseTwoRadar = 1;
	}
	if (no_fpscap.found())
	{
		Cmdline_NoFPSCap = 1;
