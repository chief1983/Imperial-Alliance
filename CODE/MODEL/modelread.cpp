 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:22 $
 * $Author: Darkhill $
 *
 * file which reads and deciphers POF information
 *
 * $Log: modelread.cpp,v $
 * Revision 1.2  2004/09/07 22:56:22  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:56:42  Darkhill
 * no message
 *
 ...
 * Added turret and turret_fov_cos
 * 
...
static void set_subsystem_info( model_subsystem *subsystemp, char *props, char *dname )
{
	...
	} else if ( strstr(lcdname, "pod") ) {
		subsystemp->type = SUBSYSTEM_POD;
	} else if ( strstr(lcdname, "sfoil") ) {
		subsystemp->type = SUBSYSTEM_SFOIL;
	
		if ( (p = strstr(props, "$side")) != NULL) {
			get_user_prop_value(p+5, buf);

			subsystemp->sfoil_side = (int)atoi(buf);
		} else
			subsystemp->sfoil_side = -1;

		if ( (p = strstr(props, "$direction")) != NULL) {
			get_user_prop_value(p+10, buf);
					
			subsystemp->sfoil_direction = (int)atoi(buf);
		} else
			subsystemp->sfoil_direction = -1;

		if ( (p = strstr(props, "$rom")) != NULL) {
			get_user_prop_value(p+4, buf);
			
			subsystemp->sfoil_rom = (float)atof(buf);
		} else
			subsystemp->sfoil_rom = -1;
	...
	// if the submodel has the $fov entry in properties and *isn't* a turret we assume that the
	// user wants a limited movement field for this object, so away we go
	if ( (p = strstr(props, "$fov")) != NULL && !(strstr(lcdname, "turret")) ) {
		float angle;
			
		// about the only thing this is good for is the actual code that rotates the suboject
		// if the flag is set then we go back and forth, if not set then use the original code
		subsystemp->flags |= MSS_FLAG_ROTATES_LIMITED;
		get_user_prop_value(p+4, buf);			// get the value of the fov
		angle = (float)atoi(buf);
		subsystemp->rotation_fov = (float)angle;
		subsystemp->direction = rand()%1+1;  // randomize which direction it starts in
		subsystemp->ray = (float)ANG_TO_RAD(subsystemp->rotation_fov / 2.0);  // calculate area it can travel in
	}
	// Rotating subsystem
...
void model_load_texture(polymodel *pm, int i, char *file)
{
    ...
    //if (Cmdline_nospec) {
    //    pm->normal_textures[i] = -1;
    //    pm->normal_original_textures[i] = -1;
	//} else {
    /*    strcpy(tmp_name, file);
        strcat( tmp_name, "-normal");

        pm->normal_textures[i] = bm_load( tmp_name );

		if (pm->normal_textures[i] > 0) {
			FILE *fp = fopen("normal.txt", "w");
			fprintf(fp, "%s %d", tmp_name, pm->normal_textures[i]);
			fclose(fp);
			normal_id = pm->normal_textures[i];
		}

        if (pm->normal_textures[i]<0) {     //if I couldn't find the PCX see if there is an ani-Bobboau
	        pm->normal_textures[i] = -1;

            pm->normal_original_textures[i] = -1;
        }

            pm->normal_original_textures[i] = pm->normal_textures[i];
      //}
      //Angelus - End*/
...
float big_degree_num;
// Rotates the angle of a submodel.  Use this so the right unlocked axis
// gets stuffed.
void submodel_rotate(int obj_num, model_subsystem *psub, submodel_instance_info *sii, object *objp)
{
	bsp_info * sm;
	...
	case MOVEMENT_AXIS_X:
		// don't spin all the way around use this
		if (psub->flags & MSS_FLAG_ROTATES_LIMITED) {
			if (psub->direction) {
				sii->angs.p += delta;
				if (sii->angs.p > psub->ray) {
					psub->direction = 0;
					sii->angs.p = psub->ray;
				}
			} else {
			...
			// spin in complete circle use this
			sii->angs.p += delta;
			if (sii->angs.p > PI2 )
				sii->angs.p -= PI2;
	...
	case MOVEMENT_AXIS_Z:
		...
		if (shipp->sfoil_status & SFOIL_OPENING) {
			// left side up
			if ((psub->sfoil_direction == 1 && psub->sfoil_side == 1) || 
			    // right side down
				(psub->sfoil_direction == 0 && psub->sfoil_side == 0) ) {
				if (sii->angs.b >= ANG_TO_RAD(0)) {
					sii->angs.b -= delta;
			...
			// left side down
			if ( (psub->sfoil_direction == 0 && psub->sfoil_side == 1) ||
			     // right side up 
				 (psub->sfoil_direction == 1 && psub->sfoil_side == 0) ) {
				if (sii->angs.b <= ANG_TO_RAD(psub->sfoil_rom)) {
					sii->angs.b += delta;
					big_degree_num = sii->angs.b;
				} else {
					shipp->sfoil_status &= ~(SFOIL_OPENING);
					shipp->sfoil_status |= SFOIL_OPEN;
				}
