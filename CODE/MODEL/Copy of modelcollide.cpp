 ...
 *
 * $Log: modelcollide.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:55:20  Darkhill
 * no message
 *
...
int model_collide_sub1(vector min, vector max )
{
	if (!mc_ray_boundingbox(&min, &max, &Mc_p0, &Mc_direction, NULL ))	{
			return 1;
	}
...
void mc_check_subobj( int mn )
{
	...
	if (mc_ray_boundingbox(&sm->min, &sm->max, &Mc_p0, &Mc_direction, &hitpt))	{
		...
		if ( Mc->flags & MC_ONLY_BOUND_BOX )	{
			...
			//model_collide_sub1(sm->min, sm->max);
		}
	} else {
		//Int3();
	}
...
//	}
	...
	//model_radius = Mc_pm->core_radius;
