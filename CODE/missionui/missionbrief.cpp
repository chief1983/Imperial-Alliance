 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:57:05 $
 * $Author: Darkhill $
 *
 * C module that contains code to display the mission briefing to the player
 *
 * $Log: missionbrief.cpp,v $
 * Revision 1.2  2004/09/07 22:57:05  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 22:00:15  Darkhill
 * no message
 *
...
static int Closeup_coords[GR_NUM_RESOLUTIONS][4] = {
	{
		203, 113, 200, 213	//203, 151, 200, 213// GR_640
	},
	{
...
int Closeup_region[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		211, 120, 215, 157 //211, 158, 215, 157
	}, 
	{ // GR_1024
		337, 253, 345, 252
	}, 
...
#define NUM_BRIEF_BUTTONS	9 //10
#define END_STAGE			0
#define NEXT_STAGE			1
#define PREV_STAGE			2
#define START_STAGE			3
#define TEXT_UP				4
#define TEXT_DOWN			5
#define SKIP_MISSION		6
...
brief_buttons	Brief_buttons[GR_NUM_RESOLUTIONS][NUM_BRIEF_BUTTONS] = {
	{ // GR_640
		brief_buttons("BRB_08",		245,	420,	117,	157,	8),
		brief_buttons("BRB_09",		190,	420,	117,	157,	9),
		brief_buttons("BRB_10",		80,		420,	117,	157,	10),
		brief_buttons("BRB_11",		25,		420,	117,	157,	11),
		brief_buttons("BRB_12",		590,	330,	117,	157,	12),
		brief_buttons("BRB_13",		590,	390,	117,	157,	13),			
		brief_buttons("BRB_15",		510,	420,	117,	157,	15),			// skip training
		brief_buttons("BRB_16",		135,	420,	117,	157,	16),
		brief_buttons("TSB_34",		455,	420,	117,	157,	34),	
		//brief_buttons("BRB_15",		562,	0,		117,	157,	15)			// exit loop	
	}, 
	{ // GR_1024
		brief_buttons("2_BRB_08",		392,	672,	117,	157,	8),
...
// briefing UI
#define BRIEF_SELECT_NUM_TEXT			9
UI_XSTR Brief_select_text[GR_NUM_RESOLUTIONS][BRIEF_SELECT_NUM_TEXT] = {
	{ // GR_640
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_LAST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_NEXT_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_FIRST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_LAST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_SCROLL_UP].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_SCROLL_DOWN].button },
		{ "Skip Training",	1442,	465,	7,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_SKIP_TRAINING].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_PAUSE].button },
		{ "Unlock",			-1,		456,	441,	UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_MULTI_LOCK].button },
		//{ "Exit Loop",		1477,	490,	7,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[0][BRIEF_BUTTON_EXIT_LOOP].button }
	}, 
	{ // GR_1024
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_LAST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_NEXT_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_FIRST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_LAST_STAGE].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_SCROLL_UP].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_SCROLL_DOWN].button },
		{ "Skip Training",	1442,	467,	7,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_SKIP_TRAINING].button },
		{ "",				-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_PAUSE].button },
		{ "Unlock",			-1,		470,	441,	UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_MULTI_LOCK].button },
	//	{ "Exit Loop",		1477,	490,	7,		UI_XSTR_COLOR_GREEN, -1, &Brief_buttons[1][BRIEF_BUTTON_EXIT_LOOP].button }
	}
};

void mission_brief_maybe_show_button_text();
...
void mission_brief_force_button_frame();
...
void brief_do_next_pressed(int play_sound)
{
	...
	if (!(timestamp_elapsed(Brief_last_auto_advance))) {
...
{ 	
	...
	Brief_last_auto_advance = timestamp(500);
...
void brief_render_closeup(int ship_class, float frametime)
{
	...
	if ( Closeup_icon->type == ICON_BUOY ) {
...
{ 
	...
	// game adds a stage automatically for the objectives at the end of the briefing if the mission
	// is not a training mission.  have to assume that if this is 1 then there was no briefing 
	// assigned to the battle and this 1 is just the added stage for the objectives so we display
	// no briefing at all
	if ( Num_brief_stages == 1 ) {
	...
	brief_render_text_tag(Current_brief_stage);
...
int brief_setup_closeup(brief_icon *bi)
{
	...
	case ICON_BUOY:
	//		case ICON_JUMP_NODE:
		strcpy(pof_filename, NOX("navbuoy.pof"));
		strcpy(Closeup_icon->closeup_label, "nav buoy");
...
/*	case ICON_UNKNOWN:
	...
	default:
		...
		if ( sip->flags & (SIF_SMALL_SHIP||SIF_MEDIUM_SHIP|SIF_BIG_SHIP|SIF_HUGE_SHIP|SIF_SENTRYGUN) ) {
...
void brief_do_frame(float frametime)
{
	...
	if ( Current_brief_stage < (Num_brief_stages-1) ) {
		if ( !help_overlay_active(BR_OVERLAY) && brief_time_to_advance(Current_brief_stage, frametime) ) {
			...
			common_flash_button_init();
			Brief_last_auto_advance = timestamp(500);//timer_get_milliseconds();
		}
	}
	...
	if ( !Background_playing ) {
		...
		mission_common_force_button_frame(BRIEFING);
		mission_brief_force_button_frame();
		brief_redraw_pressed_buttons();
		common_render_selected_screen_button();
		mission_common_maybe_show_button_text(BRIEFING);
		...
		mission_brief_maybe_show_button_text();
		}
#endif
	}	
...
void mission_brief_force_button_frame()
{
	for (int x=0; x<NUM_BRIEF_BUTTONS; x++) {
		if ( !Brief_buttons[gr_screen.res][x].button.button_down()) 
			// don't show lock button if not in multi
			if ((x == LOCK_STUFF) && !(Game_mode & GM_MULTIPLAYER))
				continue;
			// don't show skip mission button if not in training mission
			else if ((x == SKIP_MISSION) && !(The_mission.game_type & MISSION_TYPE_TRAINING))
				continue;
			else
				Brief_buttons[gr_screen.res][x].button.draw_forced(1);
	}
}
...
void mission_brief_maybe_show_button_text()
{
	...
	for (int i=0; i<BRIEF_SELECT_NUM_TEXT; i++) {
		b = &Brief_buttons[gr_screen.res][i];

		if (b->button.button_hilighted()) {
			gr_set_color_fast(&Color_bright_white);
			gr_string(Brief_select_text[gr_screen.res][i].x, Brief_select_text[gr_screen.res][i].y, Brief_select_text[gr_screen.res][i].xstr);
		}
	}
}