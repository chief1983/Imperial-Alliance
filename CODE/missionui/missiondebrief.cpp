 ...
 *
 * $Log: missiondebrief.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 22:00:42  Darkhill
 * no message
 *
...
int Debrief_text_wnd_coords[GR_NUM_RESOLUTIONS][4] = {
	{	// GR_640
		275, 135, 288, 210//240,25, 392,245//43, 140, 339, 303			
	},	
	{	// GR_1024
		69, 224, 535, 485		
	}
...
int Debrief_list_coords[GR_NUM_RESOLUTIONS][4] = {
	{	// GR_640
		35, 260, 188, 164//416, 280, 195, 101
	},
	{	// GR_1024
		666, 448, 312, 162
	}
...
int Debrief_stats_lines[GR_NUM_RESOLUTIONS] = {
	25,	// GR_640
	30  // GR_1024
};
...
char *Debrief_multi_mask_name[GR_NUM_RESOLUTIONS] = {
	"DebriefMulti-m",			// GR_640
	"2_DebriefMulti-m"			// GR_1024
};
...
char *Debrief_slider_filename[GR_NUM_RESOLUTIONS] = {
	"slider",
	"2_slider"
};

static UI_SLIDER2 Debrief_slider;

int Debrief_slider_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		565,155,13,192
	},
	{ // GR_1024
...
static ui_button_info Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("DB_00",		45,		375,		-1,	7,		0),		// debriefing
		ui_button_info("DB_01",		100,	375,		-1,	23,		1),		// statistics
		ui_button_info("DB_02",		565,	135,	-1,	-1,		2),		// scroll stats up
		ui_button_info("DB_03",		565,	345,	-1,	-1,		3),		// scroll stats down
		ui_button_info("DB_04",		155,	375,	-1,	447,	4),		// replay mission
		ui_button_info("DB_05",		210,	375,	-1,	464,	5),		// recommendations
		ui_button_info("DB_06",		320,	375,	-1,	-1,		6),		// first page
		ui_button_info("DB_07",		375,	375,	-1,	-1,		7),		// prev page
		ui_button_info("DB_08",		430,	375,	-1,	-1,		8),		// next page
		ui_button_info("DB_09",		485,	375,	-1,	-1,		9),		// last page
		ui_button_info("DB_10",		441,	384,	-1,413,	10),		// pilot info
		ui_button_info("DB_11",		511,	384,	-1,	413,	11),		// kick
		ui_button_info("DB_12",		265,	375,	-1,	-1,	12),		// medals
		ui_button_info("DB_13",		240,	260,	-1,	-1,	13),		// scroll pilots up
		ui_button_info("DB_14",		240,	345,	-1,	-1,	14),		// scroll pilots down
		ui_button_info("DB_15",		538,	431,	-1,	440,	15),		// help
		ui_button_info("DB_16",		538,	455,	-1,	464,	16),		// options
		ui_button_info("DB_17",		540,	375,	-1,	413,	17),		// accept
	},
	{ // GR_1024
...
UI_XSTR Debrief_strings[GR_NUM_RESOLUTIONS][NUM_DEBRIEF_TEXT] = {
	{ // GR_640
		{ "Debriefing",						804,		36,	 396,		UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][DEBRIEF_TAB].button },
		{ "Statistics",						1333,		92, 396,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][STATS_TAB].button },
		{ "",								-1,			548, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][TEXT_SCROLL_UP].button },
		{ "",								-1,			596, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][TEXT_SCROLL_DOWN].button },
		{ "Replay Mission",					444,		129, 396,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][REPLAY_MISSION].button },
		{ "Recommendations",				1334,		176, 396,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][RECOMMENDATIONS].button },
		{ "",								-1,			372, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][FIRST_STAGE].button },
		{ "",								-1,			460, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PREV_STAGE].button },
		{ "",								-1,			504, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][NEXT_STAGE].button },
		{ "",								-1,			424, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][LAST_STAGE].button },
		{ "",								1310,		433, 413,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MULTI_PINFO_POPUP].button },
		{ "",								1266,		510, 413,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MULTI_KICK].button },
		{ "Medals",							1037,		266, 396,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MEDALS_BUTTON].button },	
		{ "",								-1,			548, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PLAYER_SCROLL_UP].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PLAYER_SCROLL_DOWN].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][HELP_BUTTON].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][OPTIONS_BUTTON].button },
		{ "Accept",							1035,		542, 396,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][ACCEPT_BUTTON].button },
	},
	{ // GR_1024
		{ "Debriefing",						804,		39,	 395,		UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][DEBRIEF_TAB].button },
		{ "Statistics",						1333,		98, 395,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][STATS_TAB].button },
		{ "",								-1,			548, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][TEXT_SCROLL_UP].button },
		{ "",								-1,			596, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][TEXT_SCROLL_DOWN].button },
		{ "Replay Mission",					444,		135, 395,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][REPLAY_MISSION].button },
		{ "Recommendations",				1334,		181, 395,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][RECOMMENDATIONS].button },
		{ "",								-1,			372, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][FIRST_STAGE].button },
		{ "",								-1,			460, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PREV_STAGE].button },
		{ "",								-1,			504, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][NEXT_STAGE].button },
		{ "",								-1,			424, 280,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][LAST_STAGE].button },
		{ "",								1310,		433, 413,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MULTI_PINFO_POPUP].button },
		{ "",								1266,		510, 413,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MULTI_KICK].button },
		{ "Medals",							1037,		270, 395,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][MEDALS_BUTTON].button },	
		{ "",								-1,			548, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PLAYER_SCROLL_UP].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][PLAYER_SCROLL_DOWN].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][HELP_BUTTON].button },
		{ "",								-1,			596, 392,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][OPTIONS_BUTTON].button },
		{ "Accept",							1035,		545, 395,	UI_XSTR_COLOR_GREEN,	-1,	&Buttons[0][ACCEPT_BUTTON].button },
	}
};
...
void debrief_text_scroll_up();
void debrief_text_scroll_down();

void debrief_maybe_show_button_text();
void debrief_force_button_frame();
...
static char* Debrief_multi_award_filename[GR_NUM_RESOLUTIONS][6] = {
	{
		"DebriefWings",
		"DebriefMedal",
		"DebriefCrest",
...
static char* Real_debrief_award_filename[GR_NUM_RESOLUTIONS][6];
...
void debrief_ui_init()
{
	...
	if (Game_mode & GM_NORMAL)
		...
		Debrief_ui_window.set_mask_bmap(Debrief_multi_mask_name[gr_screen.res]);
	...
	Debrief_slider.create(&Debrief_ui_window, Debrief_slider_coords[gr_screen.res][0], Debrief_slider_coords[gr_screen.res][1], Debrief_slider_coords[gr_screen.res][2], Debrief_slider_coords[gr_screen.res][3], 0, Debrief_slider_filename[gr_screen.res], &debrief_text_scroll_up, &debrief_text_scroll_down, NULL);

	// if debriefing ever starts crashing this is probably the problem
	if (Game_mode & GM_NORMAL) {
		memcpy(Real_debrief_award_filename, Debrief_award_filename, sizeof(Debrief_award_filename));
	} else {
#ifndef NO_NETWORK
		memcpy(Real_debrief_award_filename, Debrief_multi_award_filename, sizeof(Debrief_multi_award_filename));
#endif
	}
	...
	Award_bg_bitmap = bm_load(Real_debrief_award_filename[gr_screen.res][DB_AWARD_BG]);
...
void debrief_award_init()
{
	...
	if (Player->stats.m_medal_earned != -1) {
		if (Player->stats.m_medal_earned == 13) {  // special hack for the wings..
			...
			sprintf(buf, NOX("%s%0.2d"), Real_debrief_award_filename[gr_screen.res][DB_AWARD_WINGS], ver);		
			Wings_bitmap = bm_load(buf);

		} else if (Player->stats.m_medal_earned == 17) {  // special hack for the soc crest
			Crest_bitmap = bm_load(Real_debrief_award_filename[gr_screen.res][DB_AWARD_SOC]);
		} else {
			sprintf(buf, NOX("%s%0.2d"), Real_debrief_award_filename[gr_screen.res][DB_AWARD_MEDAL], Player->stats.m_medal_earned);
	...
	if ( Player->stats.m_promotion_earned != -1 ) {
		...
		sprintf(buf, NOX("%s%0.2d"), Real_debrief_award_filename[gr_screen.res][DB_AWARD_RANK], Promoted + 1);
	...
	if ( Player->stats.m_badge_earned != -1 ) {
		...
		sprintf(buf, NOX("%s%0.2d"), Real_debrief_award_filename[gr_screen.res][DB_AWARD_BADGE], i + 1);
...
{	
	...
	// hack to get around that FS2 wants to display all stats on one screen.  IA window is to
	// small to do this, to somewhat emulate the text scolling of all the other screens we will
	// adjust the y coord to start printing everything based on how the user has clicked the 
	// text up/down buttons.  since a clip window has been created what should be gone is clipped
	// there are about 25 (Debrief_stats_lines)lines of text for stats so if anything is added 
	// it needs to be adjusted
	if (Current_stage == DEBRIEF_MISSION_STATS || Current_stage == DEBRIEF_ALLTIME_STATS)
		y = -Text_offset * gr_get_font_height();
	else
		y = 0;
	
	gr_set_color_fast(&Color_bright_blue);
	gr_set_clip(Debrief_text_wnd_coords[gr_screen.res][0], Debrief_text_wnd_coords[gr_screen.res][1], Debrief_text_wnd_coords[gr_screen.res][2], Debrief_text_wnd_coords[gr_screen.res][3]);
	gr_string(0, y, Debrief_current_callsign);
	font_height = gr_get_font_height();
	y += 30;
	...
	switch ( Current_stage ) {
		case DEBRIEF_MISSION_STATS:
			...
			Num_text_lines = Debrief_stats_lines[gr_screen.res];
			//Debrief_slider.set_numberItems(25-17, 0);
			break;
...
void debrief_button_pressed(int num)
{
	switch (num) {
		...
		case TEXT_SCROLL_UP:
			debrief_text_scroll_up();
			Debrief_slider.forceUp();
		
			break;

		case TEXT_SCROLL_DOWN:
			debrief_text_scroll_down();
			Debrief_slider.forceDown();
...
void debrief_setup_ship_kill_stats(int stage_num)
{
	...
	int max_num_entries_viewable = Debrief_text_wnd_coords[gr_screen.res][3] / gr_get_font_height();
	Debrief_slider.set_numberItems((Num_text_lines > max_num_entries_viewable) ? (Num_text_lines - max_num_entries_viewable) : 0);
}
...
void debrief_text_init()
{
	...
	// hack so the slider has correct num of items for stats
	if (Current_stage == DEBRIEF_ALLTIME_STATS || Current_stage == DEBRIEF_MISSION_STATS)
		Debrief_slider.set_numberItems(Debrief_stats_lines[gr_screen.res]-max_num_entries_viewable);
...
*/
	...
	debrief_maybe_show_button_text();
...
void debrief_force_button_frame()
{
	// don't need to force door since it has animation
	for (int x=0; x<NUM_BUTTONS-1; x++) {
		if ( !Buttons[gr_screen.res][x].button.button_down()) 
			if (x == MEDALS_BUTTON && !Award_active)
				continue;
			else
...
void debrief_maybe_show_button_text()
{
	...
	for (int i=0; i<NUM_BUTTONS; i++) {
		if ((i == MULTI_PINFO_POPUP) || (i == MULTI_KICK))
			continue;
#ifndef NO_NETWORK
		if ( (i == MP_TEXT_INDEX_1 || i == MP_TEXT_INDEX_2) && !(Game_mode & GM_MULTIPLAYER) )
			continue;
#endif
		...
		if (b->button.button_hilighted()) {
			gr_set_color_fast(&Color_bright_white);
			gr_string(Debrief_strings[gr_screen.res][i].x, Debrief_strings[gr_screen.res][i].y, Debrief_strings[gr_screen.res][i].xstr);
