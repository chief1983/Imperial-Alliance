 ...
 *
 * $Log: missionweaponchoice.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 22:02:00  Darkhill
 * no message
 *
...
float WeaponSelectScreenShipRot = 0.0f;
int WeaponSelectModelNum = -1;
static matrix WeaponScreenOrient = IDENTITY_MATRIX;
...
#define IS_BANK_SECONDARY(x)		(x>4?1:0)
...
static wl_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_WEAPON_BUTTONS] = {
	{
		wl_buttons("CPB_03",		24,	125,		-1,		-1,	26),
		wl_buttons("CPB_04",		24,	276,		-1,		-1,	27),
		wl_buttons("CPB_03",		379,135,		-1,		-1,	8),
		wl_buttons("CPB_04",		379,287,		-1,		-1,	9),
		wl_buttons("ssb_39",		375,375,		-1,		-1,	39),
		wl_buttons("ssb_39",		0,		0,			-1,		-1,	99),
		wl_buttons("TSB_34",		540,320,		-1,		-1,	34)
	},
	{
...
static int Weapon_button_scrollable[NUM_WEAPON_BUTTONS] = {0, 0, 0, 0, 0, 0};
...
static int Wl_bank_count_draw_flags[MAX_WL_WEAPONS] = { 
	0, 0, 0, 0,			// primaries -- dont draw counts
...
static int Ship_view_coords[GR_NUM_RESOLUTIONS][4] = {
	{
		// GR_640
		49,81,279,226
	},
...
UI_XSTR Weapon_select_text[GR_NUM_RESOLUTIONS][WEAPON_SELECT_NUM_TEXT] = {
	{ // GR_640
		...
		{ "Unlock",			-1,			542,	341,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][WL_BUTTON_MULTI_LOCK].button }
	}, 
	{ // GR_1024
		{ "Reset",			1337,		938,	546,	UI_XSTR_COLOR_GREEN, -1, &Buttons[1][WL_BUTTON_RESET].button },
		{ "Unlock",			-1,		964,	584,	UI_XSTR_COLOR_GREEN, -1, &Buttons[1][WL_BUTTON_MULTI_LOCK].button }
	}
};

void weapon_select_maybe_show_button_text();
...
void wl_weapon_desc_start_wipe(int);
...
void weapon_buttons_init()
{
	...
	if ( Game_mode & GM_MULTIPLAYER ) {
		Common_buttons[WEAPON_SELECTION][gr_screen.res][COMMON_RESET_BUTTON].button.disable();
		Common_buttons[WEAPON_SELECTION][gr_screen.res][COMMON_RESET_BUTTON].button.hide();
...
void wl_maybe_reset_selected_weapon_class()
{
	...
	// don't need this, don't list laser weapons in game anymore
	...
	// don't need, don't list laser weapons in game
...
void weapon_select_init()
{
   ...
   // initialize the different regions of the menu that will react when the mouse moves over it
	...
	snazzy_menu_add_region(&Weapon_select_region[Num_weapon_select_regions++], "",	ICON_SHIP_PRIMARY_3,		0);
	...
	ship_class = Wss_slots[0].ship_class;
...
void wl_render_weapon_desc(float frametime)
{
	...
	//gr_set_color_fast(&Color_bright_white);
	//gr_string(10, 10, Weapon_info[Selected_wl_class].title);
}



// re-inits wiping vars and causes the current text to wipe in again
void wl_weapon_desc_start_wipe(int weapon_class)
{
	...
	//title_len = strlen(Weapon_info[weapon_class].title);
...
void weapon_select_do(float frametime)
{
	...
	if ( !Background_playing ) {		
		...
		mission_common_force_button_frame(WEAPON_SELECTION);
		wl_redraw_pressed_buttons();
		draw_wl_icons();
		...
		for ( int i = 0; i < MAX_WING_BLOCKS; i++ ) {
			draw_wing_block(i, Hot_wl_slot, Selected_wl_slot, -1, WEAPON_SEL_SCREEN);
		}
		common_render_selected_screen_button();
		mission_common_maybe_show_button_text(WEAPON_SELECTION);
			}

#ifndef NO_NETWORK
	// maybe blit the multiplayer "locked" button
	if((Game_mode & GM_MULTIPLAYER) && multi_ts_is_locked()) 
		Buttons[gr_screen.res][WL_BUTTON_MULTI_LOCK].button.draw_forced(1);
	if((Game_mode & GM_MULTIPLAYER) && !multi_ts_is_locked()) {
		Buttons[gr_screen.res][WL_BUTTON_MULTI_LOCK].button.draw_forced(2);
			weapon_select_maybe_show_button_text();
	}
	...
	//sprintf(aa, "%d", Selected_wl_class);
	//gr_string(10, 10, Weapon_info[Selected_wl_class].pofbitmap_name);

	if ( Weapon_anim_class != Selected_wl_class ) {
		wl_weapon_desc_start_wipe(Selected_wl_class);
		//start_weapon_animation(Selected_wl_class);
	}
	...
	if (ship_class >= 0)
	{
		...
		WeaponSelectScreenShipRot += PI2 * frametime / rev_rate;
		while (WeaponSelectScreenShipRot > PI2){
			WeaponSelectScreenShipRot -= PI2;	
		}
		...
		view_angles.b = 0.0f;
		view_angles.h = 0.0f;
		vm_angles_2_matrix(&WeaponScreenOrient, &view_angles);
	
		rot_angles.p = 0.0f;
		rot_angles.b = 0.0f;
		rot_angles.h = WeaponSelectScreenShipRot;
		vm_rotate_matrix_by_angles(&WeaponScreenOrient, &rot_angles);
	
		gr_set_clip(Ship_view_coords[gr_screen.res][0], Ship_view_coords[gr_screen.res][1], Ship_view_coords[gr_screen.res][2], Ship_view_coords[gr_screen.res][3]);
		// render the ship
		
		//strcpy(sip->pof_file, "missile.pof");
		//sip->closeup_pos.xyz.x = 0;
		//sip->closeup_pos.xyz.y = 0;
		//sip->closeup_pos.xyz.z = -50;
		g3_start_frame(1);
		...
		light_add_directional(&light_dir, 0.45f, 0.73f, 0.97f, 1.0f);
		// light_filter_reset();
		light_rotate_all();
		// lighting for techroom

		// Load the necessary model file
		WeaponSelectModelNum = model_load(sip->pof_file, sip->n_subsystems, &sip->subsystems[0]);
	
		// page in ship textures properly (takes care of nondimming pixels)
		model_page_in_textures(WeaponSelectModelNum, ship_class);
	
		model_clear_instance(WeaponSelectModelNum); //
		model_set_detail_level(0);
		model_render(WeaponSelectModelNum, &WeaponScreenOrient, &vmd_zero_vector, MR_LOCK_DETAIL | MR_AUTOCENTER);
...
void wl_draw_ship_weapons(int index)
{
	...
	// changed from 0 to 4 as we aren't displaying the guns on the craft
	// if we ever want to do that again change back to 0
	for ( i = 4; i < MAX_WL_WEAPONS; i++ ) {
...
void draw_wl_icons()
{
	...
	// not drawing the gun list anymore
	/*count=0;
...
//	int *weapon_ani_coords;
	...
	// start the text wipe
	wl_weapon_desc_start_wipe(Weapon_anim_class);
}
...
void weapon_select_maybe_show_button_text()
{
	...
	for (int i=0; i<NUM_WEAPON_BUTTONS; i++) {
		b = &Buttons[gr_screen.res][i];

		if ((b->button.button_hilighted()) && i == (WL_BUTTON_MULTI_LOCK)) {
			gr_set_color_fast(&Color_bright_white);
			gr_string(Weapon_select_text[gr_screen.res][1].x, Weapon_select_text[gr_screen.res][1].y, Weapon_select_text[gr_screen.res][1].xstr);
		}
	}
}
