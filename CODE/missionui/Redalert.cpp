 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 22:01:23  Darkhill
 * no message
 *
...
struct redalert_buttons {
	...
	redalert_buttons(char *name, int x1, int y1, char *text1, int x2, int y2, int h) : filename(name), x(x1), y(y1), text(text1), xt(x2), yt(y2), hotspot(h) {}
}; //ui_button_info

static redalert_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
	{	// GR_640
		redalert_buttons("RAB_00",	45,		375,	"Refly", 51,	397, 0),
		redalert_buttons("RAB_01",	540,	375,	"Continue", 535, 397, 1),
	},	
	{	// GR_1024
		redalert_buttons("2_RAB_00",	4,		712, "Refly", 51,	397, 0),
		redalert_buttons("2_RAB_01",	920,	691, "Continue", 535, 397, 1),
	}
};

#define RED_ALERT_NUM_TEXT		2
UI_XSTR Red_alert_text[GR_NUM_RESOLUTIONS][RED_ALERT_NUM_TEXT] = {
	{ // GR_640
		{ "Refly",		1405,	51, 397,	UI_XSTR_COLOR_GREEN,	-1, &Buttons[0][RA_REPLAY_MISSION].button },
		//{ "Previous Mission",	1452,	46,	462,	UI_XSTR_COLOR_PINK,	-1, &Buttons[0][RA_REPLAY_MISSION].button },
		{ "Continue",	1069,	535, 397,	UI_XSTR_COLOR_GREEN,	-1, &Buttons[0][RA_CONTINUE].button },
	},
	{ // GR_1024
		{ "Refly",		1405,	75,	722,	UI_XSTR_COLOR_PINK,	-1, &Buttons[1][RA_REPLAY_MISSION].button },
...
{
	char *str = "Incoming Transmission";
	int w, h;
	...
	// just need to center the 'incoming transmission' text so don't need the crap above
	gr_string(Ra_flash_coords[gr_screen.res][0], Ra_flash_coords[gr_screen.res][1], str);
...
//			gameseq_post_event(GS_EVENT_ENTER_GAME);
	...
	red_alert_force_button_frame();

	red_alert_maybe_show_button_text();
...
void red_alert_force_button_frame()
{
...
void red_alert_maybe_show_button_text()
{
	redalert_buttons *b;
	...
	for (int i=0; i<2; i++) {
		...
		if (b->button.button_hilighted()) {
			...
			gr_string(Red_alert_text[gr_screen.res][i].x, Red_alert_text[gr_screen.res][i].y, Red_alert_text[gr_screen.res][i].xstr);
		}
	}
}
