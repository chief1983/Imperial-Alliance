 ...
 * $Author: Spearhawk $
 *
 * $Log: multi_pinfo.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:20:46  Darkhill
 * no message
 *
...
ui_button_info Multi_pinfo_buttons[GR_NUM_RESOLUTIONS][MULTI_PINFO_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("PIB_00",	565,160,	-1,	-1,	0),
		ui_button_info("PIB_01",	565,315,	-1,	-1,	1),
		ui_button_info("PIB_02",	225,350,	-1,	-1,	2),
		ui_button_info("PIB_03",	280,350,	217,	318,	3)
	},
	{ // GR_1024
...
#define MULTI_PINFO_NUM_TEXT			2
UI_XSTR Multi_pinfo_text[GR_NUM_RESOLUTIONS][MULTI_PINFO_NUM_TEXT] = {
	{ // GR_640
		{ "Medals",		1037, 226,372,	UI_XSTR_COLOR_GREEN, -1,	&Multi_pinfo_buttons[0][MPI_MEDALS].button },		
		{ "Close",		428, 285,372,	UI_XSTR_COLOR_PINK, -1,	&Multi_pinfo_buttons[0][MPI_EXIT].button },		
	},
	{ // GR_1024
		{ "Medals",		1037,372,	318,	UI_XSTR_COLOR_GREEN, -1,	&Multi_pinfo_buttons[0][MPI_MEDALS].button },		
...
// stats labels
#define MULTI_PINFO_NUM_STATS_LABELS		10
#define MPI_RANK								0
#define MPI_FCHG								1
#define MPI_MISSIONS_FLOWN						2
#define MPI_FLIGHT_TIME							3
#define MPI_LAST_FLOWN							4
#define MPI_FIGHTER_KILLS						5
// #define MPI_OTHER_KILLS							5
#define MPI_PSHOTS_FIRED						6
//#define MPI_PSHOTS_HIT							6
 #define MPI_PSHOTS_PCT							7
#define MPI_SSHOTS_FIRED						8
// #define MPI_SSHOTS_HIT							10
#define MPI_SSHOTS_PCT							9
...
int Multi_pinfo_stats_label_offsets[MULTI_PINFO_NUM_STATS_LABELS] = {
	10,20,10,10,20,20,10,20,10,10,
};

// stats area defs
int Multi_pinfo_stats_area_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		227,140, 333, 210//215, 163, 414, 155
	},
	{ // GR_1024
		335, 261, 662, 248
	}
...
void multi_pinfo_popup_init(net_player *np)
{
	...
	Multi_pinfo_stats_labels[1] = strdup("FCHG");
	Multi_pinfo_stats_labels[2] = strdup(XSTR("Missions Flown", 1008));
	Multi_pinfo_stats_labels[3] = strdup(XSTR("Flight Time", 1009));
	Multi_pinfo_stats_labels[4] = strdup(XSTR("Last Flown",1010));
	Multi_pinfo_stats_labels[5] = strdup(XSTR("Total Kills", 115));
	Multi_pinfo_stats_labels[6] = strdup(XSTR("Primary Shots Fired", 1012));
	Multi_pinfo_stats_labels[7] = strdup(XSTR("Primary Hit %", 1013));
	Multi_pinfo_stats_labels[8] = strdup(XSTR("Secondary Shots Fired",	1014));
	Multi_pinfo_stats_labels[9] = strdup(XSTR("Secondary Hit %", 1015));				
}
...
void multi_pinfo_popup_do()
{
	...
	while(!Multi_pinfo_popup_done){		
		...
		// draw floating text if needed
		for (int i=2; i<=3; i++) 
		{
			b = &Multi_pinfo_buttons[gr_screen.res][i];
		
			if (b->button.button_hilighted())
			{
				gr_set_color_fast(&Color_bright_white);
				
				gr_string(Multi_pinfo_text[gr_screen.res][i-2].x,Multi_pinfo_text[gr_screen.res][i-2].y,
						Multi_pinfo_text[gr_screen.res][i-2].xstr);
			}
		}
...
void multi_pinfo_build_stats()
{
	...
	*/
	...
	strcpy(Multi_pinfo_stats_vals[MPI_FCHG], FCHG[sc->fchg].name);
