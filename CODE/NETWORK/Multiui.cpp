 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:28 $
 * $Author: Darkhill $
 *
 * C file for all the UI controls of the mulitiplayer screens
 *
 * $Log: Multiui.cpp,v $
 * Revision 1.2  2004/09/07 22:56:28  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:20:33  Darkhill
 * no message
 *
...
void multi_join_force_button_frame(int, int);
void multi_create_force_button_frame(int, int);
void multi_sg_force_button_frame(int, int);
void multi_ho_force_button_frame(int, int);
void multi_mj_force_button_frame(int, int);
void multi_sync_force_button_frame(int, int);
...
// text crap :)
int Multi_common_text_coords[GR_NUM_RESOLUTIONS][4][4] = {
	{ // GR_640
		// x,y, length, "more" y coord
		
		{75,327,291,91}, // multi join
		{75,350,291,67}, // multi create
		{75,350,291,67}, // multi client join
		{75,350,291,63}, // multi sync	
			//29, 396, 393, 76
	},
	{ // GR_1024
		{47, 634, 630, 122},
...
int Multi_common_text_max_display[GR_NUM_RESOLUTIONS][4] = {
	// join, create, client join, sync
	{9,6,6,6},		// GR_640
	{12,12,12,12},	// GR_1024
};
...
{
	Multi_common_top_text_line++;
	if ( (Multi_common_num_text_lines - Multi_common_top_text_line) < Multi_common_text_max_display[gr_screen.res][screen] ) {
		Multi_common_top_text_line--;
		if ( !mouse_down(MOUSE_LEFT_BUTTON) ){
			gamesnd_play_iface(SND_GENERAL_FAIL);
		}
...
void multi_common_move_to_bottom()
{
	...
	if(Multi_common_num_text_lines <= Multi_common_text_max_display[gr_screen.res][screen]){
		return;
	}
		
	Multi_common_top_text_line = Multi_common_num_text_lines - Multi_common_text_max_display[gr_screen.res][screen];
}
...
void multi_common_split_text()
{
	...
	//n_lines = split_str(Multi_common_all_text, Multi_common_text_coords[gr_screen.res][2][screen], n_chars, p_str, MULTI_COMMON_TEXT_MAX_LINES, MULTI_COMMON_TEXT_META_CHAR);

	n_lines = split_str(Multi_common_all_text, Multi_common_text_coords[gr_screen.res][0][2], n_chars, p_str, MULTI_COMMON_TEXT_MAX_LINES, MULTI_COMMON_TEXT_META_CHAR);
	Assert(n_lines != -1);
...
void multi_common_render_text()
{
	...
	for ( i = Multi_common_top_text_line; i < Multi_common_num_text_lines; i++ ) {
		if ( line_count >= Multi_common_text_max_display[gr_screen.res][screen] ){
			break;	
		}
		//gr_string(Multi_common_text_coords[gr_screen.res][0][screen], Multi_common_text_coords[gr_screen.res][1][screen] + (line_count*fh), Multi_common_text[i]);		

		gr_string(Multi_common_text_coords[gr_screen.res][screen][0], Multi_common_text_coords[gr_screen.res][screen][1] + (line_count*fh), Multi_common_text[i]);		
		line_count++;
	}

	if ( (Multi_common_num_text_lines - Multi_common_top_text_line) > Multi_common_text_max_display[gr_screen.res][screen]) {
		gr_set_color_fast(&Color_bright_red);
		//gr_string(Multi_common_text_coords[gr_screen.res][0][screen], (Multi_common_text_coords[gr_screen.res][1][screen] + Multi_common_text_coords[gr_screen.res][3][screen])-5, XSTR("more",755));

		//gr_string(Multi_common_text_coords[gr_screen.res][screen][0], (Multi_common_text_coords[gr_screen.res][screen][1] + Multi_common_text_coords[gr_screen.res][screen][3])-5, XSTR("more",755));
	}
}
...
ui_button_info Multi_join_buttons[GR_NUM_RESOLUTIONS][MULTI_JOIN_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info( "MJ_00",	50, 48,	-1,	-1,	0 ),						// scroll up
		ui_button_info( "MJ_02",	50,290,	-1,	-1,	1 ),						// scroll down
		ui_button_info( "MJ_03",	375,320,	65,	364,	2 ),						// refresh
		ui_button_info( "MJ_04",	50,325,	-1,	-1,	3 ),						// scroll info up
		ui_button_info( "MJ_05",	50,400,	-1,	-1,	4 ),						// scroll info down
		ui_button_info( "MJ_06",	430,320,	-1,	-1,	5 ),						// join as observer
		ui_button_info( "MJ_07",	485,320,	-1,	-1,	6 ),						// create game
		ui_button_info( "MJ_08",	540,320,	588,	376,	7),						// cancel
		ui_button_info( "MJ_09",	485,375,	-1,	-1,	8 ),						// help
		ui_button_info( "MJ_10",	430,375,	-1,	-1,	9 ),						// options
		ui_button_info( "MJ_11",	540,375,	589,	416,	10 ),						// join
	},
	{ // GR_1024
		ui_button_info( "2_MJ_00",	2,		92,	-1,	-1,	0 ),						// scroll up
		ui_button_info( "2_MJ_02",	2,		475,	-1,	-1,	1 ),						// scroll down
		ui_button_info( "2_MJ_03",	16,	541,	104,	582,	2 ),						// refresh
		ui_button_info( "2_MJ_04",	2,		648,	-1,	-1,	3 ),						// scroll info up
		ui_button_info( "2_MJ_05",	2,		713,	-1,	-1,	4 ),						// scroll info down
		ui_button_info( "2_MJ_06",	783,	542,	-1,	-1,	5 ),						// join as observer
		ui_button_info( "2_MJ_07",	861,	542,	-1,	-1,	6 ),						// create game
		ui_button_info( "2_MJ_08",	933,	542,	588,	376,	7 ),						// cancel
		ui_button_info( "2_MJ_09",	854,	681,	-1,	-1,	8 ),						// help
		ui_button_info( "2_MJ_10",	854,	727,	-1,	-1,	9 ),						// options
		ui_button_info( "2_MJ_11",	914,	681,	937,	668,	10 ),						// join
	}
};
...
UI_XSTR Multi_join_text[GR_NUM_RESOLUTIONS][MULTI_JOIN_NUM_TEXT] = {
	{ // GR_640		
		{"Refresh",			1299,	374,342,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_REFRESH].button},
		{"Join as",			1300,	431,337,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_JOIN_OBSERVER].button},
		{"Observer",		1301,	425,346,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_JOIN_OBSERVER].button},
		{"Create",			1408,	487,337,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_START_GAME].button},	
		{"Game",			1302,	493,346,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_START_GAME].button},
		{"Cancel",			387,	542,342,	UI_XSTR_COLOR_PINK, -1, &Multi_join_buttons[0][MJ_CANCEL].button},	
		{"Help",			928,	495,397,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_HELP].button},
		{"Options",			1036,	429,397,	UI_XSTR_COLOR_GREEN, -1, &Multi_join_buttons[0][MJ_OPTIONS].button},
		{"Join",			1303,	551,397,	UI_XSTR_COLOR_PINK, -1, &Multi_join_buttons[0][MJ_ACCEPT].button},
		{"Status",			1304,	80, 57,	UI_XSTR_COLOR_GREEN,	-1, NULL},
		{"Server",			1305,	160, 57,	UI_XSTR_COLOR_GREEN, -1, NULL},
		{"Players",			1306,	455, 57,	UI_XSTR_COLOR_GREEN,	-1, NULL},
		{"Ping",			1307,	524, 57,	UI_XSTR_COLOR_GREEN, -1, NULL}
	},
	{ // GR_1024		
...
void make_fake_games() {
	...
	for (idx=0; idx<50; idx++) {
		...
		sprintf(ag.title, "Mission %d", idx);
		sprintf(ag.mission_name, "Name %d", idx);
		ping.ping_avg = 10;
		ag.ping = ping;
		ag.num_players = 2;
		...
		ag.flags = (AG_FLAG_DOGFIGHT | AG_FLAG_FORMING | AG_FLAG_STANDALONE);		
...
void fs2netd_maybe_init()
{
		if (PXO_port == -1)
		{
			...
			if(file == NULL){
				ml_printf("Network","Error loading MP.cfg file!\n");
				return;
			}
...
void multi_join_game_init()
{
	...
	// create all xstrs
	for(idx=9; idx<MULTI_JOIN_NUM_TEXT; idx++){
	...
	// does what says, useful if you want a bunch of games to appear in the server list
	//make_fake_games();
}
...
void multi_join_game_do_frame()
{
	...
	if ( !psnet_rel_check() ) {
		popup(PF_USE_AFFIRMATIVE_ICON,1,POPUP_OK,XSTR("Network Error.  Try exiting and restarting Imperial Alliance to clear the error.  Otherwise, please reboot your machine.",756));
	...
	// get the refresh button, since it is not with the rest
	multi_join_force_button_frame(2, 1);
	
	for (int u=5; u<10; u++)
		multi_join_force_button_frame(u, 1);
	...
	for (int i=2; i<=10; i++) 
	{
		...
		b = &Multi_join_buttons[gr_screen.res][i];
		...
		if (b->button.button_hilighted())
		{
			...
			{
			case 5:
				gr_string(Multi_join_text[gr_screen.res][i-4].x,Multi_join_text[gr_screen.res][i-4].y,
					  Multi_join_text[gr_screen.res][i-4].xstr);
				gr_string(Multi_join_text[gr_screen.res][i-3].x,Multi_join_text[gr_screen.res][i-3].y,
					  Multi_join_text[gr_screen.res][i-3].xstr);
			...
			case 6:
				gr_string(Multi_join_text[gr_screen.res][i-3].x,Multi_join_text[gr_screen.res][i-3].y,
					  Multi_join_text[gr_screen.res][i-3].xstr);
				gr_string(Multi_join_text[gr_screen.res][i-2].x,Multi_join_text[gr_screen.res][i-2].y,
					  Multi_join_text[gr_screen.res][i-2].xstr);
...
ui_button_info Multi_sg_buttons[GR_NUM_RESOLUTIONS][MULTI_SG_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("MSG_00",	50,150,	34,	191,	2),		// open
//******ui_button_info("MSG_01",	1,		159,	34,	166,	1),		// closed
//******ui_button_info("MSG_02",	1,		184,	34,	191,	2),		// restricted
		ui_button_info("MSG_03",	50,175,	34,	218,	3),		// password
		ui_button_info("MSG_04",	50,255,	34,	266,	4),		// rank set
		ui_button_info("MSG_05",	55,285,	-1,	-1,	5),		// rank scroll up
		ui_button_info("MSG_06",	55,310,	-1,	-1,	6),		// rank scroll down
		ui_button_info("MSG_07",	260,285,	210,	290,	7),		// rank above
		ui_button_info("MSG_08",	260,310,	210,	315,	8),		// rank below
		ui_button_info("MSG_09",	485,375,	500,	440,	9),		// help
		ui_button_info("MSG_10",	430,375,	479,	464,	10),		// options
		ui_button_info("MSG_11",	540,375,	571,	415,	11),		// accept
	},
	{ // GR_1024
...
#define MULTI_SG_NUM_TEXT			9
UI_XSTR Multi_sg_text[GR_NUM_RESOLUTIONS][MULTI_SG_NUM_TEXT] = {
	{ // GR_640
		{"Open",				1322,		80,157,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_OPEN_GAME].button},
...
//******{"Restricted",			1324,		34,	191,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_RESTRICTED_GAME].button},
		{"Password Protected",	1325,		80,182,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_PASSWD_GAME].button},
		{"Allow Rank",			1326,		80,262,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_RANK_SET_GAME].button},
		{"Above",				1327,		290,292,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_RANK_ABOVE].button},
		{"Below",				1328,		290,317,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_RANK_BELOW].button},
		{"Help",				928,		495,397,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_HELP].button},
		{"Options",				1036,		429,397,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_sg_buttons[0][MSG_OPTIONS].button},
		{"Accept",				1035,		541,397,	UI_XSTR_COLOR_PINK,	-1,	&Multi_sg_buttons[0][MSG_ACCEPT].button},
		//{"Start Game",			1329,		60, 45,	UI_XSTR_COLOR_GREEN,	-1,	NULL},
		{"Title",				1330,		60, 66,	UI_XSTR_COLOR_GREEN,	-1,	NULL},
...
void multi_start_game_do()
{
	...
	for (int u=7; u<=9; u++)
		multi_sg_force_button_frame(u, 1);

	// draw text if mouse is over button
	for (int i=7; i<=9; i++) 
	{
		b = &Multi_sg_buttons[gr_screen.res][i];
		
		if (b->button.button_hilighted())
		{
			gr_set_color_fast(&Color_bright_white);
			
			gr_string(Multi_sg_text[gr_screen.res][i-2].x,Multi_sg_text[gr_screen.res][i-2].y,
					  Multi_sg_text[gr_screen.res][i-2].xstr);
		}
	}
...
void multi_sg_init_gamenet()
{
	...
	Multi_sg_netgame->mode = NG_MODE_OPEN;
	Multi_sg_netgame->rank_base = RANK_CT;
...
ui_button_info Multi_create_buttons[GR_NUM_RESOLUTIONS][MULTI_CREATE_NUM_BUTTONS] = {	
	{ // GR_640
		ui_button_info("MC_00", 50, 55,	36,	158,	0),		// show all missions
		ui_button_info("MC_01", 50, 80,	71,	158,	1),		// show coop missions
		ui_button_info("MC_02", 50,105,	119,	158,	2),		// show team missions
		ui_button_info("MC_03", 50,130,	166,	158,	3),		// show dogfight missions
		ui_button_info("MC_04", 430,100,	229,	130,	4),		// pxo mission refresh
		ui_button_info("MC_05", 540,100,	467,	132,	5),		// pilot info
		ui_button_info("MC_06", 50,160,	-1,	-1,	6),		// scroll mission info up
		ui_button_info("MC_08", 50,315,	-1,	-1,	8),		// scroll mission info down
		ui_button_info("MC_09", 565,160,	-1,	-1,	9),		// scroll players up
		ui_button_info("MC_10", 565,315,	-1,	-1,	10),		// scroll players down
		ui_button_info("MC_11", 155,130,	27,	376,	11),		// mission filter
		ui_button_info("MC_12", 260,130,	110,	376,	12),		// campaign filter
		ui_button_info("MC_13", 485,100,	328,	364,	13),		// cancel
		ui_button_info("MC_14", 375,345,	482,	381,	14),		// team 0	
		ui_button_info("MC_15", 400,345,	514,	381,	15),		// team 1
		ui_button_info("MC_16", 485,345,	539,	381,	16),		// kick
		ui_button_info("MC_17", 540,345,	582,	381,	17),		// close
		ui_button_info("MC_18", 50,350,	-1,	-1,	18),		// scroll list up
		ui_button_info("MC_19", 50,400,	-1,	-1,	19),		// scroll list down
		ui_button_info("MC_20", 375,375,	436,	423,	20),		// host options
		ui_button_info("MC_21", 485,375,	-1,	-1,	21),		// help
		ui_button_info("MC_22", 430,375,	-1,	-1,	22),		// options
		ui_button_info("MC_23", 540,375,	572,	413,	23),		// commit
	}, 	
	{ // GR_1024
...
UI_XSTR Multi_create_text[GR_NUM_RESOLUTIONS][MULTI_CREATE_NUM_BUTTONS] = {
	{ // GR_640
		{"All",					1256,		80, 62,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_SHOW_ALL].button},
		{"Coop",				1257,		80, 87,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_SHOW_COOP].button},
		{"Team",				1258,		80,112,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_SHOW_TEAM].button},
		{"Dogfight",			1259,		80,137,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_SHOW_DOGFIGHT].button},
		{"Refresh Missions",	1260,		229,	130,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_PXO_REFRESH].button},
		{"Pilot Info",			1261,		548,115,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_PILOT_INFO].button},	
		{"Missions",			1262,		185,137,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_MISSION_FILTER].button},
		{"Campaigns",			1263,		290,137,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_CAMPAIGN_FILTER].button},
		{"Cancel",				387,		487,122,	UI_XSTR_COLOR_PINK,	-1,	&Multi_create_buttons[0][MC_CANCEL].button},
		{"1",					1264,		384,349,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_TEAM0].button},
		{"2",					1265,		408,349,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_TEAM1].button},
		{"Kick",				1266,		509,352,	UI_XSTR_COLOR_PINK,	-1,	&Multi_create_buttons[0][MC_KICK].button},
		{"Close",				1508,		564,352,	UI_XSTR_COLOR_PINK,	-1,	&Multi_create_buttons[0][MC_CLOSE].button},	
		{"Host Options",		1267,		383,390,	UI_XSTR_COLOR_GREEN,	-1,	&Multi_create_buttons[0][MC_HOST_OPTIONS].button},		
		{"Commit",				1062,		541,397,	UI_XSTR_COLOR_PINK,	-1,	&Multi_create_buttons[0][MC_ACCEPT].button}
	},		
	{ // GR_1024		
...
int Mc_players_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		415,173,146,151//463, 164, 144, 180
	},
	{ // GR_1024
		741, 262, 144, 180
	}
...
int Mc_slider_coords[GR_NUM_RESOLUTIONS][4] = {
	{
		50,179,19,138//3, 197, 13, 105	// GR_640
	},
	{
...
void multi_create_game_init()
{
	...
	// create xstrs MULTI_CREATE_NUM_TEXT
	for(idx=0; idx<=4; idx++){
...
void multi_create_game_do()
{
	...
	multi_create_force_button_frame(5, 1);
	multi_create_force_button_frame(12, 1);

	for (int i=19; i<=22; i++)
		multi_create_force_button_frame(i, 1);

	for (i=5; i<=22; i++) 
	{
			
		b = &Multi_create_buttons[gr_screen.res][i];
		...
		if (b->button.button_hilighted())
		{
			...
			{
			case MC_PILOT_INFO:
				gr_string(548, 115, "Pilot");
				gr_string(552, 128, "Info");

				break;
			
			case MC_CANCEL:
				gr_string(487, 122, "Cancel");
				
				break;

			case MC_HOST_OPTIONS:
				gr_string(383,390, "Host");
				gr_string(374,403, "Options");

				break;

			case MC_HELP:
				gr_string(495,397, "Help");

				break;

			case MC_OPTIONS:
				gr_string(429,397, "Options");
	...
	for(idx = 0; idx < file_count; idx++){
		...
		if ( flags ) {
			...
			if ( Multi_create_mission_count < MULTI_CREATE_MAX_LIST_ITEMS ) {
				...
				if (!strstr(mcip->filename, ".iam"))
					v_string = v_string + ".iam";
...
ui_button_info Multi_ho_buttons[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_BUTTONS] = {	
	{ // GR_640
		...
		ui_button_info("MH_00",	50,155,	46,	166,	0),		// highest rank
		ui_button_info("MH_01",	50,180,	46,	185,	1),		// team/wing leader
		ui_button_info("MH_02",	50,205,	46,	203,	2),		// any
		ui_button_info("MH_03",	50,230,	46,	220,	3),		// host
		
		// who is allowed to end the mission
		ui_button_info("MH_04",	280,155,	46,	265,	4),		// highest rank
		ui_button_info("MH_05",	280,180,	46,	283,	5),		// team/wing leader
		ui_button_info("MH_06",	280,205,	46,	300,	6),		// any
		ui_button_info("MH_07",	280,230,	46,	317,	7),		// host		

		// voice on/off button
		ui_button_info("MH_09",	515,155,	545,	185,	9),	
		ui_button_info("MH_10",	570,155,	604,	185,	10),	

		// host modifies ships
		ui_button_info("MH_13",	50,260,	437,	363,	13),	

		// exit
		ui_button_info("MH_14",	540,375,	580,	414,	14),	
	},
	{ // GR_1024
		// who is allowed to message
...
UI_XSTR Multi_ho_text[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_BUTTONS] = {
	{ // GR_640
		{"Highest rank",				1280,	129,162,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_MSG_RANK].button},
		{"Team / FG-leader",			-1,		104,187,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_MSG_LEADER].button},
		{"Any",							1282,	162,212,	UI_XSTR_COLOR_GREEN,	-1, &Multi_ho_buttons[0][MULTI_HO_MSG_ANY].button},
		{"Host",						1283,	157,237,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_MSG_HOST].button},
		{"Highest rank",				1280,	46,		265,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_END_RANK].button},
		{"Team / FG-leader",			-1,		46,		283,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_END_LEADER].button},
		...
		{"On",							1285,	517,180,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_VOICE_ON].button},
		{"Off",							1286,	570,180,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_VOICE_OFF].button},
		{"Host modifies ships",			1287,	80,267,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[0][MULTI_HO_HOST_MODIFIES].button},
		{"Accept",						1483,	541,397,	UI_XSTR_COLOR_PINK,	-1, &Multi_ho_buttons[0][MULTI_HO_ACCEPT].button},
	},
	{ // GR_1024
		{"Highest rank",					1280,	62,	269,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[1][MULTI_HO_MSG_RANK].button},
		{"Team / FG-leader",			-1, 62,	297,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[1][MULTI_HO_MSG_LEADER].button},
		...
		{"Team / FG-leader",			-1,	62,	452,	UI_XSTR_COLOR_GREEN, -1, &Multi_ho_buttons[1][MULTI_HO_END_LEADER].button},
...
ho_sliders Multi_ho_sliders[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_SLIDERS] = {
	{ // GR_640
		ho_sliders("MH_11",	394,233,	437,	199,	11,	19,	10),			// voice qos
		ho_sliders("MH_12",	394,275,	437,	246,	12,	19,	10),			// voice duration
		ho_sliders("MH_08",	220,403,	230,	411,	8,		36,	5),			// skill level
	},
	{ // GR_1024		
...
UI_XSTR Multi_ho_titles[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_TITLES] = {
	{ // GR_640
		{ "AI Orders",			1289,		45,140, UI_XSTR_COLOR_GREEN, -1, NULL },		
		{ "End Mission",		1290,		225,140, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Time Limit",			1291,		60,300, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Min",				1292,		103,315, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Respawn Limit",		1288,		60,331, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Kill Limit",			1293,		60,362, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Observers",			1294,		60,394, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Skill Level",		1284,		215,368, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Voice Transmission",	1295,		373,162, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Voice Quality",		1296,		390,221, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Message Duration",	1297,		390,263, UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "sec",				1522,		456,347, UI_XSTR_COLOR_GREEN, -1, NULL },		
		{ "sec",				1523,		456,307, UI_XSTR_COLOR_GREEN, -1, NULL },		
		{ "Voice Wait",			1298,		390,328, UI_XSTR_COLOR_GREEN, -1, NULL },
	},
	{ // GR_1024		
...
void multi_host_options_init()
{
	...
	for(idx=0;idx<MULTI_HO_NUM_BUTTONS;idx++){
		...
		// add xstr text
		// not all of the text needs to be displayed
		if (idx == 4 || idx == 5 || idx == 6 || idx == 7 || idx == 11)
			continue;
...
void multi_host_options_do()
{
	...
	// force mouse over to be drawn on exit button
	multi_ho_force_button_frame(11, 1);

	
	// draw floating text for exit button
	b = &Multi_ho_buttons[gr_screen.res][MULTI_HO_ACCEPT];
		
	if (b->button.button_hilighted())
	{
		gr_set_color_fast(&Color_bright_white);
			
		gr_string(Multi_ho_text[gr_screen.res][MULTI_HO_ACCEPT].x,Multi_ho_text[gr_screen.res][MULTI_HO_ACCEPT].y,
			  Multi_ho_text[gr_screen.res][MULTI_HO_ACCEPT].xstr);
	}
...
ui_button_info Multi_jw_buttons[GR_NUM_RESOLUTIONS][MULTI_JW_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("MJW_00",	565, 160,	-1,		-1,		0),
		ui_button_info("MJW_01",	565, 315,	-1,		-1,		1),
		ui_button_info("MJW_02",	410, 345,	20,	    272,	2),
		ui_button_info("MJW_03",	435, 345,	73,	    272,	3),
		ui_button_info("MJW_04",	410, 375,	134,	272,	4),
		ui_button_info("MJW_05",	50,  350,	-1,	    -1,		5),
		ui_button_info("MJW_06",	50,  400,	-1,	    -1,		6),
		ui_button_info("MJW_07",	540, 375,	570,	414,	7),
	},
	{ // GR_1024
...
UI_XSTR Multi_jw_text[GR_NUM_RESOLUTIONS][MULTI_JW_NUM_TEXT] = {
	{ // GR_640
		...
		{ "Pilot",				1310,		418,390,	UI_XSTR_COLOR_GREEN, -1, &Multi_jw_buttons[0][MJW_PILOT_INFO].button },
		{ "Info",				1311,		422,403,	UI_XSTR_COLOR_GREEN, -1, &Multi_jw_buttons[0][MJW_PILOT_INFO].button },
		{ "Cancel",				387,		541,397,	UI_XSTR_COLOR_PINK, -1, &Multi_jw_buttons[0][MJW_CANCEL].button },
		{ "Players",			1269,		420,160,		UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Team",		1312,		460,352,	UI_XSTR_COLOR_GREEN, -1, NULL },
	},
	{ // GR_1024
		...
		{ "Team",		1312,		45,	373,	UI_XSTR_COLOR_GREEN, -1, NULL },
	}
};

int Mjw_players_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		424,173,146,159//153,210//29, 18, 153, 210
	},
	{ // GR_1024
		46, 29, 254, 336
	}
...
void multi_game_client_setup_init()
{
	...
	chatbox_close();
	chatbox_create(CHATBOX_FLAG_SMALL | CHATBOX_FLAG_DRAW_BOX | CHATBOX_FLAG_BUTTONS);
	...
	// create all xstrs
	for(idx=3; idx<MULTI_JW_NUM_TEXT; idx++){
...
void multi_game_client_setup_do_frame()
{
	...
	// give buttons mouse over graphic
	multi_mj_force_button_frame(MJW_PILOT_INFO, 1);
	multi_mj_force_button_frame(MJW_CANCEL, 1);
		
	// show text if needed
	b = &Multi_jw_buttons[gr_screen.res][MJW_PILOT_INFO];
		
	if (b->button.button_hilighted())
	{
		gr_set_color_fast(&Color_bright_white);
		
		gr_string(Multi_jw_text[gr_screen.res][0].x,Multi_jw_text[gr_screen.res][0].y,
				  Multi_jw_text[gr_screen.res][0].xstr);
		gr_string(Multi_jw_text[gr_screen.res][1].x,Multi_jw_text[gr_screen.res][1].y,
				  Multi_jw_text[gr_screen.res][1].xstr);
	}

	b = &Multi_jw_buttons[gr_screen.res][MJW_CANCEL];
		
	if (b->button.button_hilighted())
	{
		gr_set_color_fast(&Color_bright_white);
		
		gr_string(Multi_jw_text[gr_screen.res][2].x,Multi_jw_text[gr_screen.res][2].y,
				  Multi_jw_text[gr_screen.res][2].xstr);
	}
...
ui_button_info Multi_sync_buttons[GR_NUM_RESOLUTIONS][MULTI_SYNC_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("MS_00",		50,350,	-1,	-1,	0),
		ui_button_info("MS_01",		50,400,	-1,	-1,	1),
		ui_button_info("MS_03",		485,375,	519,	416,	3),
		ui_button_info("MS_02",		430,375,	479,	416,	2),	
		ui_button_info("MS_04",		540,375,	577,	416,	4),
	},
	{ // GR_1024
...
UI_XSTR Multi_sync_text[GR_NUM_RESOLUTIONS][MULTI_SYNC_NUM_TEXT] = {
	{ // GR_640
		{ "Cancel",		387,		487,397,	UI_XSTR_COLOR_PINK,	-1, &Multi_sync_buttons[0][MS_KICK].button },
		{ "Kick",		1266,		442,397,	UI_XSTR_COLOR_PINK,	-1, &Multi_sync_buttons[0][MS_CANCEL].button },
		{ "Launch",		801,		542,397,	UI_XSTR_COLOR_PINK,	-1, &Multi_sync_buttons[0][MS_LAUNCH].button },
		{ "Players",	1269,		55,140,	UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "Status",		1304,		260,140,	UI_XSTR_COLOR_GREEN,	-1, NULL }
	},
	{ // GR_1024
...
int Ms_status2_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		260,157, 391, 220//228, 150, 391, 220
	},
	{ // GR_1024
		370, 228, 626, 367
	}
...
void multi_sync_common_init()
{
	...
	// add xstrs
	for(idx=3; idx<MULTI_SYNC_NUM_TEXT; idx++) {
...
void multi_sync_blit_screen_all()
{
	...
	// make sure our buttons are displayed
	for (int u=2; u<=4; u++)
		multi_sync_force_button_frame(u, 1);

	// show text if needed
	for (int i=2; i<=4; i++) 
	{
		if(!MULTIPLAYER_MASTER && !MULTIPLAYER_HOST && (i == MST_KICK)) 
			continue;
		
		b = &Multi_sync_buttons[gr_screen.res][i];
		
		if (b->button.button_hilighted())
		{
			gr_set_color_fast(&Color_bright_white);
			
			gr_string(Multi_sync_text[gr_screen.res][i-2].x,Multi_sync_text[gr_screen.res][i-2].y,
				  Multi_sync_text[gr_screen.res][i-2].xstr);
			...
			if(Net_player->flags & NETINFO_FLAG_GAME_HOST){
				...
				for(idx=0;idx<MAX_PLAYERS;idx++){
					if(MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) && !MULTI_PERM_OBSERVER(Net_players[idx]) && (Net_players[idx].player->objnum != -1)){
						...
						multi_server_update_player_sfoils(&Net_players[idx],&Ships[Objects[Net_players[idx].player->objnum].instance]);
					}
				}			
			}			
...
ui_button_info Multi_pwd_buttons[GR_NUM_RESOLUTIONS][MULTI_PWD_NUM_BUTTONS] = {
	{ // GR_640
		...
		ui_button_info("PWB_01",	466,	151,	465,	141,	1),
	}, 
	{ // GR_1024
...
// ghastly functions to make button on the various MP screens to show
// mouse over graphic all the time
void multi_sync_force_button_frame(int n, int frame_num)
{
	if ( !Multi_sync_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_sync_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}

void multi_join_force_button_frame(int n, int frame_num)
{
	if ( !Multi_join_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_join_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}

void multi_create_force_button_frame(int n, int frame_num)
{
	if ( !Multi_create_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_create_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}

void multi_sg_force_button_frame(int n, int frame_num)
{
	if ( !Multi_sg_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_sg_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}

void multi_ho_force_button_frame(int n, int frame_num)
{
	if ( !Multi_ho_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_ho_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}

void multi_mj_force_button_frame(int n, int frame_num)
{
	if ( !Multi_jw_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_jw_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}
