 ...
 * $Author: Spearhawk $
 *
 * $Log: multi_endgame.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:21:55  Darkhill
 * no message
 *
	...
	if(popup_active()){
		...
		if(notify_code != MULTI_END_NOTIFY_NONE){
			...
			case MULTI_END_ERROR_WAVE_COUNT:
				strcat(err_msg,"The player flight groups must have only 1 wave.  One of these flight groups currently has more than 1 wave.");
				break;
			case MULTI_END_ERROR_TEAM0_EMPTY:
				strcat(err_msg,XSTR("All players from team 1 have left the game", 1479));
				break;
			case MULTI_END_ERROR_TEAM1_EMPTY:
				strcat(err_msg,XSTR("All players from team 2 have left the game", 1480));
				break;
