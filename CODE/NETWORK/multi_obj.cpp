...
{	
	...
	if ( oo_flags & OO_HULL_NEW ){
		...
		// add system strength percent
		temp = (objp->system_strength / shipp->ship_initial_system_strength);
		PACK_PERCENT(temp);
		multi_rate_add(NET_PLAYER_NUM(pl), "sys", 1);
	...
	if( oo_flags & OO_SUBSYSTEMS_AND_AI_NEW ){
		...
		// pod weapon energy
		temp = shipp->pod_energy / shipp->pod_energy_capacity;
		PACK_PERCENT(temp);
	}		
...
{	
	...
	if ( oo_flags & OO_HULL_NEW ){
		...
		UNPACK_PERCENT(fpct);
		pobjp->system_strength = fpct * Ships[pobjp->instance].ship_initial_system_strength;
	...
	if ( oo_flags & OO_SUBSYSTEMS_AND_AI_NEW ) {
		...
		//pod energy
		float pod_energy_pct;
		UNPACK_PERCENT(pod_energy_pct);
		shipp->pod_energy = shipp->pod_energy_capacity * pod_energy_pct;
	}	
