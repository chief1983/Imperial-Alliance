 ...
 * $Author: Spearhawk $
 *
 * $Log: multi_ingame.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:42  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:22:12  Darkhill
 * no message
 *
...
{
	int offset, sflags, sflags2, oflags, team, iff, pod_weapon, j;
	ubyte p_type;
	ushort net_signature;	
	short wing_data;	
	...
	while ( p_type == INGAME_SHIP_NEXT ) {
		...
		Ships[ship_num].iff = iff;
		Ships[ship_num].wingnum = (int)wing_data;
		Ships[ship_num].pod_weapon = pod_weapon;
...
void send_ingame_ship_request_packet(int code,int rdata,net_player *pl)
{
	...
	case INGAME_SR_CONFIRM:
		...
		// add what p.o.d. if any is on ship
		val = (ubyte)shipp->pod_weapon;
		ADD_DATA(val);
		break;
	}
...
void process_ingame_ship_update_packet(ubyte *data, header *hinfo)
{
	...
	GET_DATA(lookup->system_strength);
