 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:22:19  Darkhill
 * no message
 *
...
void multi_log_write_header()
{
	...
	// header message
	timer = time(NULL);	
	strftime(str, 1024, "Imperial Alliance Multi Log - Opened %a, %b %d, %Y  at %I:%M%p\n----\n----\n----\n\n", localtime(&timer));
	ml_string(str, 0);	
}
...
void multi_log_write_trailer()
{
	...
	// header message
	timer = time(NULL);
	strftime(str, 1024, "\n\n----\n----\n----\nImperial Alliance Multi Log - Closing on %a, %b %d, %Y  at %I:%M%p", localtime(&timer));
	ml_string(str, 0);	
}
