 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:36 $
 * $Author: Darkhill $
 * 
 * $Log: multi_dogfight.cpp,v $
 * Revision 1.2  2004/09/07 22:56:36  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:21:46  Darkhill
 * no message
 *
...
void mdogfight_force_button_frame(int, int);
...
ui_button_info Multi_df_buttons[GR_NUM_RESOLUTIONS][NUM_MULTI_DF_BUTTONS] = {
	{ // GR_640
		// accept
		ui_button_info("KM_00",	540,375,	578,	413,	5),
	}, 	
	{ // GR_1024
		// accept
		ui_button_info("KM_00",	914,	681,	914,	660,	5),
	}
};
...
void multi_df_debrief_do()
{
	...
	// draw mouse over graphic
	mdogfight_force_button_frame(0,1);
	
	b = &Multi_df_buttons[gr_screen.res][ACCEPT_BUTTON];
...
void mdogfight_force_button_frame(int n, int frame_num)
{
	if ( !Multi_df_buttons[gr_screen.res][n].button.button_down() ) {
		Multi_df_buttons[gr_screen.res][n].button.draw_forced(frame_num);
	}
}