 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:25:40  Darkhill
 * no message
 *
...
// send to clients this ship was disabled
void send_ship_disabled_packet(ship *shipp)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size;
	
	// build up the header portion
	BUILD_HEADER(SHIP_DISABLED);
...
// process disabling a ship
void process_ship_disabled_packet(ubyte *data, header *hinfo)
{
	int offset;
	ushort signature;
	object *objp;
	...
	objp = multi_get_network_object( signature );
	if ( objp == NULL ) {
		nprintf(("Network", "Could not find object with net signature %d for disabling\n", signature ));
		return;
	}
	...
	// disable the sucker
	Ships[objp->instance].flags2 |= SF2_DISABLED;

	// server should rebroadcast
	if(MULTIPLAYER_MASTER){
		send_ship_disabled_packet(&Ships[objp->instance]);
	}
}

// send s-foil update
void send_ship_sfoil_update_packet(ship *shipp, int status)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size;
	
	// build up the header portion
	BUILD_HEADER(SFOIL_UPDATE);
...
void process_ship_sfoil_update_packet(ubyte *data, header *hinfo)
{
	int offset, status;
	ushort signature;
	object *objp;
	...
	objp = multi_get_network_object( signature );
	if ( objp == NULL ) {
		nprintf(("Network", "Could not find object with net signature %d for s-foil updating\n", signature ));
		return;
	}
	...
	// just to be safe remove all the flags
	...
	Ships[objp->instance].sfoil_status |= 1 << status;

	// server should rebroadcast
	//if(MULTIPLAYER_MASTER){
	//	process_ship_sfoil_update_packet(&Ships[objp->instance], status);
	//}
}

void send_ship_barrel_link_packet(ship *shipp, int mode, int curr_bank)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size;

	BUILD_HEADER(LINK_UPDATE);

	ADD_DATA(Objects[shipp->objnum].net_signature);
	ADD_DATA(shipp->primary_link_status[curr_bank]);
	ADD_DATA(curr_bank);

	//HUD_printf("Sending link update packet");
...
void process_ship_barrel_link_packet(ubyte *data, header *hinfo)
{
	int offset, link_status, curr_bank, lookup;
	ushort signature;
	object *objp;
	...
	objp = multi_get_network_object( signature );
	if ( objp == NULL ) {
		nprintf(("Network", "Could not find object with net signature %d for link updating\n", signature ));
		HUD_printf("Could not find object with net signature %d for link updating\n", signature );
		return;
	}
	...
	Ships[objp->instance].primary_link_status[curr_bank] = link_status;
	//Ships[objp->instance].last_fired_point[Ships[objp->instance].weapons.current_primary_bank] = 0;
	Ships[objp->instance].last_fired_point[curr_bank] = 0;
	//Net_players[lookup].s_info.link_mode[curr_bank] = link_status;
	
}

// send pod update
void send_ship_pod_update_packet(ship *shipp, int status, int flag)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size;
		
	// build up the header portion
	BUILD_HEADER(POD_UPDATE);
...
void process_ship_pod_update_packet(ubyte *data, header *hinfo)
{
	int offset, status;
	ushort signature;
	object *objp;
	...
	objp = multi_get_network_object( signature );
	if ( objp == NULL ) {
		nprintf(("Network", "Could not find object with net signature %d for pod updating\n", signature ));
		return;
	}
	...
	if (flag == SF2_CLOAK_ON) {
		ship_fire_tertiary(objp);
		return;
	}
	
	if (status)
		Ships[objp->instance].flags2 |= flag;
	else
		Ships[objp->instance].flags2 &= ~(flag);
	//ship_fire_tertiary(objp);
}
...
void send_firing_info_packet()
{
	...
	link_status = Player_ship->primary_link_status[0];
	ADD_DATA( link_status );
...
void process_firing_info_packet( ubyte *data, header *hinfo )
{
	...
	shipp->primary_link_status[0] = link_status;
}
...
void process_deny_packet(ubyte *data, header *hinfo)
{
	...
	case JOIN_DENY_JR_BAD_VERSION :
		popup(PF_USE_AFFIRMATIVE_ICON,1,POPUP_OK,XSTR("You cannot join this game because you are running an older version of Imperial Alliance than the server.  Exit FreeSpace, and choose the 'Update FreeSpace' button in the FreeSpace launcher to download the latest version of FreeSpace.",734));
		break;	
...
void send_post_sync_data_packet(net_player *p, int std_request)
{
	...
	for ( so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so) ) {
		...
		// get link mode on all primary weapons
		for (int x=0; x<MAX_SHIP_PRIMARY_BANKS; x++) {
			val = shipp->primary_link_status[x];
			ADD_DATA(val);
		}
		
		val = bitmask_2_bitnum(shipp->sfoil_status);
		ADD_DATA(val);
...
void process_post_sync_data_packet(ubyte *data, header *hinfo)
{
	...
	for(idx=0; idx<ship_count; idx++){	
		...
		for (int x=0; x<MAX_SHIP_PRIMARY_BANKS; x++) {
			GET_DATA(val);
			shipp->primary_link_status[x] = (int)val;
		}

		GET_DATA(val);
		shipp->sfoil_status = (int)(1<<val);
...
void send_client_update_packet(net_player *pl)
{
	...
	if ( val & UPDATE_HULL_INFO ) {
		...
		temp = (objp->system_strength / shipp->ship_initial_system_strength * 100.0f);
		if (temp < 0.0f) {
			percent = 0;
		} else {
			percent = (ubyte)temp;
...
void process_client_update_packet(ubyte *data, header *hinfo)
{
	...
	int ammo_count;
	int ammo[10];
	float weapon_energy, pod_energy;
	int offset = HEADER_LENGTH;
	...
	if ( have_hull_info ) {
		...
		ship_info *sip;
		ship *shipp;
		ubyte hull_percent, system_percent, shield_percent[MAX_SHIELD_SECTIONS], n_subsystems, subsystem_percent[MAX_MODEL_SUBSYSTEMS], threats;
		ubyte ub_tmp;
		ship_subsys *subsysp;
		object *objp;
		int i;
		...
		if ( !(Net_player->flags & NETINFO_FLAG_INGAME_JOIN) && (Player_ship != NULL) && (Player_obj != NULL) && (Net_player != NULL)) {			
			...
			val = system_percent * shipp->ship_initial_system_strength / 100.0f;
			objp->system_strength = val;
...
void send_change_team_packet(ushort net_signature, int new_team)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size = 0;
...
void process_change_team_packet( ubyte *data, header *hinfo)
{
	int offset = HEADER_LENGTH;
...
void send_change_iff_packet(ushort net_signature, int new_iff)
{
	ubyte data[MAX_PACKET_SIZE];
	int packet_size = 0;
...
void process_change_iff_packet( ubyte *data, header *hinfo)
{
	...
	objp = multi_get_network_object(net_signature);
	if((objp != NULL) && (objp->type == OBJ_SHIP) && (objp->instance >=0)){
		Ships[objp->instance].iff = new_iff;
	}	
}
