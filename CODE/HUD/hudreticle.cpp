 ...
 *
 * $Log: hudreticle.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:49:43  Darkhill
 * no message
 *
...
#define NUM_RETICLE_ANIS			4		// keep up to date when modifying the number of reticle ani files

//#define RETICLE_TOP_ARC				0
#define RETICLE_LASER_WARN			0
#define RETICLE_LOCK_WARN			1
#define RETICLE_BIG_WARN			2
//#define RETICLE_LEFT_ARC			3
//#define RETICLE_RIGHT_ARC			3
...
#define THREAT_DUMBFIRE_BIG			(1<<3)

#define THREAT_UPDATE_DUMBFIRE_TIME		1000		// time between checking for dumbfire threats
#define THREAT_UPDATE_DUMBFIRE_BIG_TIME 500			// time between checking for big dumbfire threats
...
#define THREAT_DUMBFIRE_BIG_FLASH			180
...
static int Threat_dumbfire_big_timer;	// timestamp to show next frame for big dumbfire threat
...
static int Threat_dumbfire_big_frame;		// frame offset of current big dumbfire flashing warning
...
void hud_update_reticle( player *pp )
{
	...
	rval = ship_dumbfire_threat(shipp);
	if (rval == 1) {
	...
	if (rval == 2) {
		pp->threat_flags |= THREAT_DUMBFIRE_BIG;
		pp->update_dumbfire_big_time = timestamp(THREAT_UPDATE_DUMBFIRE_BIG_TIME);
	}
	...
	if ( timestamp_elapsed(pp->update_dumbfire_big_time) ) {
		pp->update_dumbfire_time = timestamp(THREAT_UPDATE_DUMBFIRE_BIG_TIME);
		pp->threat_flags &= ~THREAT_DUMBFIRE_BIG;
	}
...
void hud_render_throttle_speed(float current_speed, int y_end)
{
	...
	//gr_printf(sx, sy, buf);
	// Draw the craft speed at a fixed location
	
	gr_printf(148,3, "Speed:");
	gr_printf(x_pos, y_pos, buf);
	...
	if ( Players[Player_num].flags & PLAYER_FLAGS_MATCH_TARGET ) {
		...
		gr_printf(sx, sy, "%c", Lcl_special_chars + 3);
...
void hud_show_throttle()
{
	...
	// Get the throttle percent and print it to screen
	throttle_percent = percent_max * 100;
	gr_printf(127,12, "Throttle:");
	gr_printf(194, 12, "%d%c",fl2i(throttle_percent), '%');
...
void hud_show_dumbfire_threat()
{
	...
	// big lasers
	if ( Player->threat_flags & THREAT_DUMBFIRE_BIG ) {
		if ( timestamp_elapsed(Threat_dumbfire_big_timer) ) {
			Threat_dumbfire_big_timer = timestamp(THREAT_DUMBFIRE_BIG_FLASH);
			Threat_dumbfire_big_frame++;
			if ( Threat_dumbfire_big_frame > 2 ) {
				Threat_dumbfire_big_frame = 1;
			}
		}
		frame_offset = Threat_dumbfire_big_frame;
	} else {
		frame_offset = 0;
	}
	...
	GR_AABITMAP(Reticle_gauges[RETICLE_BIG_WARN].first_frame + frame_offset, Reticle_frame_coords[gr_screen.res][RETICLE_BIG_WARN][0], Reticle_frame_coords[gr_screen.res][RETICLE_BIG_WARN][1]);	
