 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:49:03  Darkhill
 * no message
 *
...
char Shield_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"shield1",
	"2_shield1"
};


static int Shield_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		1, 93
	},
...
void hud_shield_level_init()
{
	...
	//shield console crap
	Shield_console.first_frame = bm_load_animation(Shield_console_fname[gr_screen.res], &Shield_console.num_frames);
	if ( Shield_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Shield_console_fname[gr_screen.res]);
	}
}
...
void hud_shield_show(object *objp)
{
	...
	if (objp == Player_obj) {
		...
		hud_set_gauge_color(HUD_TARGET_MONITOR);// HUD_TARGET_SHIELD_ICON);
	}
	...
	for ( i = 0; i < MAX_SHIELD_SECTIONS; i++ ) {
		...
		if ( !flash ) {
			...
			if ( objp == Player_obj ) {
				...
				hud_set_gauge_color(HUD_TARGET_MONITOR, hud_color_index); //HUD_TARGET_SHIELD_ICON
			}
	...
	if ( objp == Player_obj ) {
		hud_set_gauge_color(HUD_PLAYER_SHIELD_ICON);
		GR_AABITMAP(Shield_console.first_frame+1, Shield_console_coords[gr_screen.res][0], Shield_console_coords[gr_screen.res][1]);
	}
...
void hudshield_page_in()
{
	...
	//shield console
	bm_page_in_aabitmap(Shield_console.first_frame, Shield_console.num_frames);
}
