 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:06 $
 * $Author: Darkhill $
 *
 * C module that contains all the HUD functions at a high level
 *
 * $Log: hud.cpp,v $
 * Revision 1.2  2004/09/07 22:56:06  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:48:21  Darkhill
 * no message
 *
...
static int Player_turbo_snd_loop = -1;
...
// top console stuff
char Top_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"topcons1",
	"topcons1"
};

// bottom console stuff
char Bottom_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"botcons1",
	"botcons1"
};

// left console
char Left_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"sidecons1",
	"sidecons1",
};

// right console
char Right_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"sidecons2",
	"sidecons2",
};

// LSE console
char LSE_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"lseconsole",
	"lseconsole",
};

// LSE console
char LSE_border_hud_console_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"lseborder",
	"lseborder",
};

char Cockpit_overlay_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {
	"xwingint1",
	"xwingint1",
};


static int Cockpit_overlay_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		1,1
	},
...
hud_frames Cockpit_overlay;
static int cockpit;

static int Top_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		93,1
	},
...
hud_frames Top_hud_console;


static int Bottom_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		175,351
	},
...
hud_frames Bottom_hud_console;

static int Left_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		0,363
	},
...
hud_frames Left_hud_console;

static int Right_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		 454,363
	},
...
hud_frames Right_hud_console;

static int LSE_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		 564,93
	},
...
hud_frames LSE_hud_console;

static int LSE_border_hud_console_coords[GR_NUM_RESOLUTIONS][2] = 
{
	{ // GR_640
		 564,91
	},
...
hud_frames LSE_border_hud_console;
...
// show all huds parts SP/MP for debuging
void debug_netlag_icon();
...
void HUD_init()
{
	...
	// top console
	Top_hud_console.first_frame = bm_load_animation(Top_hud_console_fname[gr_screen.res], &Top_hud_console.num_frames);
	if ( Top_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Top_hud_console_fname[gr_screen.res]);
	}

	// bottom console
	Bottom_hud_console.first_frame = bm_load_animation(Bottom_hud_console_fname[gr_screen.res], &Bottom_hud_console.num_frames);
	if ( Bottom_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Bottom_hud_console_fname[gr_screen.res]);
	}

	// left console
	Left_hud_console.first_frame = bm_load_animation(Left_hud_console_fname[gr_screen.res], &Left_hud_console.num_frames);
	if ( Left_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Left_hud_console_fname[gr_screen.res]);
	}

	// right console
	Right_hud_console.first_frame = bm_load_animation(Right_hud_console_fname[gr_screen.res], &Right_hud_console.num_frames);
	if ( Right_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Right_hud_console_fname[gr_screen.res]);
	}

	// LSE console
	LSE_hud_console.first_frame = bm_load_animation(LSE_hud_console_fname[gr_screen.res], &LSE_hud_console.num_frames);
	if ( LSE_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", LSE_hud_console_fname[gr_screen.res]);
	}

	// LSE border console
	LSE_border_hud_console.first_frame = bm_load_animation(LSE_border_hud_console_fname[gr_screen.res], &LSE_border_hud_console.num_frames);
	if ( LSE_hud_console.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", LSE_border_hud_console_fname[gr_screen.res]);
	}

	// cockpit
	Cockpit_overlay.first_frame = bm_load_animation(Cockpit_overlay_fname[gr_screen.res], &Cockpit_overlay.num_frames);
	if ( Cockpit_overlay.first_frame < 0 ) {
		Warning(LOCATION,"Cannot load hud ani: %s\n", Top_hud_console_fname[gr_screen.res]);
	}
	cockpit = bm_load(Ship_info[Player_ship->ship_info_index].cockpit);
	//cockpit = bm_load(Cockpit_overlay_fname[gr_screen.res]);

	// set the weapon gauge to be a popup 
	hud_config_popup_flag_set(HUD_WEAPONS_GAUGE);
...
{
	// no targeting info if ship is cloaked
	if (Player_ship->flags2 & SF2_CLOAK_ON) //    ->phys_info.flags & PF_CLOAK_ON)
		return;
...
void hud_show_common_3d_gauges(float frametime, int in_cockpit)
{
	// no targeting info if ship is cloaked
	if (Player_ship->flags2 & SF2_CLOAK_ON)
	//	if (Player_obj->phys_info.flags & PF_CLOAK_ON)
		return;
...
void hud_maybe_show_damage()
{
	...
	if ( hud_gauge_active(HUD_DAMAGE_GAUGE) ) {
		...
		// changed from 1.0f so that damage pop appears if ship is even at 99% hull
		if ( (Player_ship->ship_initial_hull_strength - Player_obj->hull_strength) > 0.1f ) {
			show_gauge_flag = 1;
		} else {
			show_gauge_flag = 0;
		}

		// if any of the subsystems are damaged then show the display
...
void hud_show_mission_time()
{
	...
	hud_set_gauge_color(HUD_THROTTLE_GAUGE); // HUD_MISSION_TIME part of top console
...
*/
	...
	if (!(Viewer_mode & (VM_EXTERNAL | VM_SLEWED |/* VM_CHASE |*/ VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY ))) {		
		...
		if ( hud_gauge_active(HUD_THROTTLE_GAUGE) ) {
			hud_set_gauge_color(HUD_THROTTLE_GAUGE); // part of top console
			GR_AABITMAP(Top_hud_console.first_frame, Top_hud_console_coords[gr_screen.res][0], Top_hud_console_coords[gr_screen.res][1]);
		}

		//show bottom console
		hud_set_gauge_color(HUD_TARGET_MONITOR);
		GR_AABITMAP(Bottom_hud_console.first_frame, Bottom_hud_console_coords[gr_screen.res][0], Bottom_hud_console_coords[gr_screen.res][1]);
		...
		if ( hud_gauge_active(HUD_WEAPONS_GAUGE) ) {
			...
			if ( show_gauge_flag ) {
				...
				GR_AABITMAP(Left_hud_console.first_frame, Left_hud_console_coords[gr_screen.res][0], Left_hud_console_coords[gr_screen.res][1]);
			}
		}	
		
		//show right console
		//hud_set_gauge_color(HUD_TARGET_MONITOR);
		//GR_AABITMAP(Right_hud_console.first_frame, Right_hud_console_coords[gr_screen.res][0], Right_hud_console_coords[gr_screen.res][1]);
		...
		//gr_reset_clip();	
		//GR_MAYBE_CLEAR_RES(cockpit);
		
		
		//gr_set_bitmap(cockpit); 
		//GR_AABITMAP(Cockpit_overlay.first_frame, Cockpit_overlay_coords[gr_screen.res][0], Cockpit_overlay_coords[gr_screen.res][1]);
		//gr_bitmap(1,1);
		...
		if ( hud_gauge_active(HUD_ETS_GAUGE) ) {
			...
			if ( show_gauge_flag ) {
				...
				GR_AABITMAP(LSE_hud_console.first_frame, LSE_hud_console_coords[gr_screen.res][0], LSE_hud_console_coords[gr_screen.res][1]);
		
				//show LSE border console
				if (Cmdline_UseTwoRadar == 0)
				{
					hud_set_gauge_color(HUD_ETS_GAUGE);
					GR_AABITMAP(LSE_border_hud_console.first_frame, LSE_border_hud_console_coords[gr_screen.res][0], LSE_border_hud_console_coords[gr_screen.res][1]);
				}
...
void hud_stop_looped_engine_sounds()
{
	...
	if (Player_turbo_snd_loop > -1) {
		snd_stop(Player_turbo_snd_loop);
		Player_turbo_snd_loop = -1;
	}
}
	...
	if ( timestamp_elapsed(throttle_sound_check_id) ) {
		...
		if ( percent_throttle != last_percent_throttle || Player_engine_snd_loop == -1 ) {
			...
			if ( percent_throttle < ZERO_PERCENT ) {
				...
				if ( Player_turbo_snd_loop > -1 )	{
					snd_chg_loop_status(Player_turbo_snd_loop, 0);
					Player_turbo_snd_loop = -1;
				}
			}
			else {
				if ( Player_engine_snd_loop == -1 ){ 
					// curious that the sounds.tbl has a list of engines for the different craft types in the game but the player
					// only gets to hear one sound in their cockpit.  so this has been fixed to play whatever
					// sound is read out of ships.tbl that goes with the engine sounds section in the sound table
					Player_engine_snd_loop = snd_play_looping( &Snds[sip->engine_snd], 0.0f , -1, -1, percent_throttle * ENGINE_MAX_VOL ); //SND_ENGINE
				} else {
				...
				if ((Player_turbo_snd_loop == -1) && (Player_ship->pod_weapon == TWT_TURBOCHARGER)){ 
					Player_turbo_snd_loop = snd_play_looping( &Snds[Player_ship->pod_sound_loop], 0.0f , -1, -1, percent_throttle * ENGINE_MAX_VOL ); //SND_ENGINE
				} else {
					// The sound may have been trashed at the low-level if sound channel overflow.
					// TODO: implement system where certain sounds cannot be interrupted (priority?)
					if ( snd_is_playing(Player_turbo_snd_loop) ) {
						snd_set_volume(Player_turbo_snd_loop, percent_throttle * ENGINE_MAX_VOL);
					}
					else {
						Player_turbo_snd_loop = -1;
					}
...
void hud_show_damage_popup()
{
	...
	float					strength, shield, integrity, system;
	char					buf[128];
	...
	hud_get_target_strength(Player_obj, &shield, &integrity, &system);
	screen_integrity = fl2i(integrity*100);

	// removed in case ship is damaged at start of mission
	//if ( hud_gauge_is_popup(HUD_DAMAGE_GAUGE) ) {
	...
	// show hull integrity even if at 100% if other subsystems are damaged
	// show hull integrity
	//if ( screen_integrity < 100 ) {		
...
void hud_show_kills_gauge()
{
	...
	// hud_set_default_color();
	hud_set_gauge_color(HUD_THROTTLE_GAUGE); //HUD_KILLS_GAUGE part of top console
...
//	gr_string(Support_text_coords[gr_screen.res][0], Support_text_coords[gr_screen.res][1], XSTR( "support", 224));
	...
	if ( Player_ai->ai_flags & AIF_BEING_REPAIRED ) {
		...
		if (  (ship_get_subsystem_strength(Player_ship, SUBSYSTEM_ENGINE) < 1.0 ) ||
				...
				(ship_get_subsystem_strength(Player_ship, SUBSYSTEM_POD) < 1.0 ) ||
...
// hud_set_iff_color() will set the color to the IFF color based on the iff
//
...
void hud_set_iff_color(object *objp, int is_bright)
{
	...
	// do a specific color for weapons
	if (objp->type == OBJ_WEAPON) {
		gr_set_color_fast(&IFF_colors[IFF_COLOR_WARHEAD][is_bright]);
	} else if ( ship_is_tagged(objp) ) {
		gr_set_color_fast(&IFF_colors[IFF_COLOR_TAGGED][is_bright]);
	} else if (objp->type == OBJ_JUMP_NODE) {
		gr_set_color_fast(&IFF_colors[Jump_nodes[objp->instance].iff][is_bright]);	
	} else { 
		gr_set_color_fast(&IFF_colors[Ships[objp->instance].iff][is_bright]);
	}
...
void hud_maybe_display_subspace_notify()
{
	...
	gr_string(0x8000, Subspace_text_coords[gr_screen.res][1],"Hyperdrive");
	if ( warp_aborted ) {
...
{
	// why do we need to skew the position of hud parts based on where the eye point of the model is located at?
...
//	bm_page_in_aabitmap( Head_frame_gauge.first_frame, Head_frame_gauge.num_frames );
	...
	// top console
	bm_page_in_aabitmap(Top_hud_console.first_frame, Top_hud_console.num_frames);

	// bottom console
	bm_page_in_aabitmap(Bottom_hud_console.first_frame, Bottom_hud_console.num_frames);

	// left console
	bm_page_in_aabitmap(Left_hud_console.first_frame, Left_hud_console.num_frames);

	// right console
	bm_page_in_aabitmap(Right_hud_console.first_frame, Right_hud_console.num_frames);

	// LSE console
	bm_page_in_aabitmap(LSE_hud_console.first_frame, LSE_hud_console.num_frames);

	// LSE border console
	bm_page_in_aabitmap(LSE_border_hud_console.first_frame, LSE_border_hud_console.num_frames);

	bm_page_in_aabitmap(Cockpit_overlay.first_frame, Cockpit_overlay.num_frames);
}

void render_cockpit(void)
{
	if (!(Viewer_mode & (VM_EXTERNAL | VM_SLEWED | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY )))		
	{
		if (cockpit >= 0) {
			gr_set_bitmap(cockpit);
			gr_bitmap(0, 0);
