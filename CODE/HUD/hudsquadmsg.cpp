 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:09 $
 * $Author: Darkhill $
 *
 * File to control sqaudmate messaging
 *
 * $Log: hudsquadmsg.cpp,v $
 * Revision 1.2  2004/09/07 22:56:09  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:50:15  Darkhill
 * no message
 *
...
#define TYPE_REPAIR_REARM_ABORT_ITEM	4

#define NUM_TYPE_SELECT		5
...
static hud_frames Mbox_gauge;
static int Mbox_frames_loaded = 0;
static char *Mbox_fnames[GR_NUM_RESOLUTIONS] = {
	"sidecons2",
	"sidecons2"
};
...
int hud_squadmsg_ship_order_valid( int shipnum, int order )
{
	...
	// if the ship is disabled or has no engines then it really can't do anything, can it?
	if (Ships[shipnum].flags & SF_ENGINES_DISABLED || Ships[shipnum].flags2 & SF2_DISABLED)
		return 0;
	...
	switch ( order  )
	{
		case DEPART_ITEM:
			...
			if (Ships[shipnum].flags & (SF_ENGINES_DISABLED))
				return 0;
...
int hud_squadmsg_is_target_order_valid(int order, int find_order, ai_info *aip )
{
	...
	if ( objp->type == OBJ_WEAPON ) {
		...
		if ( (Comm_orders[order].value == ATTACK_TARGET_ITEM )
			&& !(Weapon_info[Weapons[objp->instance].weapon_info_index].subtype == WP_LASER)
	...
	if ( (Comm_orders[order].value == DISABLE_TARGET_ITEM) && (shipp->flags2 & SF2_DISABLED) ){
		return 0;
	}
...
int hud_squadmsg_send_ship_command( int shipnum, int command, int send_message, int player_num )
{
	...
	if ( hud_squadmsg_is_target_order_valid(command, 1, ainfo) ) {
		...
		case DISABLE_TARGET_ITEM:
			...
			ai_mode = AI_GOAL_DISABLE;
			//ai_submode = SM_DISABLE;
			message = MESSAGE_ATTACK_TARGET;
			break;
...
void hud_squadmsg_wing_select()
{
	...
	hud_squadmsg_display_menu("Select FG");
	k = hud_squadmsg_get_key();
	if ( k != -1 ) {						// if true, we have selected a ship.
...
{
	Mbox_gauge.first_frame = bm_load_animation(Mbox_fnames[gr_screen.res], &Mbox_gauge.num_frames);
	if ( Mbox_gauge.first_frame == -1 ) {
			Warning(LOCATION, "Could not load in ani: %s\n", Mbox_fnames[gr_screen.res]);
			return;
	}
...
//		int who_to_sig;
	...
	// draw top of frame
	GR_AABITMAP(Mbox_gauge.first_frame, Mbox_top_coords[gr_screen.res][0], Mbox_top_coords[gr_screen.res][1]);		
...
void hudsquadmsg_page_in() 
{
	...
	bm_page_in_aabitmap( Mbox_gauge.first_frame, Mbox_gauge.num_frames );
