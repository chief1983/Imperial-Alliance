 ...
 *
 * $Log: hudmessage.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:48:59  Darkhill
 * no message
 *
...
void message_force_button_frame(int, int);
...
static int Hud_mission_log_list_coords[GR_NUM_RESOLUTIONS][4] = {
	{
		75,60,500,300//46,60,558,269		// GR_640
	},
	{
...
static int Hud_mission_log_list2_coords[GR_NUM_RESOLUTIONS][4] = {
	{
		293, 60, 500, 300//108, 60, 496, 297		// GR_640
	},
	{
...
int MSG_WINDOW_X_START = 57;//5;475
int MSG_WINDOW_Y_START = 50; //5375
...
static scrollback_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
	{	// GR_640
	//XSTR:OFF
		scrollback_buttons("LB_00",	50, 47,	-1,	-1,	0),
		scrollback_buttons("LB_01",	50,345,	-1,	-1,	1),
		scrollback_buttons("LB_02",	100,375,	108,	413,	2),
		scrollback_buttons("LB_03",	155,375,	205,	413,	3),
		scrollback_buttons("LB_04",	45,375,	7,		413,	4),
		scrollback_buttons("LB_05",540,375,	564,	413,	5)
	//XSTR:ON
	},
	{	// GR_1024
	//XSTR:OFF
		...
		scrollback_buttons("2_LB_05",914,	681,	946,	661,	5)
	//XSTR:ON
	}
}
;
...
void hud_init_msg_window()
{
	...
	MSG_WINDOW_WIDTH = gr_screen.clip_width - 40;
...
// converts iff to a HUD_SOURCE_* define
int HUD_get_iff_source(int iff)
{
	switch (iff) {
		...
		case IFF_COLOR_BLUE:
			return HUD_SOURCE_BLUE;
		case IFF_COLOR_PURPLE:
			return HUD_SOURCE_PURPLE;
		case IFF_COLOR_YELLOW:
			return HUD_SOURCE_YELLOW;
		case IFF_COLOR_RED:
			return HUD_SOURCE_RED;
		case IFF_COLOR_ORANGE:
			return HUD_SOURCE_ORANGE;
	}
...
void HUD_ship_sent_printf(int sh, char *format, ...)
{
	...
	len = strlen(tmp);
	Assert(len < HUD_MSG_LENGTH_MAX);	//	If greater than this, probably crashed anyway.
	hud_sourced_print(HUD_get_iff_source(Ships[sh].iff), tmp);
}
...
void hud_sourced_print(int source, char *msg)
{
	...
	int line_break = (gr_screen.res == GR_640) ? 435 : 460;      // how long text line is before broken up
	if ( !strlen(msg) ) {
	...
	t = timestamp();
	str = msg;
	while ((ptr = split_str_once(str, line_break - x - 7)) != NULL) {		// the 7 is a fudge hack
...
//		y = ((LIST_H / font_height) - 1) * font_height;
			...
			while ( node_ptr != END_OF_LIST(&Msg_scrollback_used_list) ) {
				if ((node_ptr->source == HUD_SOURCE_HIDDEN) || (i++ < Scroll_offset)) {
					...
					switch (node_ptr->source) {
						...
						case HUD_SOURCE_BLUE:
							SET_COLOR_BLUE;
							break;

						case HUD_SOURCE_PURPLE:
							SET_COLOR_PURPLE;
							break;
						
						case HUD_SOURCE_YELLOW:
							SET_COLOR_YELLOW;
							break;
	...
	{
		if (i == current_button)
			message_force_button_frame(i, 2);
		else
			message_force_button_frame(i, 1);
	}
...
#define ACCEPT_BUTTON		5*/
	for (i=2; i<=5; i++) 
	{
		...
		if (b->button.button_hilighted())
		{
			...
			{
			case SHOW_MSGS_BUTTON:
				gr_string(93,397, "Messages");
				break;

			case SHOW_EVENTS_BUTTON:
				gr_string(158,397, "Events");
				break;

			case SHOW_OBJS_BUTTON:
				gr_string(34,397, "Objectives");
				break;

			case ACCEPT_BUTTON:
				gr_string(535,397, "Continue");
				break;
			}
		}
	}
...
void message_force_button_frame(int n, int frame_num)
{
