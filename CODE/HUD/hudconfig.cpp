 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:48:46  Darkhill
 * no message
 *
...
void hud_config_maybe_show_button_text();
void hud_config_force_button_frame();
...
// In changing the HUD config from the original some stupid things have to be done that probably
// won't be obvious unless I explain them here.  Unlike in Freespace2 not every HUD part can be
// colored seperately.  But so I don't start breaking things later on I'm not going to change any
// of the #defines or anything else in the code.  Just going to change the hud_set_color() function
// so that it matches the parent HUD part.  Also so I don't break things I'm not going to change
// any of the names.  So here will be the list of the groupings and which is the color define for
// the group.
//
// Player shield console - both the icon and the console around it - HUD_TARGET_SHIELD_ICON
// C.M.D. - HUD_TARGET_MONITOR is for the color
//   HUD_TARGET_MONITOR
//   HUD_AUTO_TARGET
// Weapon console - actual console never got its own #define so it uses the old weapons gauge
//   HUD_WEAPONS_GAUGE
// Top console - actual console never got its own #define -- HUD_THROTTLE_GAUGE
//   HUD_MISSION_TIME
//   HUD_AUTO_SPEED
//   HUD_CMEASURE_GAUGE
//   HUD_THROTTLE_GAUGE
//   HUD_KILLS_GAUGE
// L.S.E. - console never got define so uses L.S.E. (E.T.S.) gauge
...
struct HC_gauge_region	HC_gauge_regions[GR_NUM_RESOLUTIONS][NUM_HUD_GAUGES] = //
{
	{ // GR_640
	//XSTR:OFF
		HC_gauge_region("none",	293, 92,	35,	1,	0,	-1, 0,	2),			// lead indicator 409,	69 HCB_35 -took out
		HC_gauge_region("none",	305,	119,	36,	1,	0,	-1, 0,	2),			// orientation tee HCB_36 -took out
		HC_gauge_region("none",		1,		1,		-1,	1,	0,	-1, 0,	0),			// hostile triangle -took out
		HC_gauge_region("none",	391,	107,	37,	1,	0,	-1, 0,	2),			// target triangle HCB_37 -took out
		HC_gauge_region("none",	575,	352,	63,	0,	0,	-1, 0,	0),			// mission time
		HC_gauge_region("none",		1,		1,		1,		0,	0,	-1, 0,	0),			// reticle circle?
		HC_gauge_region("HCB_40",	230,	60,		40,	0,	0,	-1, 0,	0),			// throttle gauge 285,	146
		HC_gauge_region("HCB_50",	170,	60,		50,	0,	0,	-1, 0,	0),			// radar 317,	291
		HC_gauge_region("HCB_31",	283,	282,	31,	0,	0,	-1, 0,	0),			// target monitor 123,249
		HC_gauge_region("HCB_41",	361,	192,	41,	0,	0,	-1, 0,	0),			// center of reticle 361,	188
		HC_gauge_region("HCB_30",	179,	243,	30,	0,	0,	-1, 0,	0),			// extra target data 123,	221
		HC_gauge_region("none",		237,	303,	49,	0,	0,	-1, 0,	0),			// target shield icon
		HC_gauge_region("HCB_51",	170,	119,	51,	0,	0,	-1, 0,	0),			// player shield icon 435,	304
		HC_gauge_region("HCB_58",	541,	120,	58,	0,	1, -1, 0,	0),			// ets gauge 524,	299
		HC_gauge_region("none",		566,	299,	61,	0,	1, -1, 0,	0),			// auto target
		HC_gauge_region("none",		566,	317,	62,	0,	1, -1, 0,	0),			// auto speed
		HC_gauge_region("HCB_55",	170,    291,	55,	0,	1, -1, 0,	0),			// weapons gauge 504,	216
		HC_gauge_region("HCB_54",	488,	212,	54,	0,	1, -1, 0,	0),			// escort view 496,	166
		HC_gauge_region("HCB_29",	170,	184,	29,	0,	0, -1, 0,	0),			// directives view 123,	142
		HC_gauge_region("none",		398,	147,	43,	0,	0, -1, 0,	0),			// threat gauge
		HC_gauge_region("HCB_39",	332,	279,	39,	0,	0, -1, 0,	0),			// afterburner energy 250,	212
		HC_gauge_region("HCB_39",	381,	279,	44,	0,	0, -1, 0,	0),			// weapons energy 449,	212
		HC_gauge_region("none",		1,		1,		-1,	0,	0, -1, 0,	0),			// weapon linking
		HC_gauge_region("none",		356,	232,	42,	0,	1, -1, 0,	0),			// target mini icon (shield)
		HC_gauge_region("none",		438,	5,		34,	1,	0, -1, 0,	2),			// offscreen indicator
		HC_gauge_region("none",		123,	31,	28,	0,	0, -1, 0,	0),			// talking head
		HC_gauge_region("HCB_32",	328,	85,	32,	0,	1, -1, 0,	0),			// damage gauge 309,	33
		HC_gauge_region("HCB_27",	253,	106,	27,	0,	0, -1, 0,	0),			// message lines 124,	19
		HC_gauge_region("none",		307,	249,	45,	1,	0, -1, 0,	1),			// missile warnings
		HC_gauge_region("none",		505,	271,	56,	0,	1,	-1, 0,	0),			// cmeasure gauge
		HC_gauge_region("HCB_33",	328,	133,	33,	0,	0,	-1, 0,	0),			// objectives notify gauge 309,	87
		HC_gauge_region("none",	546,	117,	53,	0,	0,	-1, 0,	0),			// wingman status gauge
		HC_gauge_region("none",		1,		1,		-1,	0,	0,	-1, 0,	0),			// offscreen indicator range
		HC_gauge_region("none",	505,	285,	57,	0,	1,	-1, 0,	0),			// kills gauge
		HC_gauge_region("none",		1,		1,		-1,	0,	0,	-1, 0,	0),			// attacking target count
		HC_gauge_region("HCB_38",	357,	173,	38,	0,	0,	-1, 0,	0),			// text flash gauge 342,	138
		HC_gauge_region("HCB_52",	465,	291,		52,	0,	0,	-1, 0,	0),			// comm menu 465,	8
#ifndef FS2_DEMO
		HC_gauge_region("HCB_46",	342, 148,	46,	0,	0,	-1, 0,	0),			// support view gauge 324,	264
		HC_gauge_region("HCB_47",	563,	191,	47,	0,	0,	-1, 0,	0),			// netlag icon gauge 418,	262
#endif
	//XSTR:ON
	},
	{ // GR_1024
	//XSTR:OFF
		...
		HC_gauge_region("2_HCB_50",	170,	60,	50,	0,	0,	-1, 0,	0),			// radar
		HC_gauge_region("2_HCB_31",	170,	60,	31,	0,	0,	-1, 0,	0),			// target monitor
		...
		HC_gauge_region("2_HCB_51",	696,	486,	31,	0,	0,	-1, 0,	0),			// player shield icon
...
char *HC_gauge_descriptions(int n)
{
	...
	case 6:
		return XSTR( "top console", 1492); //throttle
	case 7:
		return XSTR( "radar", 256);
	case 8:
		return XSTR( "c.m.d.", 1493);
	case 9:
...
ui_button_info HC_buttons[GR_NUM_RESOLUTIONS][NUM_HUD_BUTTONS] = {
	{ // GR_640
		ui_button_info("HCB_00",		55, 81,	-1,	-1,	0),
		ui_button_info("HCB_01",		80, 81,	-1,	-1,	1),
		ui_button_info("HCB_02",		105, 81,	-1,	-1,	2),
		ui_button_info("HCB_03",		130, 81,	-1,	-1,	3),
		ui_button_info("HCB_08",		55,345,	-1,	-1,	8),
		ui_button_info("HCB_09",		80,345,	-1,	-1,	9),
		ui_button_info("HCB_10",		105,345,	-1,	-1,	10),
		ui_button_info("HCB_11",		130,345,	-1,	-1,	11),
		ui_button_info("HCB_12",		410, 30,	-1,	-1,	12),
		ui_button_info("HCB_13",		465, 30,	-1,	-1,	13),
		ui_button_info("HCB_14",		520, 30,	-1,	-1,	14),
		ui_button_info("HCB_15",		45,375,	-1,	-1,	15),
		ui_button_info("HCB_17",		320,405,	-1,	-1,	16),
		ui_button_info("HCB_16",		320,375,	-1,	-1,	17),
		ui_button_info("HCB_18",		350,375,	-1,	-1,	18),
		ui_button_info("HCB_19",		350,405,	-1,	-1,	19),
		ui_button_info("HCB_20",		327,	459,	-1,	-1,	20),
		ui_button_info("HCB_24",		430,375,	-1,	-1,	24),
		ui_button_info("HCB_25",		485,375,	-1,	-1,	25),
		ui_button_info("HCB_26",		540,375,	-1,	-1,	26),
	},
	{ // GR_1024
		...
		ui_button_info("2_HCB_17",		426,	730,	-1,	-1,	16),
		ui_button_info("2_HCB_16",		467,	730,	-1,	-1,	17),
...
// text
#define NUM_HUD_TEXT					9
UI_XSTR HC_button_text[GR_NUM_RESOLUTIONS][7] = {
	{ // GR_640
		{ "Save",			1454,	54,396,		UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SAVE_HCF].button },
		{ "Reset",			1337,	491,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_RESET].button },
		{ "Accept",			1035,	541,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_ACCEPT].button },
		{ "Next",			-1,		316,380,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_NEXT_HCF].button },
		{ "Prev",			-1,		317,411,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_PREV_HCF].button },
		{ "Select",			1550,	432,391,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SELECT_ALL].button },
		{ "All",			1551,	443,402,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SELECT_ALL].button },

	},
	{ // GR_1024
		{ "Save",			1454,	54,396,		UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_SAVE_HCF].button },
		{ "Reset",			1337,	491,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_RESET].button },
		{ "Accept",			1035,	541,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_ACCEPT].button },
		{ "Next",			-1,		100,100,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_NEXT_HCF].button },
		{ "Prev",			-1,		200,200,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_PREV_HCF].button },
		{ "Select",			1550,	432,391,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_SELECT_ALL].button },
		{ "All",			1551,	443,402,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[1][HCB_SELECT_ALL].button },
	}
};

UI_XSTR HC_text[GR_NUM_RESOLUTIONS][NUM_HUD_TEXT] = {
	{ // GR_640
		{ "R",				1512,	61, 69,		UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "G",				1513,	86, 69,		UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "B",				1514,	111, 69,		UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "S",				-1,	136, 69,		UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "On",				1285,	435, 36,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_ON].button },
		{ "Off",			1286,	490, 36,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_OFF].button },
		{ "Popup",			1453,	545, 36,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_POPUP].button },
	//	{ "Save",			1454,	54,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SAVE_HCF].button },
		{ "Amber",			1455,	375,381,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_AMBER].button },
		{ "Blue",			1456,	375,411,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_BLUE].button },
		//{ "Green",			1457,	364,	464,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_GREEN].button },		
	//	{ "Select",			1550,	432,391,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SELECT_ALL].button },
	//	{ "All",			1551,	443,402,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_SELECT_ALL].button },
	//	{ "Reset",			1337,	491,396,	UI_XSTR_COLOR_GREEN,	-1, &HC_buttons[0][HCB_RESET].button },
	//	{ "Accept",			1035,	541,396,	UI_XSTR_COLOR_PINK,	-1, &HC_buttons[0][HCB_ACCEPT].button },
	},
	{ // GR_1024
		{ "R",				1512,	23,	14,	UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "G",				1513,	60,	14,	UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "B",				1514,	100,	14,	UI_XSTR_COLOR_GREEN,	-1, NULL },
		{ "S",				-1,	144,	14,	UI_XSTR_COLOR_GREEN,	-1, NULL },
...
//	common_set_interface_palette("HUDConfigPalette");  // set the interface palette
	...
	for (i=0; i<NUM_HUD_GAUGES; i++) { //NUM_HUD_GAUGES
...
// 			hg->bitmap = bm_load_animation(hg->filename, &hg->nframes);
	// 	} else {
			...
			//gr_set_bitmap(hg->bitmap);
			//gr_aabitmap(hg->x, hg->y);
...
void hud_config_render_gauges()
{
	...
	for ( i=0; i<NUM_HUD_GAUGES; i++ ) {
		...
		if ( (hud_config_show_flag_is_set(i)) ) {
			...
			if(!HC_gauge_regions[gr_screen.res][i].use_iff){
				...
				if(HC_gauge_regions[gr_screen.res][i].color == 1){
					...
					use_color = &IFF_colors[IFF_COLOR_REBEL][0];
				}
			}
		...
		//if ( HC_gauge_regions[gr_screen.res][i].bitmap >= 0 ) {
			...
			gr_aabitmap(HC_gauge_regions[gr_screen.res][i].x, HC_gauge_regions[gr_screen.res][i].y);//HC_gauge_offset[gr_screen.res][0], HC_gauge_offset[gr_screen.res][1]);
		//}
...
void hud_config_do_frame(float frametime) 
{
	...
	*/
	...
	for(int i=0; i<NUM_HUD_TEXT; i++){
		...
		gr_string(HC_text[gr_screen.res][i].x, HC_text[gr_screen.res][i].y, HC_text[gr_screen.res][i].xstr);
	}

	// force button frames
	hud_config_force_button_frame();

	// print floating text
	hud_config_maybe_show_button_text();
...
void hud_config_force_button_frame()
{
	for (int x=11; x<19; x++) {
		if (x == HCB_GREEN)
			continue;
		if (!HC_buttons[gr_screen.res][x].button.button_down()) 
			HC_buttons[gr_screen.res][x].button.draw_forced(1);
	}
}

// crime against humanity number 129843.  blame it on the $100/day coke habit.
void hud_config_maybe_show_button_text()
{
	ui_button_info *b;
	int print = 0;
	...
	// select all needs two lines so it gets a special case
	for (int i=0; i<NUM_HUD_BUTTONS; i++) {
		if (i == HCB_SELECT_ALL) {
			b = &HC_buttons[gr_screen.res][i];
			if (b->button.button_hilighted()) {
				gr_set_color_fast(&Color_bright_white);
				gr_string(HC_button_text[gr_screen.res][5].x, HC_button_text[gr_screen.res][5].y, HC_button_text[gr_screen.res][5].xstr);
				gr_string(HC_button_text[gr_screen.res][6].x, HC_button_text[gr_screen.res][6].y, HC_button_text[gr_screen.res][6].xstr);
			}
		...
		if (print) {
			...
			if (b->button.button_hilighted()) {
				gr_set_color_fast(&Color_bright_white);
				gr_string(HC_button_text[gr_screen.res][string].x, HC_button_text[gr_screen.res][string].y, HC_button_text[gr_screen.res][string].xstr);
			}
		}
		print = 0;
	}
