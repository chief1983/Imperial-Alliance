 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:49:33  Darkhill
 * no message
 *
...
int Weapon_title_coords[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][2] =
{
	{ // normal HUD
		{ // GR_640
			50, 372 // 518, 274 -diff- x = subtract 468 y = add 98 
		},
		{ // GR_1024
			901, 527
		}
...
int Weapon_plink_coords[GR_NUM_RESOLUTIONS][2][2] = {
	{ // GR_640
		{50, 383},//{530, 285},				// fire-linked thingie, for the first primary
		{50, 393},//{530, 295}				// fire-linked thingie, for the second primary
	},
	{ // GR_1024
...
int Weapon_pname_coords[GR_NUM_RESOLUTIONS][2][2] = {
	{ // GR_640
		{56, 383}, // {536, 285}// weapon name, first primary
		{56, 393}	//{536,295},			// weapon name, second primary
	},
	{ // GR_1024
...
int Weapon_slinked_x[GR_NUM_RESOLUTIONS] = {
	52,//525,							// where to draw the second thingie if this weapon is fire-linked
	908
};
int Weapon_sunlinked_x[GR_NUM_RESOLUTIONS] = {
	50, //62, //530,							// where to draw the first thingie if this weapon is selected at all (fire-linked or not)
	913
};
int Weapon_secondary_y[GR_NUM_RESOLUTIONS][3] = {
	{ // GR_640
		407,//309,						// y location of where to draw text for the first secondary
		416,//318,						// y location of where to draw text for the second secondary
		425//327						// y location of where to draw text for the third secondary
	},
	{ // GR_1024
...
int Weapon_primary_y[GR_NUM_RESOLUTIONS][2] = {
	{ // GR_640
		383,//285,						// y location of where to draw text for the first primary
...
int Weapon_secondary_name_x[GR_NUM_RESOLUTIONS] = {
	56,//68, //536,							// x location of where to draw weapon name
	919
};
int Weapon_secondary_ammo_x[GR_NUM_RESOLUTIONS] = {
	45, //57, //525,							// x location of where to draw weapon ammo count
	908
};
int Weapon_primary_ammo_x[GR_NUM_RESOLUTIONS] = {
	45, //57, //525,							// x location of where to draw primary weapon ammo count
	908
};
int Weapon_secondary_reload_x[GR_NUM_RESOLUTIONS] = {
	135, //615,							// x location of where to draw the weapon reload time
	998
};
...
void hud_target_subobject_common(int next_flag)
{
	...
	for ( A = start2; A != start; A = advance_subsys(A, next_flag) ) {
		...
		// ignore s-foil wings
		if (A->system_info->type == SUBSYSTEM_SFOIL) {
			continue;
		}
		
		// if the craft doesn't have a pod system installed then we
		// don't want the player to be able to select it
		if (A->system_info->type == SUBSYSTEM_POD) {
			if (target_shipp->pod_weapon == -1)
				continue;
		}
...
void hud_target_common(int team, int next_flag)
{
	...
	for ( A = start2; A != start; A = advance_fb(A, next_flag) ) {
		...
		//if (Ships[A->instance].flags2 & SF2_NO_RADAR)
		//	continue;
		...
		if (A->type == OBJ_JUMP_NODE) {
			if (Jump_nodes[A->instance].iff != Ships[Player_obj->instance].iff)
				continue;
		}
		
		if ( A->type == OBJ_WEAPON ) {
			if (Weapon_info[Weapons[A->instance].weapon_info_index].subtype == WP_LASER)
		...
		if ( A->type == OBJ_SHIP ) {
			...
			// if ship is cloaked on decoy beam can't target
			if (Ships[A->instance].flags2 & SF2_NO_RADAR) {
				continue;
			}
			is_ship=1;
...
void hud_target_missile(object *source_obj, int next_flag)
{
	...
	for ( mo = start; mo != end; mo = advance_missile_obj(mo, next_flag) ) {
		...
		// what you talkin' about willis?  lets target any warhead
		if (Weapon_info[Weapons[A->instance].weapon_info_index].subtype == WP_LASER) {
...
void evaluate_ship_as_closest_target(esct *esct)
{
	...
	// can't target if cloaked or using decoy
	if  (esct->shipp->flags2 & SF2_NO_RADAR)
		return;
...
void hud_show_brackets(object *targetp, vertex *projected_v)
{
	...
	if ( Player->target_is_dying <= 0 ) {
		...
		if ( draw_box == TRUE ) {
			float distance;
			int color;
			color = hud_brackets_get_iff_color(iff);
...
void hud_show_lead_indicator(vector *target_world_pos)
{
	...
	// don't display lead for ship if its cloaked
	if (Ships[targetp->instance].flags2 & SF2_CLOAK_ON)
		return;
...
//	}
	...
	Assert(Player_ship);
	if ( !(Player_ship->pod_weapon == TWT_SLAM) ) {
		// Goober5000 - instead of drawing an empty burner gauge, don't draw the gauge at all
		return;
		//percent_left = 0.0f;
	} else {
		percent_left = Player_ship->afterburner_fuel/Player_ship->afterburner_fuel_capacity;   //Ship_info[Player_ship->ship_info_index].afterburner_fuel_capacity;
	}

	clip_w = fl2i( (1.0f - percent_left) * Aburn_coords[gr_screen.res][3] + 0.5f );

	bm_get_info(Energy_bar_gauges.first_frame,&w,&h);
	
	//if ( clip_h > 0) {
		GR_AABITMAP_EX(Energy_bar_gauges.first_frame, Aburn_coords[gr_screen.res][0], Aburn_coords[gr_screen.res][1],w,clip_w,0,0);		
	//}

	//if ( clip_h <= Aburn_coords[gr_screen.res][3] ) {		
	//	GR_AABITMAP_EX(Energy_bar_gauges.first_frame+1, Aburn_coords[gr_screen.res][0], Aburn_coords[gr_screen.res][1]+clip_w,w,h-clip_w,0,clip_w);
	//} 
	GR_AABITMAP_EX(Energy_bar_gauges.first_frame+3, Aburn_coords[gr_screen.res][0], Aburn_coords[gr_screen.res][1], w-clip_w,h,clip_w, 0);		

}

void hud_show_pod_gauge()
{
	float percent_left;
	int	clip_w,w,h;	
	...
	//if (Player_ship->flags2

	Assert(Player_ship); 
	if ( !(Player_ship->flags2 & SF2_POD_INSTALLED) ) {
		// instead of drawing an empty burner gauge, don't draw the gauge at all
		return;
		//percent_left = 0.0f;
	// don't display gauge if solid booster rocket installed
	} else if (Player_ship->pod_weapon == TWT_BOOST_POD) {
		return;
	// don't display if ship has got sweet turbocharger fitted.
	} else if (Player_ship->pod_weapon == TWT_TURBOCHARGER) {
		return;
	} else if (Player_ship->pod_weapon == TWT_SLAM) {
		return;
	} else {
		percent_left = Player_ship->pod_energy/Player_ship->pod_energy_capacity;
	}
...
void hud_show_weapon_energy_gauge()
{
	...
	clip_w = fl2i( (1.0f - percent_left) * Wenergy_coords[gr_screen.res][3] + 0.5f );

	bm_get_info(Energy_bar_gauges.first_frame+2,&w,&h);
	
	//if ( clip_w > 0 ) {
		GR_AABITMAP_EX(Energy_bar_gauges.first_frame+2, Wenergy_coords[gr_screen.res][0], Wenergy_coords[gr_screen.res][1], w, h,0,0);	// w, clip+h	
	//}

//	if ( clip_w <= Wenergy_coords[gr_screen.res][3] ) {
		//GR_AABITMAP_EX(Energy_bar_gauges.first_frame+3, Wenergy_coords[gr_screen.res][0], (Wenergy_coords[gr_screen.res][1]+20), w+clip_w,h,-clip_w, 0);		
		//GR_AABITMAP_EX(Energy_bar_gauges.first_frame+3, Wenergy_coords[gr_screen.res][0], Wenergy_coords[gr_screen.res][1], w+clip_w,h,clip_w, 0);		
		...
		// drains left to right
		GR_AABITMAP_EX(Energy_bar_gauges.first_frame+3, Wenergy_coords[gr_screen.res][0]+clip_w, Wenergy_coords[gr_screen.res][1], w-clip_w,h,clip_w, 0);		
...
void hud_show_cmeasure_gague()
{
	...
	// hud_set_default_color();
	hud_set_gauge_color(HUD_THROTTLE_GAUGE); //HUD_CMEASURE_GAUGE part of top console
...
void hud_auto_target_icon()
{
	...
	hud_set_gauge_color(HUD_TARGET_MONITOR);  // HUD_AUTO_TARGET part of C.M.D.
...
void hud_auto_speed_match_icon()
{
	...
	hud_set_gauge_color(HUD_THROTTLE_GAUGE); //HUD_AUTO_SPEED part of top console
...
void hud_target_random_ship()
{
	...
	if((objnum >= 0) && (Player_ai != NULL) && !hud_target_invalid_awacs(&Objects[objnum])){	
		...
		if(objnum == OBJ_INDEX(Player_obj)){
			set_target_objnum(Player_ai, -1);
		} else if (Ships[shipnum].flags2 & SF2_NO_RADAR) {
			set_target_objnum(Player_ai, -1);
		} else {
