 ...
 *
 * $Log: keycontrol.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:43:48  Darkhill
 * no message
 *
...
extern int enter_hangar;							// used to tell if player is close enough to enter hangar
int on_hangar_approach = 0;
int time_to_quit_mission;
int test1 = 0;
...
int Normal_key_set[] = {
	...
	ENTER_HANGAR,
	CYCLE_LINK_MODE_GUNS,
//	CYCLE_LINK_MODE_MISSILES,
	CYCLE_SFOILS
...
void process_debug_keys(int k)
{
	...
	switch (k) {
		case KEY_DEBUGGED1 + KEY_Q:
			ship_set_subsystem_strength(Player_ship, SUBSYSTEM_POD, 0.15f);
			break;
...
//						mission_log_add_entry(LOG_SHIP_DISABLED_ENGINES, sp->ship_name, NULL );
						sp->flags |= SF_ENGINES_DISABLED;				// add the disabled flag
					}
...
void ppsk_hotkeys(int k)
{
	...
	switch (k) {
		// you don't need this goddamn many hotkeys in a game.  so i'm
		// taking the last four so i have controls on the jukebox.
		case KEY_F5:
		case KEY_F6:
		case KEY_F7:
		case KEY_F8:
...
//	char	savegame_filename[_MAX_FNAME];
...
#endif
	{
		...
		//choice = popup(pf_flags, 3, POPUP_NO, XSTR( "&Yes, Quit", 28), "Goto &Debriefing"/*XSTR( "Yes, &Restart", 29)*/, XSTR( "Do you really want to end the mission?", 30));
		choice = popup(pf_flags, 2, POPUP_NO, "Goto &Debriefing"/*XSTR( "Yes, &Restart", 29)*/, XSTR( "Do you really want to end the mission?", 30));
...
int button_function_critical(int n, net_player *p = NULL)
{
	...
	switch (n) {				
		...
		case CYCLE_LINK_MODE_GUNS:
			po = model_get(Ships[objp->instance].modelnum);
			//need to check the number of gun mounts for the current weapon.  if there is less then 3 can't dual
			// fire without some oddities going on, so we just link all.  if there is more then go through the 
			// list of link types
			if (po->gun_banks[Ships[objp->instance].weapons.current_primary_bank].num_slots <= 3) {			
				if (Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] == WEAPON_LINK_SINGLE) {
					Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] = WEAPON_LINK_ALL;
					// need to set the firing point back to the first hard point or weird things happen
					Ships[objp->instance].last_fired_point[Ships[objp->instance].weapons.current_primary_bank] = 0;
					HUD_printf("Cannons fire linked");
				}
				else {
					Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] = WEAPON_LINK_SINGLE;
					Ships[objp->instance].last_fired_point[Ships[objp->instance].weapons.current_primary_bank] = 0;
					HUD_printf("Cannons set to single fire");
				}
			}
			else {
				if (Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] == WEAPON_LINK_SINGLE) {
					Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] = WEAPON_LINK_DUAL;
					// need to set the firing point back to the first hard point or weird things happen
					Ships[objp->instance].last_fired_point[Ships[objp->instance].weapons.current_primary_bank] = 0;
					HUD_printf("Cannons dual linked");
				}
				else if (Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] == WEAPON_LINK_DUAL) {
					Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank] = WEAPON_LINK_ALL;
					Ships[objp->instance].last_fired_point[Ships[objp->instance].weapons.current_primary_bank] = 0;
					HUD_printf("Cannons fire linked");
				}
			...
			if (Game_mode & GM_MULTIPLAYER) {
				// tell all them other bitches on the network game i'm changing my barrel linking status
				send_ship_barrel_link_packet(&Ships[objp->instance], Ships[objp->instance].primary_link_status[Ships[objp->instance].weapons.current_primary_bank], Ships[objp->instance].weapons.current_primary_bank);
				// update net player info for respawning
				//Net_player->s_info.link_mode[Player_ship->weapons.current_primary_bank] = Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank];
			}
		...
		case CYCLE_SFOILS:
			if (Ships[objp->instance].flags2 & SF2_DISABLED)
				...
				// make sure that ship class has sfoils to open
				if (Ship_info[Ships[objp->instance].ship_info_index].sfoils) {

					if (Ships[objp->instance].sfoil_status & (SFOIL_CLOSED | SFOIL_CLOSING))  {
						Ships[objp->instance].sfoil_status &= ~(SFOIL_CLOSING);
						Ships[objp->instance].sfoil_status &= ~(SFOIL_CLOSED);
						Ships[objp->instance].sfoil_status &= ~(SFOIL_OPENING);
						Ships[objp->instance].sfoil_status &= ~(SFOIL_OPEN);
						
						Ships[objp->instance].sfoil_status |= SFOIL_OPENING;
						
						HUD_printf("S-Foils opening");
					} else {
						Ships[objp->instance].sfoil_status &= ~(SFOIL_CLOSING);
						Ships[objp->instance].sfoil_status &= ~(SFOIL_CLOSED);
						Ships[objp->instance].sfoil_status &= ~(SFOIL_OPENING);
						...
						HUD_printf("S-Foils closing");
					}

					#ifndef NO_NETWORK
						if (Game_mode & GM_MULTIPLAYER ) {
							int status = bitmask_2_bitnum(Ships[objp->instance].sfoil_status);
							send_ship_sfoil_update_packet(&Ships[objp->instance], status);
							multi_server_update_player_sfoils(npl, &Ships[objp->instance]);
						}	
					#endif
				}
				else
					HUD_printf("Craft not equipped with s-foils");
			}
			break;
...
int button_function(int n)
{
	...
	// if cloaked can't use targeting computer
	if ((Player_ship->flags2 & SF2_PRIMITIVE_SENSORS) || (Player_ship->flags2 & SF2_CLOAK_ON))
	{
		switch (n)
		{
...
//				HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR( "Auto targeting deactivated", -1));
		...
		case END_MISSION:
			...
			if (!ship_has_hyperdrive(Player_obj)) {
				HUD_printf("This craft is not equipped with a hyperdrive");
				gamesnd_play_iface(SND_GENERAL_FAIL);

				break;
			}

			if (Player_ship->jump_anchor >= 0) {
				float	dist;
				float	dot;
				vector	v2v;
				vector *pos = &Jumppoint_lists[Player_ship->jump_anchor].waypoints[0];
			
				dist = vm_vec_normalized_dir(&v2v, pos, &Player_obj->pos);

				if (dist > Player_obj->radius + 100.0f) {
					char str[100];
					sprintf(str, "Jump coordinates located at %s", Jumppoint_lists[Player_ship->jump_anchor].name);
					HUD_printf(str);
					break;
				}
			}
			...
			if (collide_predict_large_ship(Player_obj, 200.0f)) {
				gamesnd_play_iface(SND_GENERAL_FAIL);
				HUD_printf(XSTR( "** WARNING ** Collision danger.  Hyperdrive not activated.", 39));
			} else if (!ship_can_warp(Player_ship)  /*ship_get_subsystem_strength( Player_ship, SUBSYSTEM_ENGINE ) < 0.1f*/) {
				gamesnd_play_iface(SND_GENERAL_FAIL);
				HUD_printf(XSTR( "Engine failure.  Cannot engage hyperdrive.", 40));
			} else {
		...
		case CYCLE_LINK_MODE_GUNS:
			return button_function_critical(CYCLE_LINK_MODE_GUNS);
			break;

		case CYCLE_SFOILS:
			return button_function_critical(CYCLE_SFOILS);
			break;
		
		case ENTER_HANGAR:
			control_used(ENTER_HANGAR);
			if (enter_hangar) {
				int anchor_shipnum;
				int objnum;
				vector v_orient;
...
/*
				on_hangar_approach = 1;
				//time_to_quit_mission = timestamp(2000);
			vector add = Player_obj->pos;
			add.xyz.x = 10.0f;//+= frand_range(-700.0f, 700.0f);
			add.xyz.y = 0.0f;//+= frand_range(-700.0f, 700.0f);
			add.xyz.z = 0.0f;//+= frand_range(-700.0f, 700.0f);

			objnum = ship_create(&vmd_identity_matrix, &add, 4, "Fucker");
			strcpy(Ships[objnum].ship_name, "Fucker");*/
							
				/*if (test1 == 1) {
					//pobjp = mission_parse_get_arrival_ship(Player_ship->ship_name);	
					//parse_create_object(pobjp);

					//Player_obj->orient.vec.xyz.x;
					objnum = ship_create(&Player_obj->orient, &Player_obj->pos, Player_ship_class, Player_ship->ship_name);
					...
					Player_obj->phys_info.fspeed = 0.0f;
					...
					vm_vec_sub(&v_orient, &location, &Objects[objnum].pos);

					vm_vector_2_matrix(&m_orient, &v_orient, NULL, NULL);


					// set orientation -----------------------------
					Objects[objnum].orient = m_orient;
				...
				ai_add_ship_goal_player( AIG_TYPE_PLAYER_WING, AI_GOAL_WARP, AIS_DEPART_TO_BAY, Ships[anchor_shipnum].ship_name, &Ai_info[Player_ship->ai_index] );
				*/
