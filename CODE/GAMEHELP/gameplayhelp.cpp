...
#define TITLE_Y			50//35
#define Y_START			90//70
#define X_OFFSET_1		60//70
...
//static gameplay_help_buttons

ui_button_info Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
//XSTR:OFF
	{
		ui_button_info("F1B_00",	45,375,	0,0, 0),
		ui_button_info("F1B_01",	70,375,	0,0, 1),
		ui_button_info("F1B_02",	540,375, 0,0,	2)
	},
	{
		ui_button_info("2_F1B_00",	24,	622, 0,0,	0),
		ui_button_info("2_F1B_01",	96,	622, 0,0,	1),
		ui_button_info("2_F1B_02",	919,	689, 0,0, 	2)
	},
//XSTR:ON
};
...
static UI_XSTR Game_help_text[GR_NUM_RESOLUTIONS][GAME_HELP_NUM_TEXT] = {
	{ // GR_640
		{ "Press ESC to return to the game",	1441,	199,352,	UI_XSTR_COLOR_GREEN, -1, NULL },
		{ "Continue",		1069,		535,397,	UI_XSTR_COLOR_GREEN, -1, &Buttons[gr_screen.res][CONTINUE_BUTTON].button }
	}, 
	{ // GR_1024
...
//	gr_string(x+KEY_DESCRIPTION_OFFSET,y,ci->text);
	gr_string(x+KEY_DESCRIPTION_OFFSET, y, ci->text);//XSTR(ci->text, CONTROL_CONFIG_XSTR + id));
}
...
void gameplay_help_init()
{
	...
	for (i=0; i<NUM_BUTTONS; i++) {
		// create the object
		Buttons[gr_screen.res][i].button.create(&Ui_window, "", Buttons[gr_screen.res][i].x, Buttons[gr_screen.res][i].y, 1, 1, 0, 1);
...
void gameplay_help_draw_text()
{
	...
	switch ( Current_help_page ) {
		...
		case GP_HELP_BASIC_KEYS:
			...
			y_offset += separation;
			gameplay_help_blit_control_line(x_offset, y_offset,ENTER_HANGAR);
			...
			#ifndef FS2_DEMO
			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset,y_offset, "F5...F8", XSTR( "hotkeys", 144));
			#endif

			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset,y_offset, XSTR( "Shift-Esc", 145), "quit Imperial Alliance immediately");
		...
		case GP_HELP_ADVANCED_TARGET_KEYS:
			...
			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset, y_offset, "F5...F8", XSTR( "Select target assigned to that hotkey", 150));

			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset, y_offset, "Shift-F5...F8", XSTR( "Add/remove target from that hotkey", 152));

			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset, y_offset, "Alt-Shift-F5...F8", XSTR( "Clear that hotkey", 154));
		...
		case GP_HELP_MESSAGING:
			...
			#ifndef FS2_DEMO
			y_offset += separation;
			gameplay_help_blit_control_line_raw(x_offset, y_offset, "F5...F8", XSTR( "send specified order to these target(s)", 156));
			#endif
			break;
...
void gameplay_help_do_frame(float frametime)
{
	...
	// print floating text if needed
	b = &Buttons[gr_screen.res][CONTINUE_BUTTON];
		
	if (b->button.button_hilighted())
	{
		gr_set_color_fast(&Color_bright_white);
			
		gr_string(Game_help_text[gr_screen.res][1].x,Game_help_text[gr_screen.res][1].y,
				  Game_help_text[gr_screen.res][1].xstr);
	}
