...
char *sample_text = "This is some sample text that is here to\n" \
...
int fonttool_edit_kerning(char *font_name)
{	
	int bkg;
	int quit = 0;
	int i, k,x;
	...
	HWND hwnd = FindWindow("FonttoolClass", NULL );
	if ( hwnd )	{
	...
	if (!vm_init(5*1024*1024)) {
		MessageBox( NULL, "Not enough memory to run Fonttool, you have a real problem!\r\nTry closing down some other applications.\r\n", "Not Enough Memory", MB_OK );
		return 0;
	}
	...
	fonttool_read(font_name, &KernFont );
	...
	while (!quit) {
		...
		switch (k) {
		case KEY_F5:
			fonttool_read( font_name, &tmpfont );
		...
		case KEY_F10:
			fonttool_dump( font_name, &KernFont );
			quit=1;
			break;
...

		{
			...
			gr_string( 0x8000, 360, sample_text );
			//gr_string( 20, 360+th+20, sample_text );
		}
