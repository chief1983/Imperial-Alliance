 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:57 $
 * $Author: Darkhill $
 *
 * Freespace main body
 *
 * $Log: freespace.cpp,v $
 * Revision 1.2  2004/09/07 22:56:57  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 20:44:30  Darkhill
 * no message
 *
...
static const char RCS_Name[] = "$Name:  $";
...
#define EXE_FNAME			("imperialalliance.exe")

#define LAUNCHER_FNAME	("IA.exe")
...
// builtin mission list stuff
	...
	fs_builtin_mission Game_builtin_mission_list[MAX_BUILTIN_MISSIONS] = {
		...
		{ "imperialalliance.iac",				(FSB_FROM_VOLITION | FSB_CAMPAIGN_FILE),					"" },
			
		// act 1
		{ "single.iam",					(FSB_FROM_VOLITION | FSB_CAMPAIGN), FS_CDROM_VOLUME_2	},//{ "sm1-01.fs2",					(FSB_FROM_VOLITION | FSB_CAMPAIGN),							FS_CDROM_VOLUME_2	},
...
void run_launcher()
{
	...
	int download = MessageBox((HWND)os_get_window(), 
		"The Imperial Alliance launcher will be started to fix your problem.",
		"Imperial Alliance Startup Error", MB_OK | MB_ICONEXCLAMATION);
	...
	*/
	...
	if (!launcher_ran) 
	{
		download = MessageBox((HWND)os_get_window(), 
			"The Launcher could not be started, you cant run Imperial Allaince without it.",
			"Imperial Alliance Startup Error", MB_OK | MB_ICONEXCLAMATION);
	...
	if(fsspeech_init() == false) {
		...
		if(Cmdline_query_speech)
		{
			if(!fsspeech_was_compiled()) 
				MessageBox((HWND)os_get_window(), "Speech is not compiled in this build in code.lib", "Imperial Alliance Warning", MB_ICONWARNING);
			else
				MessageBox((HWND)os_get_window(), "Speech is compiled, but failed to init", "Imperial Alliance Warning", MB_ICONWARNING);	
		}
	} else if(Cmdline_query_speech) {
		// Its bad practice to use a negative type, this is an exceptional case
		fsspeech_play(-1,"Welcome to Imperial Alliance");
		MessageBox((HWND)os_get_window(), "Speech is compiled and initialised and should be working", "Imperial Alliance Info", MB_OK);
	}

	if (jukebox_init() == false)
		mprintf(("Faild to init jukebox\n"));
...
/////////////////////////////
	...
	if(!Is_standalone)
	{
		...
		if(ptr == NULL)
		{
			strcpy(Device_init_error, "Cant get 'videocard' reg entry.");
		}
		else 
		{
			...
			if (strstr(ptr, NOX("D3D8-") ))	
			{
				if(sscanf(ptr, "D3D8-(%dx%d)x%d bit", &width, &height, &cdepth)  != 3) 
					strcpy(Device_init_error, "Cant understand 'videocard' D3D8 reg entry.");
				else
			...
			else if (strstr(ptr, NOX("OGL-") ))
			{
				if(sscanf(ptr, "OGL-(%dx%d)x%d bit", &width, &height, &cdepth)  != 3) 
					strcpy(Device_init_error, "Cant understand 'videocard' OGL reg entry.");
				else
...
//	ENVMAP = bm_load("environment");
	...
	// just for my own self gratification i wanted to have vader's breath as an intro sound.  the problem is 
	// originally the title screen is displayed before the sounds tbl is parsed into memory.  so i have moved
	// this up here so that the sounds are in memory before the title screen is shown.  if there happens to be 
	// problems with this then move this line back under mouse_init() where it was to begin with.
...
{
	if (on_hangar_approach) {
		// shrink view by 30%
		int yborder = int(gr_screen.max_h/1.3);
	
		gr_set_clip(0, 0, gr_screen.max_w, yborder);//gr_screen.max_h - yborder*2 );
	} else if ((Game_mode & GM_DEAD) || (supernova_active() >= 2)){
	...
	if (!(Game_mode & GM_DEAD_DIED) && ((Game_mode & (GM_DEAD_BLEW_UP)) || ((Last_view_target != NULL) && (Last_view_target != view_target)))) {
		if (view_target != Player_obj){
			...
			case OBJ_JUMP_NODE: {
				...
				strcpy(jump_node_name, XSTR( "hyper buoy", 184));
...
extern int time_to_quit_mission;
...
void game_frame(int paused)
{
	...
	else
	{
		...
		if ( !Pre_player_entry && actually_playing ) {		   		
			if (! (Game_mode & GM_STANDALONE_SERVER) ) {
				...
				// don't know if this is the right place for this or not.  but if enough time has
				// passed since player started to enter the hanger then cut to debriefing screen
				if timestamp_elapsed(time_to_quit_mission) {
	...
	if (!Pre_player_entry) {
		if (! (Game_mode & GM_STANDALONE_SERVER)) {
			...
			// just playing around to see about adding in a 2D HUD
			// overlay.  To put the rendering order back to original:
			// draw 2D HUD
			// game_set_vew_clip()
			// draw 3D HUD
			// game_tst_frame()
			// hud_show_target_model() should go above where it has
...
//		flFrametime /= 15.0;
	...
	if ( game_single_step && (last_single_step == game_single_step) ) {
		...
		while( key_checkch() == 0 )
			os_sleep(10);
		os_set_title("Imperial Alliance");
  		Last_time = timer_get_fixed_seconds();
	}
...
//			if (state != GS_STATE_INITIAL_PLAYER_SELECT) {
		...
		//case KEY_SHIFTED + KEY_LEFT:
			if (Cmdline_UseJukebox == 1)
				play_song(get_prev_song_to_play());
			break;
...
void game_process_event( int current_state, int event )
{
	...
	switch (event) {
		...
		case GS_EVENT_DEBRIEF:
			...
			if(Campaign_ended_in_mission && (Game_mode & GM_CAMPAIGN_MODE) && !stricmp(Campaign.filename, "imperialalliance")) {
...
//				Viewer_mode |= VM_WARP_CHASE;
				...
				Player_ship->hyper_timestamp = timestamp(100);
		...
		case GS_EVENT_PILOT_INFO:
			gameseq_push_state( GS_STATE_PILOT_INFO );
			break;
		case GS_EVENT_PILOT_MANAGE:
			gameseq_push_state( GS_STATE_PILOT_MANAGE );
			break;
...
void game_do_state_common(int state,int no_networking)
{
	...
	// only process the event music if the game music is being used.
	// if player is using the jukebox we don't need to process this.
	if (Cmdline_UseJukebox == 0)
...
void game_do_state(int state)
{
	...
	switch (state) {
		...
		case GS_STATE_PILOT_INFO:
			game_set_frametime(GS_STATE_PILOT_INFO);
			pilot_info_do(flFrametime);
			break;

		case GS_STATE_PILOT_MANAGE:
			game_set_frametime(GS_STATE_PILOT_MANAGE);
			pilot_manage_do();
			break;
...
int game_do_ram_check(int ram_in_bytes)
{
	if ( ram_in_bytes < 30*1024*1024 )	{
		...
		if ( allowed_to_run ) {
			...
			sprintf( tmp, XSTR( "Imperial Alliance has detected that you only have %dMB of free memory.\n\nImperial Alliance requires at least 32MB of memory to run.  If you think you have more than %dMB of physical memory, ensure that you aren't running SmartDrive (SMARTDRV.EXE).  Any memory allocated to SmartDrive is not usable by applications\n\nPress 'OK' to continue running with less than the minimum required memory\n", 193), Freespace_total_ram_MB, Freespace_total_ram_MB);
...
void game_maybe_update_launcher(char *exe_dir)
{
	...
	if ( CopyFile(src_filename, dest_filename, 0) == 0 ) {
		MessageBox( NULL, XSTR("Unable to copy imperialalliance.exe from update directory to installed directory.  You should copy freespace.exe from the update directory (located in your FreeSpace install directory) to your install directory", 988), NULL, MB_OK|MB_TASKMODAL|MB_SETFOREGROUND );
		return;
	}
...
#endif
{
...
#ifdef _WIN32
	// Don't let more than one instance of Imperial Alliance run.
	HWND hwnd = FindWindow( NOX( "ImperialAllianceClass" ), NULL );
	if ( hwnd )	{
	...
	if ( ms.dwTotalVirtual < 1024 )	{
		MessageBox( NULL, XSTR( "Imperial Alliance requires virtual memory to run.\r\n", 196), XSTR( "No Virtual Memory", 197), MB_OK );
		return 0;
	}

	if (!vm_init(24*1024*1024)) {
		MessageBox( NULL, XSTR( "Not enough memory to run Imperial Alliance.\r\nTry closing down some other applications.\r\n", 198), XSTR( "Not Enough Memory", 199), MB_OK );
		return 0;
	}
...
#if defined(_MSC_VER)
	}	__except(RecordExceptionInfo(GetExceptionInformation(), "Imperial Alliance Main Thread"))	{
...
#if defined(MP_DEMO)
	strcat(str, " Imperial Alliance Multiplayer Demo");
#else
	strcat(str, " Imperial Alliance (Closed Beta)");
...
int detect_lang()
{
	...
	CFILE *detect = cfopen("font01.if", "rb");
...
//		return;
	...
	*/
	...
	// scary vader breathing
	gamesnd_play_iface(SND_INTRO);
