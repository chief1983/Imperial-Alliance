 ...
 * $Revision: 1.2 $
 * $Date: 2004/09/07 22:56:12 $
 * $Author: Darkhill $
 *
 * Header file for main-hall menu code
 *
 * $Log: mainhallmenu.cpp,v $
 * Revision 1.2  2004/09/07 22:56:12  Darkhill
 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:46:11  Darkhill
 * no message
 *
...
#define NUM_REGIONS									9				// (6 + 1 for multiplayer equivalent of campaign room)
...
#define NUM_MAIN_HALL_MOUSE_REGIONS 8
...
#define SIM_ROOM_REGION			 6
#define FILM_ROOM_REGION		 7
...
void main_hall_do_multi_ready()
{
	...
	case NETWORK_ERROR_NO_WINSOCK:
		popup( PF_NO_NETWORKING, 1, POPUP_OK, "Winsock is not installed.  You must have TCP/IP and Winsock installed to play multiplayer Imperial Alliance.");
		break;
	case NETWORK_ERROR_NO_PROTOCOL:
		if(Multi_options_g.protocol == NET_TCP){
			popup( PF_NO_NETWORKING, 1, POPUP_OK, XSTR( "TCP/IP protocol not found.  This protocol is required for multiplayer Imperial Alliance.", 362));
		} else {
			Assert(Multi_options_g.protocol == NET_IPX);
			popup( PF_NO_NETWORKING, 1, POPUP_OK, XSTR( "IPX protocol not found.  This protocol is required for multiplayer Imperial Alliance.", 362));
		}
		break;
...
void main_hall_init(int main_hall_num)
{
	...
	// tooltip strings
	Main_hall->region_descript[0] = "Exit Imperial Alliance";
	Main_hall->region_descript[1] = "Barracks - Manage your Imperial Alliance pilots";
	Main_hall->region_descript[2] = "Ready room - Start or continue a campaign";
	Main_hall->region_descript[3] = "Tech room - View specifications of Imperial Alliance ships and weaponry";
	Main_hall->region_descript[4] = "Options - Change your Imperial Alliance options";
	Main_hall->region_descript[5] = "Campaign Room - View all available campaigns";
	// Probably have to put this back in but for now need 6 for our new rooms
	//Main_hall->region_descript[6] = XSTR( "Multiplayer - Start or join a multiplayer game", 359);
	Main_hall->region_descript[6] = "Combat Chamber - Train in simulated environments";
	Main_hall->region_descript[7] = "Film Room!!!!";
	Main_hall->region_descript[8] = "Fly cool multiplayer battles!";
	// init tooltip shader
...
// 0 is for most instances where game just quits
// 1 is for when user clicks exit door and shows credits

void main_hall_exit_game(int screen)
{
...
void main_hall_do(float frametime)
{
	...
	case SNAZZY_CLICKED:
		...
		// new code so simulation room does something
		case SIM_ROOM_REGION:
			if(Player->flags & PLAYER_FLAGS_IS_MULTI){
				gamesnd_play_iface(SND_IFACE_MOUSE_CLICK);
				main_hall_set_notify_string("Simulation Room not valid for multiplayer pilots");
			} else {
				gamesnd_play_iface(SND_IFACE_MOUSE_CLICK);
		...
		case EXIT_REGION:
			gamesnd_play_iface(SND_IFACE_MOUSE_CLICK);
			main_hall_exit_game(1);
//			gameseq_post_event(GS_EVENT_CREDITS);
//			gameseq_post_event(GS_STATE_QUIT_GAME);
//			
			break;
...
void main_hall_start_ambient()
{
	...
	// if the jukebox is being used then we don't want the ambient
	// roar played in the background.
	if (Cmdline_UseJukebox == 1)
		return;
