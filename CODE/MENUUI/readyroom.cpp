 ...
 *
 * Revision 1.2  2004/08/14 03:23:01  Darkhill
 * Testing
 *
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:48:02  Darkhill
 * no message
 *
...
void sim_room_force_button_frame();
void sim_room_maybe_show_button_text();

// hotspot stuff.....
#define NUM_HOTSPOT_AREAS		2
...
static int Start_new_campaign_hotspot_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640   x1, y1, x2, y2
		0, 233, 126, 385
	},
	{ // GR_1024   x1, y1, x2, y2
...
static int Continue_old_campaign_hotspot_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640   x1, y1, x2, y2
		570, 167, 637, 284
	},
	{ // GR_1024   x1, y1, x2, y2
...
void check_campagin_room_hotspot(int x, int y)
{
	char str[50] = "";
	shader shader;
	int shader_y = 460;
	...
	// start new campaign area
	if ((x >= Start_new_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_X1]) && 
		(x <= Start_new_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_X2]))
	{
		if ((y >= Start_new_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_Y1]) && 
			(y <= Start_new_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_Y2]))
		{
			strcpy(str, "Start new campaign");
			show_shader = 1;
		}
	}
	

	// contine old campaign area
	if ((x >= Continue_old_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_X1]) && 
		(x <= Continue_old_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_X2]))
	{
		if ((y >= Continue_old_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_Y1]) && 
			(y <= Continue_old_campaign_hotspot_coords[gr_screen.res][HOTSPOT_MASK_Y2]))
		{
			//str = (char *)malloc(13);
			strcpy(str, "Continue current campaign");
			show_shader = 1;
		}
	}
...
//XSTR:OFF
	{		// GR_640					//1 99
		sim_room_buttons("LMB_04",		565,	47,	-1,		-1,		1),
		sim_room_buttons("LMB_05",		565,	345,	-1,		-1,		0),
		sim_room_buttons("LMB_06",		45,		375,	70,		380,	2),
		sim_room_buttons("LMB_07",		45,		405,	70,		410,	3),
		sim_room_buttons("LMB_08",		485,	375,	500,	440,	4),
		sim_room_buttons("LMB_09",		540,	375,	572,	413,	5),
		sim_room_buttons("LMB_10",		430,	375,	130,	385,	6),
		sim_room_buttons("LMB_11",		375,	375,	-1,		-1,		7),
		//sim_room_buttons("TDB_00",		7,		3,		37,		7,		0),
		//sim_room_buttons("TDB_01",		7,		18,		37,		23,		1),
		//sim_room_buttons("TDB_02",		7,		34,		37,		38,		2),//7,		34,
		//sim_room_buttons("TDB_03",		7,		49,		37,		54,		3),
	},
	{		// GR_1024
		sim_room_buttons("LMB_04",		904,	76,	-1,		-1,		1),
		sim_room_buttons("LMB_05",		904,	552,	-1,		-1,		0),
		sim_room_buttons("LMB_06",		72,		600,	313,	351,	2),
		sim_room_buttons("LMB_07",		72,		648,	315,	392,	3),
		sim_room_buttons("LMB_08",		485,	600,	500,	440,	4),
		sim_room_buttons("LMB_09",		540,	600,	572,	413,	5),
		sim_room_buttons("LMB_10",		430,	600,	480,	462,	6),
		sim_room_buttons("LMB_11",		375,	600,	-1,		-1,		7),
...
#define NUM_TEXT			6
UI_XSTR sim_room_text[GR_NUM_RESOLUTIONS][NUM_TEXT] = {
	{ // GR_640
		{ "Single Missions",	-1,		70,		380,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][MISSION_TAB].button },
		{ "Battle Missions",	-1,		70,		410,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CAMPAIGN_TAB].button },
		{ "Help",				-1,		497,	395,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][HELP_BUTTON].button },
		{ "Fly Mission",		-1,		533,	395,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][COMMIT_BUTTON].button },
		{ "Options",			-1,		434,	395,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][OPTIONS_BUTTON].button },
		{ "Back",				-1,		390,	395,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][OPTIONS_BUTTON].button },
	//	{ "Techical Database",	-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][TECH_DATABASE_BUTTON].button },
	//	{ "Mission Simulator",	1442,	465,	7,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][SIMULATOR_BUTTON].button },
	//	{ "Cutscenes",			-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CUTSCENES_BUTTON].button },
	//	{ "Credits",			-1,		456,	441,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CREDITS_BUTTON].button },
	}, 
	{ // GR_1024
		{ "Single Missions",	-1,		112,	608,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][MISSION_TAB].button },
		{ "Battle Missions",	-1,		112,	656,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CAMPAIGN_TAB].button },
		{ "Help",				-1,		796,	632,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][HELP_BUTTON].button },
		{ "Fly Mission",		-1,		853,	632,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][COMMIT_BUTTON].button },
		{ "Options",			-1,		695,	632,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][OPTIONS_BUTTON].button },
		{ "Back",				-1,		624,	632,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][OPTIONS_BUTTON].button },
	//	{ "Techical Database",	-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][TECH_DATABASE_BUTTON].button },
	//	{ "Mission Simulator",	1442,	465,	7,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][SIMULATOR_BUTTON].button },
	//	{ "Cutscenes",			-1,		-1,		-1,		UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CUTSCENES_BUTTON].button },
	//	{ "Credits",			-1,		456,	441,	UI_XSTR_COLOR_GREEN, -1, &Buttons[0][CREDITS_BUTTON].button },
...
void sim_room_build_listing()
{
	...
	if (Player->readyroom_listing_mode == MODE_MISSIONS) {
		...
		if (!Campaign_mission_names_inited) {  // Is this the first time through
			...
			popup_till_condition(build_campaign_mission_list_do_frame, POPUP_CANCEL, XSTR("Loading battle missions",992));
		} else {
...
void sim_room_init()
{
	...
	*Game_current_mission_filename = 0;
	...
	// add 'single missions' 'battle missions' text
	for(i=0; i<2; i++) {
		Ui_window.add_XSTR(&sim_room_text[gr_screen.res][i]);
	}
...
/*	for (i=TECH_DATABASE_BUTTON; i<=CREDITS_BUTTON; i++){
	...
	sim_room_force_button_frame();
	sim_room_maybe_show_button_text();
...
int Cr_list_coords[GR_NUM_RESOLUTIONS][4] = {
	{ // GR_640
		180, 127, 342, 159//100, 21, 565, 233
	},
	{ // GR_1024
		64, 34, 916, 459
	}
...
ui_button_info Cr_buttons[GR_NUM_RESOLUTIONS][CR_NUM_BUTTONS] = {
	{ // GR_640
		ui_button_info("CAB_00",	308,	403,	-1,	-1,	0),
		ui_button_info("CAB_01",	308,	435,	-1,	-1,	1),
		ui_button_info("CAB_02",	495,	33,	-1,	-1,	2),
		ui_button_info("CAB_03",	495,	86,	-1,	-1,	3),
		ui_button_info("CAB_04",	0,		230,	-1,	-1,	4),
		ui_button_info("CAB_05",	570,165,	-1,	-1,	5),
	},
	{ // GR_1024
...
/*UI_XSTR Cr_text[GR_NUM_RESOLUTIONS][CR_NUM_TEXT] = {
	{ // GR_640
		...
		{ "campaign66",		1404,		569,	337, UI_XSTR_COLOR_GREEN,	-1, &Cr_buttons[0][CR_RESET_BUTTON].button },
...
void campaign_room_commit()
{
	...
	if (stricmp(Campaign_file_names[Selected_campaign_index], Campaign.filename)) {  // new campaign selected
		...
		if ( (Campaign.flags & CF_CUSTOM_TECH_DATABASE) || !stricmp(Campaign.filename, "imperialalliance") )
		{
...
//	Ui_window.draw();
	...
	// get mouse coords and check if in hotspot
	int x;
	Hotspot_region.get_mouse_pos(&x, &y);
	if (!help_overlay_active(CAMPAIGN_ROOM_OVERLAY)) 
		check_campagin_room_hotspot(x, y);
...
void sim_room_force_button_frame()
{
	for (int x=0; x<NUM_BUTTONS; x++) {
		// don't want to force the checkmark boxes
		if (x == MISSION_TAB || x == CAMPAIGN_TAB)
			continue;
...
void sim_room_maybe_show_button_text()
{
	...
	// don't worry about first two strings in list
	for (int i=4; i<NUM_BUTTONS; i++) {
		b = &Buttons[gr_screen.res][i];

		if (b->button.button_hilighted()) {
			gr_set_color_fast(&Color_bright_white);
			gr_string(sim_room_text[gr_screen.res][i-2].x, sim_room_text[gr_screen.res][i-2].y, sim_room_text[gr_screen.res][i-2].xstr);
		}
	}
}
