 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:40:40  Darkhill
 * no message
 *
...
//   Operator, Identity, Min / Max arguments
	...
	{ "depart-jump-delay",					OP_DEPART_JUMP_DELAY,				3, INT_MAX, },	
...
//	{ "is-friendly-stealth-visible",		OP_IS_FRIENDLY_STEALTH_VISIBLE,	1, 1, },
	...
	{ "change-team",				OP_CHANGE_TEAM,					2, INT_MAX,		},
...
//	{ "friendly-stealth-visible",	OP_FRIENDLY_STEALTH_VISIBLE,	1, INT_MAX },	// Goober5000
	...
	{ "break-hyperdrive",					OP_WARP_BROKEN,					1, INT_MAX,	},
	{ "fix-hyperdrive",					OP_WARP_NOT_BROKEN,				1, INT_MAX,	},
	{ "never-hyper",					OP_WARP_NEVER,					1, INT_MAX, },
	{ "allow-hyper",					OP_WARP_ALLOWED,				1, INT_MAX, },
...
*/
	...
	{ "ai-chase-flight-group",			OP_AI_CHASE_WING,				2, 2, },
	...
	{ "ship-cloak-on",			OP_CLOAK_ON, 1, INT_MAX },
	{ "ship-cloak-off",			OP_CLOAK_OFF, 1, INT_MAX },
...
sexp_ai_goal_link Sexp_ai_goal_links[] = {
	...
	{ AI_GOAL_DISABLE, OP_AI_DISABLE_SHIP },
	{ AI_GOAL_DISABLE_ENGINES, OP_AI_DISABLE_ENGINES },
	...
	while (index != -1) {
		...
		switch (type) {
			...
			case OPF_TEAM:
				...
				if (i == Num_team_names)
				{
					return SEXP_CHECK_INVALID_TEAM;
				}
			...
			case OPF_IFF:
				...
				for (i=0; i<MAX_IFF; i++)
				{
					if (!stricmp(Iff_names[i], CTEXT(index)))
					{
						break;
					}
				}
...
// function to return the remaining system hits left as a percentage of the whole.  
int sexp_system_left(int n)
{
	int shipnum, percent;
	char *shipname;
	...
	ship		*shipp = &Ships[shipnum];
	object	*objp = &Objects[shipp->objnum];
	percent = (int) (100.0f * objp->system_strength /  shipp->ship_initial_system_strength);
	return percent;
}
...
// function to tell is a list of ships has departed from within a radius of a given hyper buoy.
// returns true N seconds after the list of ships have departed
int sexp_depart_jump_delay( int n )
{
...
{
	char *ship_or_wing_name, *iff_text;
	int i, ship_num, wing_num, iff = -1;
	wing *wingp;
	...
	for (i=0; i<MAX_IFF; i++) {
		if (!stricmp(iff_text, Iff_names[i]))
			iff = i;
	}
	
	if (iff == -1) {
		Int3();
		mprintf(("Warning: IFF %s no longer supported.  Just Friendly and Hostile.\n", iff_text));
		iff = IFF_COLOR_IMPERIAL;
	}
	...
	for ( ; n != -1; n = CDR(n) )
	{
		...
		if (wing_num != -1)
		{
			...
			{
				// if the team doesn't match the team specified, return false immediately
				if (!ship_is_iff(wingp->ship_index[i], iff))
				{
					return SEXP_FALSE;
				}
			}
...
{
	char *ship_or_wing_name, *new_iff_text;
	int i, ship_num, wing_num, new_iff = -1;
	wing *wingp;

	Assert ( n >= 0 );
	new_iff_text = CTEXT(n);
	
	for (i=0; i<MAX_IFF; i++) {
		if (!stricmp(new_iff_text, Iff_names[i]))
			new_iff = i;
	}
		
	if (new_iff == -1) {
		mprintf(("Warning: IFF %s no longer supported.  Just Rebel and Imperial.\n", new_iff_text));
		new_iff= IFF_COLOR_IMPERIAL;
		Int3();
	}
	...
	for ( ; n != -1; n = CDR(n) )
	{
		...
		if (ship_num != -1)
		{
			ship_change_iff(ship_num, new_iff);
		}
		...
		if (wing_num != -1)
		{
			...
			for (i=0; i<wingp->current_count; i++)
			{
				ship_change_iff(wingp->ship_index[i], new_iff);
			}
		}
	}
}

int sexp_is_team( int n )
{
	char *ship_or_wing_name, *team_text;
	int i, ship_num, wing_num, team;
	wing *wingp;

	Assert ( n >= 0 );

	// team value is the first parameter, second is a list of one or more ships to check to see if the
	// team value matches
	team_text = CTEXT(n);
	if ( !stricmp(team_text, "friendly") )
		team = TEAM_FRIENDLY;
	else if ( !stricmp(team_text, "hostile") )
		team = TEAM_HOSTILE;
	else if ( !stricmp(team_text, "neutral") )
		team = TEAM_NEUTRAL;
	else if ( !stricmp(team_text, "unknown") )
		team = TEAM_UNKNOWN;
	else if ( !stricmp(team_text, "traitor") )
		team = TEAM_TRAITOR;
	else {
		Int3();
	...
	for ( ; n != -1; n = CDR(n) )
	{
		...
		if (wing_num != -1)
		{
			...
			{
				// if the team doesn't match the team specified, return false immediately
				if (!ship_is_team(wingp->ship_index[i], team))
				{
					return SEXP_FALSE;
				}
			}
...
void sexp_change_team( int n )
{
	char *ship_or_wing_name, *new_team_text;
	int i, ship_num, wing_num, new_team;
	wing *wingp;

	Assert ( n >= 0 );
	new_team_text = CTEXT(n);
	if ( !stricmp(new_team_text, "friendly") )
		new_team = TEAM_FRIENDLY;
	else if ( !stricmp(new_team_text, "hostile") )
		new_team = TEAM_HOSTILE;
	else if ( !stricmp(new_team_text, "neutral") )
		new_team = TEAM_NEUTRAL;
	else if ( !stricmp(new_team_text, "unknown") )
		new_team = TEAM_UNKNOWN;
	else if ( !stricmp(new_team_text, "traitor") )
	...
	for ( ; n != -1; n = CDR(n) )
	{
		...
		if (ship_num != -1)
		{
			ship_change_team(ship_num, new_team);
		}
		...
		if (wing_num != -1)
		{
			...
			for (i=0; i<wingp->current_count; i++)
			{
				ship_change_team(wingp->ship_index[i], new_team);
			}
		}
	}
}
...
void sexp_end_campaign( int n )
{
	...
	if ((Game_mode & GM_CAMPAIGN_MODE) && !stricmp(Campaign.filename, "imperialalliance")) {
...
void sexp_transfer_cargo( int n )
{
	...
	if (! (Ship_info[Ships[shipnum2].ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP | SIF_MEDIUM_SHIP)) ) {
...
void sexp_set_cloak(int node, int status)
{
	int ship_num;		
	...
	// turn on or off, depending on flag
	if (status)
		shipfx_start_cloak(&Ships[ship_num], Tertiary_weapon_info.cloak_warmup);
	else
		shipfx_stop_cloak(&Ships[ship_num], Tertiary_weapon_info.cloak_cooldown);
}
...
int eval_sexp(int cur_node)
{
	...
	if (Sexp_nodes[cur_node].first != -1) {
		...
		// boolean operators can have one of the special sexp values (known true, known false, unknown)
			...
			case OP_DEPART_JUMP_DELAY:
				sexp_val = sexp_depart_jump_delay( node );
				break;
...
*/
			...
			case OP_CLOAK_OFF:
				sexp_val = 1;
				sexp_set_cloak(node, op_num == OP_CLOAK_ON);
				break;
...
char *sexp_error_message(int num)
{
	switch (num) {
		...
		case SEXP_CHECK_INVALID_IFF:
			return "Invalid iff name";

		case SEXP_CHECK_INVALID_TEAM:
		...
		case SEXP_CHECK_INVALID_JUMP_NODE:
			return "Invalid Hyper Buoy name";
