 ...
 *
 * $Log: playercontrol.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:43  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:58:41  Darkhill
 * no message
 *
		...
		if ( button_info_query(&Player->bi, ONE_THIRD_THROTTLE) ) {
			...
			override_analog_throttle = 1;
				...
				if((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL) && (!(Net_player->flags & NETINFO_FLAG_OBSERVER) || !(Player_obj->type == OBJ_OBSERVER)) ){
					Net_player->s_info.speed = ci->forward_cruise_percent;
				}
			#endif
		}
...
//		}
		...
		if ( button_info_query(&Player->bi, FIRE_POD) ) {
		check_control(FIRE_POD);
			if (Player_ship->flags2 & SF2_DISABLED)
				gamesnd_play_iface(SND_GENERAL_FAIL);
			else
				ci->fire_pod = 1;
		}

	/*	if ( button_info_query(&Player->bi, CYCLE_SFOILS) ) {
			check_control(CYCLE_SFOILS);
			if (Player_ship->flags2 & SF2_DISABLED)
				gamesnd_play_iface(SND_GENERAL_FAIL);
			else {
				// make sure that ship class has sfoils to open
				if (Ship_info[Player_ship->ship_info_index].sfoils) {

					if (Player_ship->sfoil_status & SFOIL_CLOSED)  {
						...
						//if ( MULTIPLAYER_MASTER ) {
							int status = bitmask_2_bitnum(Player_ship->sfoil_status);
							send_ship_sfoil_update_packet(Player_ship, status);
						//}	
					#endif
				}
				else
		...
		/*if ( button_info_query(&Player->bi, CYCLE_LINK_MODE_GUNS) ) {
			check_control(CYCLE_LINK_MODE_GUNS);

			polymodel *po = model_get(Player_ship->modelnum);
			...
			// list of link types
			if (po->gun_banks[Player_ship->weapons.current_primary_bank].num_slots <= 3) {			
				if (Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] == WEAPON_LINK_SINGLE) {
					Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] = WEAPON_LINK_ALL;
					// need to set the firing point back to the first hard point or weird things happen
					Player_ship->last_fired_point[Player_ship->weapons.current_primary_bank] = 0;
					HUD_printf("Cannons fire linked");
				}
				else {
					Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] = WEAPON_LINK_SINGLE;
					Player_ship->last_fired_point[Player_ship->weapons.current_primary_bank] = 0;
				...
				if (Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] == WEAPON_LINK_SINGLE) {
					Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] = WEAPON_LINK_DUAL;
					// need to set the firing point back to the first hard point or weird things happen
					Player_ship->last_fired_point[Player_ship->weapons.current_primary_bank] = 0;
					HUD_printf("Cannons dual linked");
				}
				else if (Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] == WEAPON_LINK_DUAL) {
					Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank] = WEAPON_LINK_ALL;
					Player_ship->last_fired_point[Player_ship->weapons.current_primary_bank] = 0;
					HUD_printf("Cannons fire linked");
				}
			...
			if (Game_mode & GM_MULTIPLAYER) {
				...
				send_ship_barrel_link_packet(Player_ship, Player_ship->primary_link_status[Player_ship->weapons.current_primary_bank], Player_ship->weapons.current_primary_bank);
		...
		/*if (button_info_query(&Player->bi, CYCLE_LINK_MODE_MISSILES)) {
			check_control(CYCLE_LINK_MODE_MISSILES);
		...
		/*if (button_info_query(&Player->bi, ENTER_HANGAR)) {
			check_control(ENTER_HANGAR);
			//if (enter_hangar)
				HUD_printf("good for hangar");
		}*/
...
*/
		...
		//if (pi->flags & PF_CLOAK_ON) {
		//	HUD_printf("Weapons offline while cloaked.");
		//}
		//else {
			if (check_control(FIRE_PRIMARY)) {
				if (Player_ship->flags2 & SF2_CLOAK_ON)   // pi->flags & PF_CLOAK_ON) 
					gamesnd_play_iface(SND_GENERAL_FAIL);
				else if (Player_ship->flags2 & SF2_DISABLED)
					gamesnd_play_iface(SND_GENERAL_FAIL);
				else
...
//		ci->fire_primary_count += mouse_down(1);
		...
		if (check_control(FIRE_SECONDARY)) {
			if (Player_ship->flags2 & SF2_CLOAK_ON) //if (pi->flags & PF_CLOAK_ON)
				gamesnd_play_iface(SND_GENERAL_FAIL);
			else if (Player_ship->flags2 & SF2_DISABLED)
				gamesnd_play_iface(SND_GENERAL_FAIL);
			else
...
#ifndef NO_NETWORK
			// if we're a multiplayer client, set our accum bits now
			if((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL) && !(Net_player->flags & NETINFO_FLAG_AM_MASTER) && !(Player_ship->flags2 & SF2_DISABLED)){
			...
			if ( (objp->type == OBJ_SHIP) && (!(Game_mode & GM_DEAD)) )	{
				...
				if ( Warpout_forced )	{
					...
					if ( Ships[objp->instance].flags & ( SF_WARP_BROKEN|SF_WARP_NEVER ) ) {
						HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR( "Cannot hyper out at this time.", 81));
						warp_failed=1;
					} else {
						int	can_warp = 0;
						if ( (!warp_failed) && (Ships[objp->instance].current_max_speed >= TARGET_WARPOUT_SPEED) )	{
							...
							if (Ship_info[Ships[objp->instance].ship_info_index].max_overclocked_speed < TARGET_WARPOUT_SPEED) {
								...
								warp_failed=1;
								HUD_sourced_printf(HUD_SOURCE_HIDDEN, "Unable to engage hyperdrive... ship must be able to reach %.1f MGLT", TARGET_WARPOUT_SPEED );
								can_warp = 0;
							} else {
...
int player_inspect_cargo(float frametime, char *outstr)
{
	...
	if ( Player_ai->current_target_distance < max(CARGO_REVEAL_MIN_DIST, (cargo_objp->radius+CARGO_RADIUS_DELTA)) ) {
		...
		// anything here that is commented out is remove the restriction that you must be facing
		// the object to scan it.  now if you are just withing scanning distance you will scan
		// the object
