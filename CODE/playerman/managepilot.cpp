 ...
 * manage the pilot
 *
 * $Log: managepilot.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:43  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:58:21  Darkhill
 * no message
 *
...
#define LOWEST_COMPATIBLE_PLAYER_FILE_VERSION		142	// compatible with release - Goober5000
...
// version 142 : added stuff for TC pilots and FCHG ranks.  clear out old pilot files

// search for PLAYER INIT for new pilot initialization stuff. I _think_ its in the right spot for now
#define PLR_FILE_ID	'FPAI'	// unique signiture to identify a .PLR file (Imperial Alliance Player File)  // FPAI appears as IAPF in file.
...
int read_pilot_file(char *callsign, int single, player *p)
{
	...
	// read in pilot quote
//	cfread_string_len(p->quote, MAX_QUOTE_LENGTH, file);

	// read if TC pilot
//	p->tc_pilot = cfread_int(file);
...
int write_pilot_file_core(player *p)
{
   ...
   int i, si_index, idx;
	...
	// write quote
//	cfwrite_string_len(p->quote, file);

	// write if TC pilot
//	cfwrite_int(p->tc_pilot, file);
...
void fuck(void *string) {
	char x[] = "fuck this shit";
...
#error Code in ManagePilot assumes NUM_DEFAULT_DETAIL_LEVELS = 4
	...
	p->stats.score = 0;
	p->stats.rank = RANK_CT;
	p->stats.fchg = RANK_NONE;

	p->tips = 1;

	// since only TC pilots will have a quote might as well make this empty
//	strcpy(p->quote, "");

	// default to not a TC pilot, will changed elsewhere if this pilot is from TC
//	p->tc_pilot = 0;
	...
	if(Cmdline_gimme_all_medals == 1){ 
		for(int idx=0; idx < NUM_MEDALS; idx++){
			p->stats.medals[idx] = 1;
			p->stats.fchg = MAX_FCHG_RANK;
			
			//Player_score->kill_count = 500;
			//scoring_eval_badges(Player_score);
		}
	}

	if(Cmdline_make_me_admiral == 1){
		p->stats.rank = MAX_IA_RANK;
	}
