 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:58:13  Darkhill
 * no message
 *
...
#define PF_TURBO_ON			(1 << 15)
#define PF_SLAM_ON			(1 << 16)
#define PF_IN_TRACTOR_BEAM	(1 << 17)	// ship is in tractor beam and can't move
//#define PF_DECOY_ON			(1 << 17)
//#define PF_CLOAK_ON			(1 << 18)
//#define PF_TRACTOR_ON		(1 << 19)
//#define PF_JAMMER_ON		(1 << 20)
...
typedef struct physics_info {
	...
	float		turbo_forward_accel_time_const;
...
typedef struct control_info {
	...
	int fire_pod;						// has player activited the p.o.d.?

	// afterburner control information
	int	afterburner_start;
	int	afterburner_stop;

	int enter_hangar;				// 1 if player within radius of hanger to be tractored in
