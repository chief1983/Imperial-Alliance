 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:58:11  Darkhill
 * no message
 *
...
*/
	// got a whole lot of shakin' going on while the booster is active
	if (pi->flags & PF_BOOSTER_ON) {
		float	max_speed;

		max_speed = vm_vec_mag_quick(&pi->max_vel);
		tangles.p += (float) (rand()-RAND_MAX/2)/RAND_MAX * pi->speed/max_speed/128.0f;
		tangles.h += (float) (rand()-RAND_MAX/2)/RAND_MAX * pi->speed/max_speed/128.0f;
	}
...
//		Int3();
//	}
	if (pi->flags & PF_IN_TRACTOR_BEAM) {
		ci->pitch = 0;						// -1.0 to 1.0					
		ci->vertical = 0;
		ci->heading = 0;
		ci->sideways = 0;
		ci->bank = 0;
...
//	mprintf(("ci->forward == %7.3f\n", ci->forward));
	...
	// no Player_ship object while fred is running so checking for the flag is a very bad thing to do
	if (!Fred_running) {
		// if player ship is disabled make sure speed is at 0 and exit before any control surfaces can be changed
	...
	else if (pi->flags & PF_TURBO_ON) {
		goal_vel.xyz.x = ci->sideways*pi->turbo_max_vel.xyz.x;
		goal_vel.xyz.y = ci->vertical*pi->turbo_max_vel.xyz.y;
		goal_vel.xyz.z = ci->forward* pi->turbo_max_vel.xyz.z;
	}
...
//			Int3();	// Get dave A
		...
		if ( goal_vel.xyz.z >= pi->prev_ramp_vel.xyz.z )  {
			if ( pi->flags & PF_AFTERBURNER_ON )
				...
				ramp_time_const = pi->turbo_forward_accel_time_const;
			else if (pi->flags & PF_BOOSTER_ON)
