 ...
 *
 * $Log: missionlog.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:51:50  Darkhill
 * no message
 *
	...
	if ( type == LOG_SHIP_DESTROYED ) {
		for (i = 0; i < last_entry; i++) {
			...
			if ( !stricmp( pname, entry->pname ) ) {
				if ( (entry->type == LOG_SHIP_SUBSYS_DESTROYED) || (entry->type == LOG_SHIP_DISARMED) || (entry->type == LOG_SHIP_DISABLED_ENGINES) )
...
void mission_log_add_entry(int type, char *pname, char *sname, int info_index)
{
	...
	case LOG_SHIP_DISABLED_ENGINES:
...
void message_log_init_scrollback(int pw)
{
	...
	for (i=0; i<last_entry; i++) {
		...
		switch (entry->type) {
			...
			case LOG_SHIP_DISABLED:
				message_log_add_segs(XSTR( "Disabled", -1), LOG_COLOR_NORMAL);
				break;
			...
			case LOG_SHIP_DISABLED_ENGINES:
				message_log_add_segs(XSTR( "Engines disabled", -1), LOG_COLOR_NORMAL);
				break;
