 ...
 *
 * $Log: missionhotkey.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:50:31  Darkhill
 * no message
 *
...
void hotkey_force_button_frame(int, int);
...
static UI_XSTR Hotkey_text[GR_NUM_RESOLUTIONS][HOTKEY_NUM_TEXT] = {
	{ 
		// GR_640
		{ "Cancel",		1516,	47,397,		UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][CANCEL_BUTTON].button },
		{ "Clear",		1517,	106,397,		UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][CLEAR_BUTTON].button },
		{ "Reset",		1518,	161,397,	UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][RESET_BUTTON].button },
		{ "Help",		1519,	495,397,	UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][HELP_BUTTON].button },
		{ "Options",	1520,	429,397,	UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][OPTIONS_BUTTON].button },
		{ "Accept",		1521,	541,397,	UI_XSTR_COLOR_GREEN, -1, &Buttons[GR_640][ACCEPT_BUTTON].button }
	}, 
	{ 
		// GR_1024
...
/*
	...
	// force mouse over picture to appear
	for (i=0; i<NUM_BUTTONS; i++)
		hotkey_force_button_frame(i, 1);

	// show floating text if needed
	for (i=2; i<=4; i++) 
	{
		b = &Buttons[gr_screen.res][i];
		...
		if (b->button.button_hilighted())
		{
			...
			gr_string(Hotkey_text[gr_screen.res][i-2].x, Hotkey_text[gr_screen.res][i-2].y,
					  Hotkey_text[gr_screen.res][i-2].xstr);
		}
	}
		...
		for (i=7; i<=9; i++) 
	{
		...
		if (b->button.button_hilighted())
		{
			...
			gr_string(Hotkey_text[gr_screen.res][i-4].x, Hotkey_text[gr_screen.res][i-4].y,
					  Hotkey_text[gr_screen.res][i-4].xstr);
		}
	}
...
void hotkey_force_button_frame(int n, int frame_num)
{
