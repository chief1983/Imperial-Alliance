 ...
 *
 * $Log: missionmessage.cpp,v $
 * Revision 1.1.1.1  2004/08/13 22:47:41  Spearhawk
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:50:18  Darkhill
 * no message
 *
		...
		if ( !stricmp(id, Messages[i].name) ) {
			...
			if ( m_source == MESSAGE_SOURCE_COMMAND ) {
				...
				if ( ship_index == -1 ) {
					...
					source = HUD_get_iff_source(Ships[ship_index].iff);
				}
				...
				source = HUD_get_iff_source(shipp->iff);
