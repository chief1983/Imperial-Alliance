 ...
 * no message
 *
 * Revision 1.1.1.1  2004/08/13 21:01:12  Darkhill
 * no message
 *
	...
	while(!optional_string("#end")){
		...
		if(optional_string("$Bitmap:")){
			...
			if(count < MAX_STARFIELD_BITMAPS){
				...
				bm->xparent = 0;
				// when in the actual game only load in bitmaps that are needed for the mission
				// for imped load in everything at startup time so they are available for the background editor
				if (Fred_running) {
...
{	
	...
	for(idx=0; idx<Num_suns; idx++){		
		...
		if(!g3_draw_bitmap(&sun_vex, 0, 0.05f * Suns[idx].scale_x * local_scale, TMAP_FLAG_TEXTURED, 100)){
			Sun_drew++;
		}
	}
}
...
void stars_page_in()
{
	...
	while((idx < MAX_STARFIELD_BITMAPS) /*&& (Starfield_bitmaps[idx].bitmap != -1)*/){	
		// actually don't even load the bitmap into memory unless its going to be used in this mission
		for (t=0; t<Num_starfield_bitmaps; t++) {
			if (!stricmp(Starfield_bitmaps[idx].filename, Starfield_bitmap_instance[t].filename)) {
				bm = &Starfield_bitmaps[idx];
				bm->bitmap = -1;
				bm->bitmap = bm_load(bm->filename);
	...
	// commented out because sometimes it makes the game crash on level load.  no clue really 
	// why, somebody who knows what they are doing should take a look at it sometime
