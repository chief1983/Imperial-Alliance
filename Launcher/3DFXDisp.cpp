 ...
 */
void C3DFXDisp::OnApply(char *reg_path)
{
	...
	if(reg_set_sz(reg_path, "Videocard", glide_modes[index].text_desc) == false)
	{
		MessageBox("Failed to set graphic mode", "Error", MB_ICONERROR);
	}

	//if(reg_set_sz(reg_path, "VideocardFs2open", glide_modes[index].text_desc) == false)
	//{
	//	MessageBox("Failed to set graphic mode", "Error", MB_ICONERROR);
	//}
}
 ...
 */
void C3DFXDisp::LoadSettings(char *reg_path)
{
	char videocard_string[MAX_PATH];

	// Lets get those video card settings
	if(reg_get_sz(reg_path, "VideocardFs2open", videocard_string, MAX_PATH) == false)
	{
		return;
	}
