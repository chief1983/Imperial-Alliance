...
void CTabSpeech::OnApply(char *reg_path, char *settings_file)
{
	...
	reg_set_dword(reg_path, "SpeechTechRoom", use);

	use = (m_checkbox_briefings.GetCheck() == CHECKED);
	reg_set_dword(reg_path, "SpeechBriefings", use);

	use = (m_checkbox_ingame.GetCheck() == CHECKED);
	reg_set_dword(reg_path, "SpeechIngame", use);

	m_speech_volume = 100;
	reg_set_dword(reg_path, "SpeechVolume", m_speech_volume);
	...
	reg_set_dword(reg_path, "SpeechVoice", selected);
}

void CTabSpeech::LoadSettings(char *reg_path)
{
	...
	DWORD use;
	if(reg_get_dword(reg_path, "SpeechTechRoom", &use) && use == 0) {
		m_checkbox_techroom.SetCheck(0);
	}

	if(reg_get_dword(reg_path, "SpeechBriefings", &use) && use == 0) {
		m_checkbox_briefings.SetCheck(0);
	}

	if(reg_get_dword(reg_path, "SpeechIngame", &use) && use == 0) {
		m_checkbox_ingame.SetCheck(0);
	}

	if(reg_get_dword(reg_path, "SpeechVolume", &m_speech_volume) == false) {
		m_speech_volume = 100;
	}
	...
	DWORD selected;
	reg_get_dword(reg_path, "SpeechVoice", &selected);
...
{
	MessageBox("The Imperial Alliance project is not responsible for the content of this site or the functionality of the installer on it", "Warning", MB_ICONWARNING);	
	WinExec("explorer.exe \"http://xtras.timehouse.fi/sapidoc.asp?con=6\"",SW_SHOW);
}
