...
BOOL CTabVideo::InitTabControl(int type)
{
	...
	api_dialogs[0] = &tab_dx8_disp; 
	api_dialogs[1] = &tab_ogl_disp; 
//  api_dialogs[2] = &tab_dx5_disp;
//	api_dialogs[3] = &tab_3dfx_disp;
 ...
 */
void CTabVideo::OnApply(char *reg_path, int flags)
{
	// Take note of checkbox options
	int use_gf4_fix = (m_gf4_fix_button.GetCheck() == 0) ? 0 : 1;
		
	reg_set_dword(reg_path, "D3DTextureOrigin", use_gf4_fix);

	reg_set_dword(reg_path, "D3DFast", 0);
	reg_set_dword(reg_path, "ComputerSpeed", NUM_LEVELS - m_gfx_level_droplist.GetCurSel());

	int use_large_textures = (m_largetxt_checkbox.GetCheck() == 0) ? 0 : 2;
	reg_set_dword(reg_path, "D3DUseLargeTextures", use_large_textures);
	...
	switch(m_last_tab)
	{
		case -1:			
			...
			return;
		case GR_OPENGL:		tab_ogl_disp.OnApply(reg_path);  break;	  
		case GR_DIRECT3D8:	tab_dx8_disp.OnApply(reg_path, flags);  break;
//		case GR_DIRECT3D5:	tab_dx5_disp.OnApply(reg_path);  break;
//		case GR_GLIDE:		tab_3dfx_disp.OnApply(reg_path); break;
	}
}
 ...
 * @param char *path
 */
void CTabVideo::Update(int type, int flags, char *path)
{
	...
	// Make a copy so we can substitute the exe name with the hi res pack name
	char exe_path[MAX_PATH];
	strcpy(exe_path, path);
	char *vp_path = cap_file_from_path(exe_path);

	if(vp_path == NULL || vp_path == exe_path)
	{
		SelectTab(-1);
		return;
	}

	strcpy(vp_path, "sparky_hi_fs2.vp");

	InitTabControl(flags);

	tab_dx8_disp.UpdateAdapterList();
	tab_ogl_disp.UpdateResList();

	// Notify tabs that need to know if hi res pack is installed
	m_hi_sparky_checkbox.SetCheck((flags & FLAG_FS1) || file_exists(exe_path));
}
 ...
 */
void CTabVideo::LoadSettings(char *reg_path, int flags)
{
	DWORD gf4_fix;
	if(reg_get_dword(reg_path, "D3DTextureOrigin", &gf4_fix) == true)
	{
		m_gf4_fix_button.SetCheck(gf4_fix ? 1 : 0);
	}	

	DWORD large_textures;
	if(reg_get_dword(reg_path, "D3DUseLargeTextures", &large_textures) == true)
	{
	...
	DWORD level_value;
	reg_get_dword(reg_path, "ComputerSpeed", &level_value);

	level_value = NUM_LEVELS - level_value;	
	m_gfx_level_droplist.SetCurSel(level_value);

	// Update this for later (switching video tabs)
	strcpy(m_int_reg_path, reg_path);

	int api = 0;
	{
		char api_name[1024];
		// If fs2_open videocard is not setup
		if(reg_get_sz(reg_path, "Videocard", api_name, 1024) == false)
		{
			// Use default videocard entry
			if(reg_get_sz(reg_path, "Videocard", api_name, 1024) == false)
			{
				api = SelectAPI(GR_DIRECT3D8);
				SelectTab(api);
				return;
			}
		}
	...
	switch(api)
	{
		case GR_OPENGL:		tab_ogl_disp.LoadSettings(m_int_reg_path); break;	  
//		case GR_GLIDE:		tab_3dfx_disp.LoadSettings(m_int_reg_path); break;
//		case GR_DIRECT3D5:	tab_dx5_disp.LoadSettings(m_int_reg_path); break;
		case GR_DIRECT3D8:	tab_dx8_disp.LoadSettings(m_int_reg_path, flags); break;
	}
